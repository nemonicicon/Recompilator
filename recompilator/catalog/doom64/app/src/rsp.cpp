/**
 * rsp.cpp — RSP microcode dispatch for DOOM64PC (Doom64 64, USA). Game #4.
 *
 * AUDIO: Doom64 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void doom64_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* doom64_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[doom64pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = doom64_get_rsp_microcode,
        .capture_gfx = doom64_run_gfx_lle,
    };
}
