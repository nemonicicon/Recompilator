/**
 * rsp.cpp — RSP microcode dispatch for MK64PC (Mk64 64, USA). Game #4.
 *
 * AUDIO: Mk64 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void mk64_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* mk64_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[mk64pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = mk64_get_rsp_microcode,
        .capture_gfx = mk64_run_gfx_lle,
    };
}
