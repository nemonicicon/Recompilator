...
static constexpr uint64_t WAYNEGRETZKYS3DHOC_ROM_HASH        = 0x2DC7177C75122199ULL;
static constexpr uint32_t WAYNEGRETZKYS3DHOC_ENTRYPOINT_VRAM = 0x80000400;
...
static void waynegretzkys3dhoc_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        waynegretzkys3dhoc_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        waynegretzkys3dhoc_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void waynegretzkys3dhoc_message_box(const char* msg) {
    waynegretzkys3dhoc_show_error("Wayne Gretzky's 3D Hockey — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI waynegretzkys3dhoc_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in waynegretzkys3dhocpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in waynegretzkys3dhocpc.exe (base=0x%p), nearest first:\n",
...
static void waynegretzkys3dhoc_vi_callback() {}
static void waynegretzkys3dhoc_gfx_init_callback() {}

static std::string waynegretzkys3dhoc_get_thread_name(const OSThread* t) {
    return "waynegretzkys3dhoc_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        if (!GetOpenFileNameA(&ofn)) {
            waynegretzkys3dhoc_show_error("ROM Required",
    ...
    std::u8string out_id = u8"waynegretzkys3dhoc_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        waynegretzkys3dhoc_show_error("Wrong ROM",
            "The selected ROM does not match Wayne Gretzky's 3D Hockey.\n"
            "Expected SHA1: 1af0be9568f8ace865d5cf76fd241da741154b0f");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        waynegretzkys3dhoc_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        waynegretzkys3dhoc_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int waynegretzkys3dhoc_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(waynegretzkys3dhoc_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "waynegretzkys3dhocpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "waynegretzkys3dhocpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "waynegretzkys3dhocpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[waynegretzkys3dhocpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "waynegretzkys3dhocpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[waynegretzkys3dhocpc] stdout log started\n"); }
    }
    ...
    entry.game_id            = u8"waynegretzkys3dhoc_us";
    entry.mod_game_id        = "";
    ...
    std::u8string game_id = u8"waynegretzkys3dhoc_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            waynegretzkys3dhoc_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    waynegretzkys3dhoc_audio_init();
    waynegretzkys3dhoc_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = waynegretzkys3dhoc::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = waynegretzkys3dhoc_vi_callback,
        .gfx_init_callback = waynegretzkys3dhoc_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = waynegretzkys3dhoc_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = waynegretzkys3dhoc_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return waynegretzkys3dhoc_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return waynegretzkys3dhoc_app_main(argc, argv);
}
#endif
