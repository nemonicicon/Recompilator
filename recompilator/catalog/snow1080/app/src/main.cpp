...
static constexpr uint64_t SNOW1080_ROM_HASH        = 0x33C24BB288C25AA1ULL;
static constexpr uint32_t SNOW1080_ENTRYPOINT_VRAM = 0x80100400;
...
static void snow1080_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        snow1080_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        snow1080_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void snow1080_message_box(const char* msg) {
    snow1080_show_error("1080 Snowboarding — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI snow1080_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in snow1080pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in snow1080pc.exe (base=0x%p), nearest first:\n",
...
static void snow1080_vi_callback() {}
static void snow1080_gfx_init_callback() {}

static std::string snow1080_get_thread_name(const OSThread* t) {
    return "snow1080_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        if (!GetOpenFileNameA(&ofn)) {
            snow1080_show_error("ROM Required",
    ...
    // SNOW1080_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("SNOW1080_ROM");
    if (!rom_hint || !*rom_hint) {
        snow1080_show_error("ROM Required",
            "Pass the 1080 Snowboarding ROM path as the first argument,\n"
            "or set the SNOW1080_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"snow1080_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        snow1080_show_error("Wrong ROM",
            "The selected ROM does not match 1080 Snowboarding.\n"
            "Expected SHA1: 79cd1166c365e5809dec9b62e6d40d6032d5db3a");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        snow1080_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        snow1080_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int snow1080_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(snow1080_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "snow1080pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "snow1080pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "snow1080pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[snow1080pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "snow1080pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[snow1080pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = SNOW1080_ROM_HASH;
    entry.internal_name      = "1080 Snowboarding";
    entry.game_id            = u8"snow1080_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Sram;
    entry.is_enabled         = true;
    entry.entrypoint_address = SNOW1080_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"snow1080_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            snow1080_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = snow1080::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = snow1080_vi_callback,
        .gfx_init_callback = snow1080_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = snow1080_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = snow1080_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return snow1080_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return snow1080_app_main(argc, argv);
}
#endif
