/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for LODERUNNER3DPC.
 *
...
void loderunner3d_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in loderunner3d_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool loderunner3d_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void loderunner3d_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t loderunner3d_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  loderunner3d_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = loderunner3d_poll_input,
        .get_input               = loderunner3d_get_input,
        .set_rumble              = loderunner3d_set_rumble,
        .get_connected_device_info = loderunner3d_get_connected_device,
    };
}
