/**
 * main.cpp — 1080 Snowboarding PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── 1080 Snowboarding game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 79cd1166c365e5809dec9b62e6d40d6032d5db3a · internal name "1080 Snowboarding"
static constexpr uint64_t G1080SNOWBOARDING_ROM_HASH        = 0x33C24BB288C25AA1ULL;
static constexpr uint32_t G1080SNOWBOARDING_ENTRYPOINT_VRAM = 0x80000400;
...
static void g1080snowboarding_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        g1080snowboarding_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "1080 Snowboarding PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        g1080snowboarding_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "1080 Snowboarding PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void g1080snowboarding_message_box(const char* msg) {
    g1080snowboarding_show_error("1080 Snowboarding — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI g1080snowboarding_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in g1080snowboardingpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in g1080snowboardingpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "1080 Snowboarding — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void g1080snowboarding_vi_callback() {}
static void g1080snowboarding_gfx_init_callback() {}

static std::string g1080snowboarding_get_thread_name(const OSThread* t) {
    return "g1080snowboarding_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select 1080 Snowboarding ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            g1080snowboarding_show_error("ROM Required",
                "A 1080 Snowboarding ROM is required.\n"
    ...
    // G1080SNOWBOARDING_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("G1080SNOWBOARDING_ROM");
    if (!rom_hint || !*rom_hint) {
        g1080snowboarding_show_error("ROM Required",
            "Pass the 1080 Snowboarding ROM path as the first argument,\n"
            "or set the G1080SNOWBOARDING_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"g1080snowboarding_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        g1080snowboarding_show_error("Wrong ROM",
            "The selected ROM does not match 1080 Snowboarding.\n"
            "Expected SHA1: 79cd1166c365e5809dec9b62e6d40d6032d5db3a");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        g1080snowboarding_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        g1080snowboarding_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int g1080snowboarding_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(g1080snowboarding_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "g1080snowboardingpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "g1080snowboardingpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "g1080snowboardingpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[g1080snowboardingpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "g1080snowboardingpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[g1080snowboardingpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = G1080SNOWBOARDING_ROM_HASH;
    entry.internal_name      = "1080 Snowboarding";
    entry.game_id            = u8"g1080snowboarding_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Sram;
    entry.is_enabled         = true;
    entry.entrypoint_address = G1080SNOWBOARDING_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"g1080snowboarding_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            g1080snowboarding_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    g1080snowboarding_audio_init();
    g1080snowboarding_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = g1080snowboarding::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = g1080snowboarding_vi_callback,
        .gfx_init_callback = g1080snowboarding_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = g1080snowboarding_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = g1080snowboarding_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return g1080snowboarding_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return g1080snowboarding_app_main(argc, argv);
}
#endif
