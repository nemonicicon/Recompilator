/**
 * rsp.cpp — RSP microcode dispatch for ECWHCRPC (Ecwhcr 64, USA). Game #4.
 *
 * AUDIO: Ecwhcr ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void ecwhcr_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* ecwhcr_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[ecwhcrpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = ecwhcr_get_rsp_microcode,
        .capture_gfx = ecwhcr_run_gfx_lle,
    };
}
