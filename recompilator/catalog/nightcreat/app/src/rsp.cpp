/**
 * rsp.cpp — RSP microcode dispatch for NIGHTCREATPC (Nightcreat 64, USA). Game #4.
 *
 * AUDIO: Nightcreat ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nightcreat_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nightcreat_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nightcreatpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nightcreat_get_rsp_microcode,
        .capture_gfx = nightcreat_run_gfx_lle,
    };
}
