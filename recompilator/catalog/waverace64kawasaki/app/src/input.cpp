/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for WAVERACE64KAWASAKIPC.
 *
...
void waverace64kawasaki_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in waverace64kawasaki_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool waverace64kawasaki_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
    ...
    *y = 0.0f;
    ...
    { static int _n=0; if (_n++ < 8 || (_n % 120)==0)
        fprintf(stderr,"[inputbridge] get_input #%d src=%s btn=0x%04X x=%.2f y=%.2f\n",
                _n, controller?"pad":"KBD", (unsigned)*buttons, *x, *y); fflush(stderr); }
    return true;
}

void waverace64kawasaki_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t waverace64kawasaki_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  waverace64kawasaki_get_input() already supplies
...
void waverace64kawasaki_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = waverace64kawasaki_poll_input,
        .get_input               = waverace64kawasaki_get_input,
        .set_rumble              = waverace64kawasaki_set_rumble,
        .get_connected_device_info = waverace64kawasaki_get_connected_device,
    };
}
