...
namespace RT64 {
    ...
    struct RenderTarget {
        ...
        // CV64 cont.23 — Brick 1 of the accurate-RDP depth (Option D, faithful LLE graphics): a
        // programmable N64-depth buffer (R32_FLOAT UAV) that opaque draws will read-modify-write with the
        // REAL RDP coplanar/dz compare via a rasterizer-ordered view, instead of the GPU's fixed-function
        // LESS_EQUAL (which loses the castle's coplanar ties). Allocated on Depth targets; INERT until the
        // ROV pipeline + in-shader compare are wired (later increments).
        std::unique_ptr<RenderTexture> n64DepthTexture;
        std::unique_ptr<RenderTextureView> n64DepthTextureView;
