/**
 * rsp.cpp — RSP microcode dispatch for GOLDENEYEPC (Goldeneye 64, USA). Game #4.
 *
 * AUDIO: Goldeneye ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void goldeneye_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* goldeneye_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[goldeneyepc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = goldeneye_get_rsp_microcode,
        .capture_gfx = goldeneye_run_gfx_lle,
    };
}
