/**
 * rsp.cpp — RSP microcode dispatch for WAYNEGRETZKYS3DHOCPC (Waynegretzkys3dhoc 64, USA). Game #4.
 *
 * AUDIO: Waynegretzkys3dhoc ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void waynegretzkys3dhoc96_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* waynegretzkys3dhoc96_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[waynegretzkys3dhoc96pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = waynegretzkys3dhoc96_get_rsp_microcode,
        .capture_gfx = waynegretzkys3dhoc96_run_gfx_lle,
    };
}
