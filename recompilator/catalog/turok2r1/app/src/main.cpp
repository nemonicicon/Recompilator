/**
 * main.cpp — Turok 2 Seeds of Evil Rev1 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Turok 2 Seeds of Evil Rev1 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: a86af6c2ac9f6d837181f07c0a27f7ee59e7df68 · internal name "Turok 2: Seeds of Ev"
static constexpr uint64_t TUROK2R1_ROM_HASH        = 0x01F77F83CE0B3DD7ULL;
static constexpr uint32_t TUROK2R1_ENTRYPOINT_VRAM = 0x80000400;
...
static void turok2r1_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        turok2r1_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Turok 2 Seeds of Evil Rev1 PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        turok2r1_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Turok 2 Seeds of Evil Rev1 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void turok2r1_message_box(const char* msg) {
    turok2r1_show_error("Turok 2 Seeds of Evil Rev1 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI turok2r1_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in turok2r1pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in turok2r1pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Turok 2 Seeds of Evil Rev1 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void turok2r1_vi_callback() {}
static void turok2r1_gfx_init_callback() {}

static std::string turok2r1_get_thread_name(const OSThread* t) {
    return "turok2r1_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Turok 2 Seeds of Evil Rev1 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            turok2r1_show_error("ROM Required",
                "A Turok 2 Seeds of Evil Rev1 ROM is required.\n"
    ...
    // TUROK2R1_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("TUROK2R1_ROM");
    if (!rom_hint || !*rom_hint) {
        turok2r1_show_error("ROM Required",
            "Pass the Turok 2 Seeds of Evil Rev1 ROM path as the first argument,\n"
            "or set the TUROK2R1_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"turok2r1_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        turok2r1_show_error("Wrong ROM",
            "The selected ROM does not match Turok 2 Seeds of Evil Rev1.\n"
            "Expected SHA1: a86af6c2ac9f6d837181f07c0a27f7ee59e7df68");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        turok2r1_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        turok2r1_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int turok2r1_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(turok2r1_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "turok2r1pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "turok2r1pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "turok2r1pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[turok2r1pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "turok2r1pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[turok2r1pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = TUROK2R1_ROM_HASH;
    entry.internal_name      = "Turok 2: Seeds of Ev";
    entry.game_id            = u8"turok2r1_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Sram;
    entry.is_enabled         = true;
    entry.entrypoint_address = TUROK2R1_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"turok2r1_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            turok2r1_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = turok2r1::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = turok2r1_vi_callback,
        .gfx_init_callback = turok2r1_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = turok2r1_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = turok2r1_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return turok2r1_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return turok2r1_app_main(argc, argv);
}
#endif
