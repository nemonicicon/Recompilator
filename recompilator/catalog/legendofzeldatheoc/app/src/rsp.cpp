/**
 * rsp.cpp — RSP microcode dispatch for LEGENDOFZELDATHEOCPC (Legendofzeldatheoc 64, USA). Game #4.
 *
 * AUDIO: Legendofzeldatheoc ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void legendofzeldatheoc_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* legendofzeldatheoc_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[legendofzeldatheocpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = legendofzeldatheoc_get_rsp_microcode,
        .capture_gfx = legendofzeldatheoc_run_gfx_lle,
    };
}
