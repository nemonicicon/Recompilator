/**
 * main.cpp — All-Star Baseball 99 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── All-Star Baseball 99 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: aa576624f2a66034cb0d6e621ba16a0d3beaca3a · internal name "All-Star Baseball 99"
static constexpr uint64_t ALLSTARBASEBALL99_ROM_HASH        = 0xA50334D0FA94F4EDULL;
static constexpr uint32_t ALLSTARBASEBALL99_ENTRYPOINT_VRAM = 0x80300000;
...
static void allstarbaseball99_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        allstarbaseball99_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "All-Star Baseball 99 PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        allstarbaseball99_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "All-Star Baseball 99 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void allstarbaseball99_message_box(const char* msg) {
    allstarbaseball99_show_error("All-Star Baseball 99 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI allstarbaseball99_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in allstarbaseball99pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in allstarbaseball99pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "All-Star Baseball 99 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void allstarbaseball99_vi_callback() {}
static void allstarbaseball99_gfx_init_callback() {}

static std::string allstarbaseball99_get_thread_name(const OSThread* t) {
    return "allstarbaseball99_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select All-Star Baseball 99 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            allstarbaseball99_show_error("ROM Required",
                "A All-Star Baseball 99 ROM is required.\n"
    ...
    // ALLSTARBASEBALL99_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("ALLSTARBASEBALL99_ROM");
    if (!rom_hint || !*rom_hint) {
        allstarbaseball99_show_error("ROM Required",
            "Pass the All-Star Baseball 99 ROM path as the first argument,\n"
            "or set the ALLSTARBASEBALL99_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"allstarbaseball99_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        allstarbaseball99_show_error("Wrong ROM",
            "The selected ROM does not match All-Star Baseball 99.\n"
            "Expected SHA1: aa576624f2a66034cb0d6e621ba16a0d3beaca3a");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        allstarbaseball99_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        allstarbaseball99_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int allstarbaseball99_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(allstarbaseball99_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "allstarbaseball99pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "allstarbaseball99pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "allstarbaseball99pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[allstarbaseball99pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "allstarbaseball99pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[allstarbaseball99pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = ALLSTARBASEBALL99_ROM_HASH;
    entry.internal_name      = "All-Star Baseball 99";
    entry.game_id            = u8"allstarbaseball99_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = ALLSTARBASEBALL99_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"allstarbaseball99_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            allstarbaseball99_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    allstarbaseball99_audio_init();
    allstarbaseball99_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = allstarbaseball99::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = allstarbaseball99_vi_callback,
        .gfx_init_callback = allstarbaseball99_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = allstarbaseball99_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = allstarbaseball99_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return allstarbaseball99_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return allstarbaseball99_app_main(argc, argv);
}
#endif
