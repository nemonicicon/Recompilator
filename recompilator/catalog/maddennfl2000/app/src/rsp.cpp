/**
 * rsp.cpp — RSP microcode dispatch for MADDENNFL2000PC (Maddennfl2000 64, USA). Game #4.
 *
 * AUDIO: Maddennfl2000 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void maddennfl2000_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* maddennfl2000_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[maddennfl2000pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = maddennfl2000_get_rsp_microcode,
        .capture_gfx = maddennfl2000_run_gfx_lle,
    };
}
