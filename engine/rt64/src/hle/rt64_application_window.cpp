...
#   endif
    ...
    // (Wave-1 purge: removed the CV64 session-31 wild-store HW-watch call — it made a per-game
    // debug hunt a LINK-TIME DEPENDENCY of every rt64-linking app. The watch tool itself is worth
    // salvaging into an env-gated engine diagnostic; see the generalization worklist.)
...
#   endif
    ...
    LRESULT CALLBACK ApplicationWindow::windowHookCallback(int nCode, WPARAM wParam, LPARAM lParam) {
        if ((nCode >= 0) && (wParam == PM_REMOVE)) {
            ...
            // cont.33 quitwatch: a spurious WM_CLOSE/WM_QUIT during the idle demo gets converted to
            // PostQuitMessage(0) → SDL_QUIT → quit() → renderer teardown → use-after-free crash. Log the
            // close-class messages + which window they target (main game HWND vs a transient) to pin the source.
            if (msg.message == WM_CLOSE || msg.message == WM_QUIT || msg.message == WM_DESTROY ||
                msg.message == WM_NCDESTROY || msg.message == WM_SYSCOMMAND || msg.message == WM_ENDSESSION) {
                void* mainHwnd = HookedApplicationWindow ? (void*)HookedApplicationWindow->windowHandle : nullptr;
                fprintf(stderr, "[quitwatch] HOOK msg=0x%04X hwnd=%p wParam=0x%llX  (mainGameHwnd=%p)\n",
                        msg.message, (void*)msg.hwnd, (unsigned long long)msg.wParam, mainHwnd);
                fflush(stderr);
            }
    ...
    // (Wave-1 purge: removed the CV64 cont.30/32 SEH-guarded dispatch — it swallowed ACCESS
    // VIOLATIONS in EVERY game's SDL event path, masking real memory corruption library-wide.
    // Real hardware never had host-side AVs; corruption must surface as a tracked engine bug,
    // not be silently eaten. If CV64's Forest wild-store resurfaces, that is the point.)
