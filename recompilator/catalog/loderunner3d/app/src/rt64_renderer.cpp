/**
 * rt64_renderer.cpp — RT64Context implementation for LODERUNNER3DPC.
 *
 * Same wrapper as cv64pc's (the engine-proven shape), minus the CV64-specific
 * DL-dump probes. LODERUNNER3D uses the same F3DEX2 fifo 2.06 microcode as CV64.
 */
                         ...
                         bool developer_mode)
{
    ...
    // osViSetXScale/YScale half-res present factor (separate channel from VI_X_SCALE_REG). Optional in
    // RT64 (null => 1.0); binding it lets the present stretch a half-res render to full (Lode Runner 3D).
    core.VI_X_SCALE_FACTOR    = &vi->xScaleFactor;
    core.VI_Y_SCALE_FACTOR    = &vi->yScaleFactor;

    RT64::ApplicationConfiguration appConfig;
    appConfig.useConfigurationFile = true;
    appConfig.appId = "loderunner3dpc";
