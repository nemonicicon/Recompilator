/**
 * rsp.cpp — RSP microcode dispatch for FIFA98RDPC (Fifa98rd 64, USA). Game #4.
 *
 * AUDIO: Fifa98rd ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void fifa98rd_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* fifa98rd_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[fifa98rdpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = fifa98rd_get_rsp_microcode,
        .capture_gfx = fifa98rd_run_gfx_lle,
    };
}
