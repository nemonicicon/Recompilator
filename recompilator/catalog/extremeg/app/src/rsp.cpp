/**
 * rsp.cpp — RSP microcode dispatch for EXTREMEGPC (Extremeg 64, USA). Game #4.
 *
 * AUDIO: Extremeg ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void extremeg_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* extremeg_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[extremegpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = extremeg_get_rsp_microcode,
        .capture_gfx = extremeg_run_gfx_lle,
    };
}
