...
namespace RT64 {
    ...
    bool FramebufferPair::earlyPresentCandidate() const {
        ...
        // A pair whose every call is a fill-mode rectangle is a CLEAR pass, not a frame (SOTE
        // shimmer, 2026-07-19): games that split one frame across multiple RSP tasks writing the
        // same color image (SOTE: a fill task, then a 188-call text task) otherwise get the bare
        // clear presented at the workload boundary — a full-black frame interleaving with the
        // finished frame at present rate, i.e. a constant dark strobe. A clear is never the
        // intended presented image; the frame-final workload's pair (real draws, fillRectOnly
        // false) remains an early-present candidate, so single-workload frames are unaffected.
        if (fillRectOnly && (gameCallCount > 0)) {
            return false;
        }
