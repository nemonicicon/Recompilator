...
namespace RT64 {
    ...
    void State::flush() {
        ...
        // cv64 SESSION 40 [fire4]: condition-keyed resolved-state dump for the FLAME's combiner
        // (FC151660 2515257F => H&0xFFFFFF==0x151660, L==0x2515257F — the 2-cycle dual-tile
        // crossfade: RGB/alpha = (TEXEL0-TEXEL1)*PRIM_a + TEXEL1, tile0@TMEM0 tile1@TMEM0x100,
        // IA16, no TLUT, level=0). The S39 chain proved the DL is WALKED; this dump names the
        // layer that kills the draw at submission: tile validity/fields, othermode (blender!),
        // geometry mode (G_FOG is set — cycle-2 alpha references SHADE=fog), prim/env/fog, tris.
        // Logged once per UNIQUE state signature — NO count caps (the S39 cap lesson); the
        // combiner only exists while fire burns, so this cannot flood.
        // If [fire] pushes appear but [fire4] NEVER does, the kill is PRE-flush (vertex/cull).
        {
            const interop::ColorCombiner &cc = drawCall.colorCombiner;
            // RT64 packs the combiner as L = w0 (incl. the FC byte), H = w1 — the GBI handler
            // builds (w1 << 32) | w0 (rt64_gbi_rdp.cpp:39). The first probe run keyed this
            // BACKWARDS (L==w1) and could never fire — that zero-[fire4] run is VOID.
            if ((cc.L & 0xFFFFFFu) == 0x151660u && cc.H == 0x2515257Fu) {
                // cv64 SESSION 40 A/B HISTORY (mutations REMOVED, S40 end — probes below remain):
                // S40d cleared Z_CMP → no fire (depth-tie branch DEAD). S40h swapped the cycle-1
                // FOG_SHADE_A blend for pass-through (0x0C18) → no fire (fog-saturation branch DEAD,
                // despite the collapsed-HLE-Z fog input being real and worth fixing on its own).
                const DrawData &dd = workload.drawData;
                uint64_t sig = 0xcbf29ce484222325ull;
                auto mix = [&sig](uint64_t v) { sig ^= v; sig *= 0x100000001b3ull; };
                mix(drawCall.otherMode.L); mix(drawCall.otherMode.H);
                mix(drawCall.geometryMode); mix(drawCall.tileCount); mix(drawCall.triangleCount);
                mix(drawCall.textureOn); mix(drawCall.textureTile); mix(drawCall.textureLevels);
                for (uint32_t t = 0; t < drawCall.tileCount && (drawCall.tileIndex + t) < dd.rdpTiles.size(); t++) {
                    const auto &rt = dd.rdpTiles[drawCall.tileIndex + t];
                    mix((uint32_t)rt.fmt); mix((uint32_t)rt.siz); mix((uint32_t)rt.stride); mix((uint32_t)rt.address);
                    mix(dd.callTiles[drawCall.tileIndex + t].valid ? 1u : 0u);
                }
                static uint64_t _f4seen[64]; static int _f4n = 0;
                bool isNew = true;
                for (int i = 0; i < _f4n; i++) { if (_f4seen[i] == sig) { isNew = false; break; } }
                if (isNew && _f4n < 64) {
                    _f4seen[_f4n++] = sig;
                    fprintf(stderr, "[fire4] flush: tris=%u omL=%08X omH=%08X geo=%08X texOn=%u texTile=%u lvls=%u tileCount=%u tileIndex=%u prim=(%.2f,%.2f,%.2f,a=%.2f) env=(%.2f,%.2f,%.2f,a=%.2f) fog=(%.2f,%.2f,%.2f,a=%.2f)\n",
                        drawCall.triangleCount, (uint32_t)drawCall.otherMode.L, (uint32_t)drawCall.otherMode.H,
                        drawCall.geometryMode, (unsigned)drawCall.textureOn, (unsigned)drawCall.textureTile,
                        (unsigned)drawCall.textureLevels, drawCall.tileCount, drawCall.tileIndex,
                        drawCall.rdpParams.primColor.x, drawCall.rdpParams.primColor.y, drawCall.rdpParams.primColor.z, drawCall.rdpParams.primColor.w,
                        drawCall.rdpParams.envColor.x, drawCall.rdpParams.envColor.y, drawCall.rdpParams.envColor.z, drawCall.rdpParams.envColor.w,
                        drawCall.rdpParams.fogColor.x, drawCall.rdpParams.fogColor.y, drawCall.rdpParams.fogColor.z, drawCall.rdpParams.fogColor.w);
                    for (uint32_t t = 0; t < drawCall.tileCount && (drawCall.tileIndex + t) < dd.rdpTiles.size(); t++) {
                        const auto &rt = dd.rdpTiles[drawCall.tileIndex + t];
                        fprintf(stderr, "[fire4]   tile%u: valid=%d fmt=%d siz=%d stride=%d addr=0x%X pal=%d masks=%d maskt=%d cms=%d cmt=%d uls=%.1f ult=%.1f lrs=%.1f lrt=%.1f sampler=%u\n",
                            t, dd.callTiles[drawCall.tileIndex + t].valid ? 1 : 0, rt.fmt, rt.siz, rt.stride, (unsigned)rt.address,
                            rt.palette, rt.masks, rt.maskt, rt.cms, rt.cmt, rt.uls, rt.ult, rt.lrs, rt.lrt, (unsigned)rt.nativeSampler);
                    }
                    fflush(stderr);
                }
            }
    ...
    void State::submitFramebufferPair(FramebufferPair::FlushReason flushReason) {
        ...
        if (!fbPair.drawColorRect.isEmpty()) {
            ...
            // SESSION 28 cont.18f: START-MENU DEPTH PROBE. The gray occluding layer appears only after
            // opening the start menu, fade is ruled out. Hypothesis: the menu stops the per-frame Z-buffer
            // clear (a fill-rect into the depth image, siz=16b) → stale depth accumulates → occludes. Track
            // the cumulative Z-clear count sampled ~per frame (color buffer rotates each frame). If zclears
            // FLATLINES right when the start menu opens (and depthFb/address change), that's it; if it keeps
            // climbing normally, the gray layer is a leaked menu OBJECT instead.
            {
                static uint32_t _zc = 0, _frames = 0, _lastColor = 0xFFFFFFFFu; static int _zn = 0;
                bool _isZClear = fbPair.fillRectOnly && (colorImg.address == depthImg.address) && (colorImg.siz == G_IM_SIZ_16b);
                if (_isZClear) _zc++;
                if (colorImg.address != _lastColor) {   // ~once per frame
                    _lastColor = colorImg.address; _frames++;
                    if ((_frames % 30u) == 0u && _zn++ < 1500) {
                        fprintf(stderr, "[zclear] frame~%u zclears=%u depth=0x%08X depthFb=%s\n",
                                _frames, _zc, depthImg.address, (depthFb ? "yes" : "NULL"));
                        fflush(stderr);
                    }
                }
            }
    ...
    void State::fullSyncFramebufferPairTiles(Workload &workload, FramebufferPair &fbPair, uint32_t &loadOpCursor, uint32_t &rdpTileCursor) {
        ...
        auto uploadTile = [&](uint32_t rdpTileIndex, FramebufferPair &fbPair, const DrawCall &drawCall) {
            ...
            // cv64 SESSION 40g [fire8]: the per-draw TMEM replay is correct-by-design (cursored loads
            // before each call's tiles) — so dump GROUND TRUTH at the flame tile's upload moment:
            // is the fire texel data IN TMEM right now (loadBlock replay worked), and which path/hash
            // does the tile take? Expected TMEM content at tmem<<3 + 0x114: nonzero (the flame edge
            // bytes); zeros = the replay/attribution put the wrong loads before this draw.
            const bool _fireDraw = ((drawCall.colorCombiner.L & 0xFFFFFFu) == 0x151660u && drawCall.colorCombiner.H == 0x2515257Fu);
            if (_fireDraw) {
                static int _f8 = 0;
                if (_f8++ < 24) {
                    const uint8_t *T8 = reinterpret_cast<const uint8_t *>(rdp->TMEM);
                    const uint32_t tb = uint32_t(callTile.loadTile.tmem) << 3;
                    fprintf(stderr, "[fire8] tile[%u] valid=%d copyUsed=%d reinterp=%d rawTMEM=%d tlut=%u tmem=0x%X preHash=0x%llX | TMEM+0x114:",
                            rdpTileIndex, callTile.valid ? 1 : 0, callTile.tileCopyUsed ? 1 : 0, callTile.reinterpretTile ? 1 : 0,
                            callTile.rawTMEM ? 1 : 0, (unsigned)callTile.tlut, tb, (unsigned long long)callTile.tmemHashOrID);
                    for (int k = 0; k < 16; k++) fprintf(stderr, "%s%02X", (k % 2) ? "" : " ", T8[(tb + 0x114 + k) & (RDP_TMEM_BYTES - 1)]);
                    fprintf(stderr, "\n"); fflush(stderr);
                }
            }
            if (!callTile.valid) {
            ...
            // cv64 SESSION 40g [fire8]: the resolved upload result for flame tiles.
            if (_fireDraw) {
                static int _f8b = 0;
                if (_f8b++ < 24) {
                    fprintf(stderr, "[fire8]   -> postHash=0x%llX\n", (unsigned long long)callTile.tmemHashOrID);
                    fflush(stderr);
                }
            }
        };
        ...
        if (renderToRDRAM) {
            ...
            auto renderAndSynchronize = [&](uint32_t maxFramebufferPair) {
                ...
                while (pairCursor < maxFramebufferPair) {
                    ...
                    // [fbpair] SOTE shimmer dig: every rendered pair, so [fbchange] silence is
                    // attributable (pair without ingest vs no pair at all).
                    {
                        static const bool fbt_on = (getenv("RECOMP_PRESENT_TRACE") != nullptr);
                        if (fbt_on) {
                            fprintf(stderr, "[fbpair] pair=%u color=0x%08X projs=%u calls=%u\n",
                                (unsigned)pairCursor, fbPair.colorImage.address, (unsigned)fbPair.projectionCount,
                    ...
                    if (getTargetsFromPair(pairCursor)) {
                        ...
                        if (colorFb->height > colorFb->readHeight) {
                            ...
                            // [fbchange] SOTE shimmer dig (2026-07-19, RECOMP_PRESENT_TRACE=1):
                            // one line per color-image RAM ingest — names whether the workload
                            // composited CPU-written fb bytes (the suspected text blit) and how
                            // much was (re)read. Correlated against [presenttrace] ct lines in
                            // the same stderr stream to reconstruct the black-frame cadence.
                            {
                                static const bool fbt_on = (getenv("RECOMP_PRESENT_TRACE") != nullptr);
                                if (fbt_on) {
                                    fprintf(stderr, "[fbchange] pair=%u addr=0x%08X rows=%u..%u change=%d\n",
                                        (unsigned)pairCursor, colorImg.address, colorFb->readHeight, colorFb->readHeight + readRowCount,
                                        (int)(colorFbChange != nullptr));
                                    fflush(stderr);
                                }
                            }
                ...
                while (pairCursor < maxFramebufferPair) {
                    if (getFramebufferPairs(pairCursor)) {
                        // SESSION 28 cont.16+ (Reinhardt): RT64 must NEVER copy a framebuffer onto low/OS
                        // RDRAM. The weretiger/fire scene tracks a BOGUS color framebuffer whose addressStart
                        // lands on the engine scheduler struct (~0x800AC000); copyNativeToRAM then memcpys the
                        // rendered image over it → zeroes the gfx-queue msgCount → graphics deadlock / div-by-0
                        // crash (pinned via a Dr0 hardware watchpoint: PC = Framebuffer::copyNativeToRAM,
                        // dst=&RDRAM[addressStart]). No valid CV64 framebuffer lives below ~0x200000, so skip the
                        // copy-back for any framebuffer in low memory, and log it to trace the bad address SOURCE.
                        // SM64PC S45 boot#7 CORRECTION: the 0x100000 floor was CV64-shaped, not a hardware
                        // invariant — SM64's z-buffer legitimately lives at PHYSICAL 0x400 and its copy-back
                        // must run. With setColorImage now rejecting only encoding-impossible widths, a bogus
                        // framebuffer can't be tracked in the first place; keep only a floor over the true OS
                        // reserve (exception vectors + boot globals, < 0x400) no game can legitimately target.
                        if (colorFb->addressStart >= 0x00000400u) {
                            colorFb->copyNativeToRAM(&RDRAM[colorFb->addressStart], colorWriteWidth, colorRowStart, std::min(colorRowEnd, colorFb->height));
                        } else {
                            static int _fbg = 0;
                            if (_fbg++ < 32) {
                                fprintf(stderr, "[fbguard] SKIP color copyNativeToRAM: bogus addressStart=0x%08X w=%u h=%u (OS-reserve RDRAM)\n",
                                        (unsigned)colorFb->addressStart, (unsigned)colorWriteWidth, (unsigned)colorFb->height);
                                fflush(stderr);
                            }
                        }

                        if (depthWriteWidth > 0 && depthFb->addressStart >= 0x00000400u) {
        ...
        for (uint32_t f = 0; f < workload.fbPairCount; f++) {
            ...
            for (uint32_t p = 0; p < fbPair.projectionCount; p++) {
                ...
                for (uint32_t d = 0; d < proj.gameCallCount; d++) {
                    ...
                    // (Wave-1 purge: removed the CV64 [pcensus] Malus depth-write census — per-
                    // perspective-draw material hashing + 96-entry scan paid by every game for a
...
#                   endif
        ...
        // [paircensus 2026-08-28] UNCAPPED, env RECOMP_PAIR_CENSUS=1. The dump below is SAMPLED
        // (first 8 fullSyncs then every 100th), and reading "every projection is a Rectangle" off
        // that sample was wrong -- the uncapped triangle census says 1,360,094 PERSPECTIVE
        // triangles flow per run. So count, without a cap, what actually happens to those pairs:
        // how many pairs carry a Perspective projection, which colour image they render into, and
        // whether that colour image is one the VI is presenting. The report is that the 2D is
        // PERSISTENT (the pause/save overlay never clears while the radar updates), which is what
        // it looks like when the frame the player sees is not the frame the 3D was drawn into.
        {
            static const bool on = [] { const char *e = std::getenv("RECOMP_PAIR_CENSUS"); return (e != nullptr) && (e[0] == '1'); }();
            if (on) {
                static uint64_t nSync = 0, nPairs = 0, nPersp = 0, nPerspPresented = 0, nRectOnly = 0;
                static uint32_t ciSeen[32]; static uint64_t ciPersp[32]; static uint64_t ciAll[32]; static int ciN = 0;
                static auto t0 = std::chrono::steady_clock::now();
                nSync++;
                for (uint32_t fp = 0; fp < workload.fbPairCount; fp++) {
                    const FramebufferPair &pr = workload.fbPairs[fp];
                    nPairs++;
                    bool hasPersp = false;
                    for (uint32_t pj = 0; pj < pr.projectionCount && pj < pr.projections.size(); pj++) {
                        if (pr.projections[pj].type == Projection::Type::Perspective) { hasPersp = true; break; }
                    }
                    if (hasPersp) nPersp++; else nRectOnly++;
                    const uint32_t addr = pr.colorImage.address;
                    int k = -1;
                    for (int j = 0; j < ciN; j++) if (ciSeen[j] == addr) { k = j; break; }
                    if (k < 0 && ciN < 32) { k = ciN++; ciSeen[k] = addr; ciPersp[k] = 0; ciAll[k] = 0; }
                    if (k >= 0) { ciAll[k]++; if (hasPersp) ciPersp[k]++; }
                    if (hasPersp) {
                        for (size_t h = 0; h < viHistory.history.size(); h++) {
                            if (viHistory.history[h].vi.fbAddress() == addr) { nPerspPresented++; break; }
                        }
                    }
                }
                ...
                if (std::chrono::duration_cast<std::chrono::milliseconds>(now - t0).count() >= 1000) {
                    t0 = now;
                    fprintf(stderr, "[paircensus] syncs=%llu pairs=%llu perspPairs=%llu perspInVIbuffer=%llu rectOnlyPairs=%llu | colorImages(persp/all):",
                            (unsigned long long)nSync, (unsigned long long)nPairs, (unsigned long long)nPersp,
                            (unsigned long long)nPerspPresented, (unsigned long long)nRectOnly);
                    for (int j = 0; j < ciN; j++) fprintf(stderr, " 0x%08X=%llu/%llu", ciSeen[j], (unsigned long long)ciPersp[j], (unsigned long long)ciAll[j]);
        ...
        {
            static uint32_t fullsync_count = 0;
            uint32_t fsn = fullsync_count++;
            if (fsn < 8 || fsn % 100 == 0) {
                fprintf(stderr, "[rt64_state] fullSync #%u: fbPairCount=%u presentEarly=%d viHistorySize=%zu\n",
                    fsn, workload.fbPairCount, (int)presentEarly, viHistory.history.size());
                for (uint32_t fp = 0; fp < workload.fbPairCount; fp++) {
                    const auto &ci = workload.fbPairs[fp].colorImage;
                    const bool dcEmpty = workload.fbPairs[fp].drawColorRect.isEmpty();
                    const bool epc = workload.fbPairs[fp].earlyPresentCandidate();
                    const uint32_t gcc = workload.fbPairs[fp].gameCallCount;
                    const uint32_t pjc = workload.fbPairs[fp].projectionCount;
                    fprintf(stderr, "[rt64_state]   pair[%u]: colorImage=0x%08X w=%u siz=%u drawColorRectEmpty=%d earlyPresentCandidate=%d gameCallCount=%u projectionCount=%u\n",
                        fp, ci.address, ci.width, (unsigned)ci.siz, (int)dcEmpty, (int)epc, gcc, pjc);
                    // Per-projection detail
                    static const char* projTypeNames[] = { "None", "Perspective", "Orthographic", "Rectangle", "Triangle" };
                    for (uint32_t pj = 0; pj < workload.fbPairs[fp].projectionCount && pj < workload.fbPairs[fp].projections.size(); pj++) {
                        const auto &proj = workload.fbPairs[fp].projections[pj];
                        uint32_t totalTris = 0;
                        uint32_t cycleType = 0xFFFFFFFF;
                        for (uint32_t gc = 0; gc < proj.gameCallCount && gc < proj.gameCalls.size(); gc++) {
                            totalTris += proj.gameCalls[gc].callDesc.triangleCount;
                            if (gc == 0) cycleType = proj.gameCalls[gc].callDesc.otherMode.cycleType();
                        }
                        int typeIdx = (int)proj.type;
                        if (typeIdx < 0 || typeIdx > 4) typeIdx = 0;
                        fprintf(stderr, "[rt64_state]     proj[%u]: type=%s gcCount=%u totalTris=%u cycleType=0x%X scis=[%d,%d,%d,%d](/4=%d,%d,%d,%d)\n",
                            pj, projTypeNames[typeIdx], proj.gameCallCount, totalTris, cycleType,
                ...
                for (size_t h = 0; h < viHistory.history.size(); h++) {
                    const uint32_t fbAddr = viHistory.history[h].vi.fbAddress();
                    fprintf(stderr, "[rt64_state]   viHistory[%zu].fbAddress=0x%08X\n", h, fbAddr);
                }
                fflush(stderr);
            }
        }
        if (presentEarly && (workload.fbPairCount > 0)) {
            ...
            for (int32_t f = workload.fbPairCount - 1; (f >= 0) && !viPresented; f--) {
                ...
                for (size_t h = 0; (h < viHistory.history.size()) && !viPresented; h++) {
                    ...
                    // ── [vi-origin-contain 2026-09-05, Rage Wars #152 + the Acclaim-kernel class] ──────────
                    // In PresentEarly mode this loop is the ONLY place a present is pushed (updateScreen's
                    // VI-driven path returns before pushing), and it accepted a colour image only when its
                    // address equalled fbAddress() = VI_ORIGIN minus one row - libultra's per-field origin
                    // offset (osViData.c fldRegs origin=640; __osViSwapContext.c:19), not a hardware rule.
                    // The hardware displays whichever buffer CONTAINS VI_ORIGIN. A kernel whose VI driver
                    // programs ORIGIN = buffer base (Acclaim: Rage Wars, Turok 2, South Park, Armorines) never
                    // matched, so RT64 pushed no present after the boot logos and the window kept its last
                    // image (measured: 5,162 visible 284x224 VI updates, 8 presents in 60 s, screen black).
                    // Accept a colour image that contains the origin within its first three rows (offset 0 =
                    // Acclaim's even field and its non-interlaced modes, 1 = libultra, 2 = libultra's odd
                    // interlaced field). libultra titles still match on the first term, unchanged.
                    bool addressMatch = (colorImg.address == fbAddress);
                    if (!addressMatch && (fbSiz >= G_IM_SIZ_16b) && (fbSize.x > 0)) {
                        const uint32_t rowBytes = fbSize.x << (fbSiz - 1);
                        const uint32_t viOrigin = entry.vi.origin;
                        addressMatch = (viOrigin >= colorImg.address) && ((viOrigin - colorImg.address) < 3u * rowBytes);
                        if (addressMatch) {
                            static uint32_t vi_origin_contain_early = 0;
                            const uint32_t oc = vi_origin_contain_early++;
                            if (oc < 8 || (oc % 1000) == 0) {
                                fprintf(stderr, "[vi-origin-contain] early-present #%u: colorImage=0x%08X w=%u contains VI_ORIGIN=0x%08X (row-offset base 0x%08X matched nothing)\n",
                                    oc, colorImg.address, colorImg.width, viOrigin, fbAddress);
                                fflush(stderr);
                            }
                        }
                    }
                    if (addressMatch &&
                        widthMatchesRelaxed(colorImg.width, fbSize.x) &&
                        (colorImg.siz == fbSiz))
                    {
            ...
            if (screenFb != nullptr) {
                ...
                if ((screenFbSize.x == screenFb->width) && (screenFbSiz == screenFb->siz)) {
                    ...
                    if (currentHash != screenFb->RAMHash) {
                        ...
                        const bool screenWroteChanges = (present.fbOperations.front().type == FramebufferOperation::Type::WriteChanges);
                        // [screenup] SOTE shimmer dig (RECOMP_PRESENT_TRACE=1): every CPU-change
                        // upload decision on the DISPLAYED fb — the counterpart of [fbpair]/
                        // [fbchange]; write-vs-discard alternation here is the suspected shimmer.
                        {
                            static const bool sut_on = (getenv("RECOMP_PRESENT_TRACE") != nullptr);
                            if (sut_on) {
                                fprintf(stderr, "[screenup] fb=0x%08X hashChanged op=%s\n",
                                    screenFb->addressStart, screenWroteChanges ? "WRITE" : "DISCARD");
                                fflush(stderr);
                            }
                        }
                        if (screenWroteChanges) {
