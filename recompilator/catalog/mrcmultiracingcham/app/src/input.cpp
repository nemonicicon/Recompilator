/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for MRCMULTIRACINGCHAMPC.
 *
...
void mrcmultiracingcham_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in mrcmultiracingcham_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool mrcmultiracingcham_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void mrcmultiracingcham_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t mrcmultiracingcham_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  mrcmultiracingcham_get_input() already supplies
...
void mrcmultiracingcham_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = mrcmultiracingcham_poll_input,
        .get_input               = mrcmultiracingcham_get_input,
        .set_rumble              = mrcmultiracingcham_set_rumble,
        .get_connected_device_info = mrcmultiracingcham_get_connected_device,
    };
}
