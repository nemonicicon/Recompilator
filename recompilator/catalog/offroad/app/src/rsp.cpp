/**
 * rsp.cpp — RSP microcode dispatch for OFFROADPC (Offroad 64, USA). Game #4.
 *
 * AUDIO: Offroad ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void offroad_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* offroad_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[offroadpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = offroad_get_rsp_microcode,
        .capture_gfx = offroad_run_gfx_lle,
    };
}
