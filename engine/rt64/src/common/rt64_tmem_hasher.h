...
namespace RT64 {
    struct TMEMHasher {
        ...
        // single-texture mode (set by Application when the device lacks descriptorIndexing):
        // per-draw binding uses pre-decoded RGBA textures only, so the raw-TMEM sampling path is
        // disabled — every tile takes the decode path and gets a bindable texture.
        static inline bool singleTextureMode = false;
