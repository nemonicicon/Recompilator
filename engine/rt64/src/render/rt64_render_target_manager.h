...
namespace RT64 {
    ...
    struct RenderTargetManager {
        ...
        // Guards structural mutation of targetMap (operator[] insert/rehash in get(), clear() in
        // destroyAll()). The present thread accesses targetMap on the interpolation-enabled path
        // WITHOUT workloadMutex (rt64_present_queue.cpp:199-208), racing the workload thread's
        // get()/destroyAll() during scene transitions; concurrent unordered_map mutation corrupts
        // the allocator (surfaces as glibc "double free or corruption" on ARM; tolerated by MSVCRT).
        // Leaf lock: get()/destroyAll() acquire nothing else while holding it (RenderTarget's ctor is
        // trivial), so it introduces no lock-ordering cycle and cannot deadlock.
        std::mutex targetMapMutex;
