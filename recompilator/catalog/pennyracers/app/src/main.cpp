/**
 * main.cpp — Penny Racers PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Penny Racers game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 1d4fce8ad6b1f0072d89aeb4c3187bc853b750a0 · internal name "Penny Racers"
static constexpr uint64_t PENNYRACERS_ROM_HASH        = 0xE5872D6F2E93D3FBULL;
static constexpr uint32_t PENNYRACERS_ENTRYPOINT_VRAM = 0x80025C00;
...
static void pennyracers_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        pennyracers_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        pennyracers_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Penny Racers PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void pennyracers_message_box(const char* msg) {
    pennyracers_show_error("Penny Racers — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI pennyracers_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in pennyracerspc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in pennyracerspc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Penny Racers — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void pennyracers_vi_callback() {}
static void pennyracers_gfx_init_callback() {}

static std::string pennyracers_get_thread_name(const OSThread* t) {
    return "pennyracers_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Penny Racers ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            pennyracers_show_error("ROM Required",
                "A Penny Racers ROM is required.\n"
    ...
    // PENNYRACERS_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("PENNYRACERS_ROM");
    if (!rom_hint || !*rom_hint) {
        pennyracers_show_error("ROM Required",
            "Pass the Penny Racers ROM path as the first argument,\n"
            "or set the PENNYRACERS_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"pennyracers_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        pennyracers_show_error("Wrong ROM",
            "The selected ROM does not match Penny Racers.\n"
            "Expected SHA1: 1d4fce8ad6b1f0072d89aeb4c3187bc853b750a0");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        pennyracers_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        pennyracers_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int pennyracers_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(pennyracers_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "pennyracerspc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "pennyracerspc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "pennyracerspc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[pennyracerspc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "pennyracerspc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[pennyracerspc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = PENNYRACERS_ROM_HASH;
    entry.internal_name      = "Penny Racers";
    entry.game_id            = u8"pennyracers_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = PENNYRACERS_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"pennyracers_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            pennyracers_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    pennyracers_audio_init();
    pennyracers_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = pennyracers::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = pennyracers_vi_callback,
        .gfx_init_callback = pennyracers_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = pennyracers_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = pennyracers_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return pennyracers_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return pennyracers_app_main(argc, argv);
}
#endif
