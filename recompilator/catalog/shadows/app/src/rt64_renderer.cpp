/**
 * rt64_renderer.cpp — RT64Context implementation for SHADOWSPC.
 *
 * Same wrapper as cv64pc's (the engine-proven shape), minus the CV64-specific
 * DL-dump probes. SHADOWS uses the same F3DEX2 fifo 2.06 microcode as CV64.
 */
...
#include <cstdio>
#include <cstdlib>   // getenv (RECOMP_PRESENT_EARLY A/B gate)
#include <atomic>
...
void RT64Context::enable_instant_present() {
    // SOTE shimmer A/B (2026-07-19): PresentEarly makes updateScreen skip its CPU-change
    // detection on VI flips entirely (rt64_state.cpp ~2056 early return), so a game that
    // CPU-blits into its framebuffer after submitting the DL (SOTE's text screens) races
    // the early-present hash instead of syncing at flip time — the suspected black-frame
    // shimmer. RECOMP_PRESENT_EARLY=0 keeps RT64's default presentation (SkipBuffering)
    // to test that mechanism.
    static const bool pe_on = [] {
        const char* e = std::getenv("RECOMP_PRESENT_EARLY");
        return !(e && e[0] == '0');
    }();
    if (app_ && pe_on) {
