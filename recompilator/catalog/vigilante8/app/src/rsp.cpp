/**
 * rsp.cpp — RSP microcode dispatch for VIGILANTE8PC (Vigilante8 64, USA). Game #4.
 *
 * AUDIO: Vigilante8 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void vigilante8_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* vigilante8_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[vigilante8pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = vigilante8_get_rsp_microcode,
        .capture_gfx = vigilante8_run_gfx_lle,
    };
}
