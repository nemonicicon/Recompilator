/**
 * main.cpp — Wave Race 64 - Kawasaki Jet Ski PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Wave Race 64 - Kawasaki Jet Ski game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 887ab588c2ecc64c52fb2065f06b0a1ee4af13dc · internal name "Wave Race 64 - Kawasaki Jet Ski"
static constexpr uint64_t WAVERACE64KAWASAKI_ROM_HASH        = 0x9655FFD366D955B9ULL;
static constexpr uint32_t WAVERACE64KAWASAKI_ENTRYPOINT_VRAM = 0x80046800;
...
static void waverace64kawasaki_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        waverace64kawasaki_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Wave Race 64 - Kawasaki Jet Ski PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        waverace64kawasaki_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Wave Race 64 - Kawasaki Jet Ski PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void waverace64kawasaki_message_box(const char* msg) {
    waverace64kawasaki_show_error("Wave Race 64 - Kawasaki Jet Ski — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI waverace64kawasaki_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in waverace64kawasakipc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in waverace64kawasakipc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Wave Race 64 - Kawasaki Jet Ski — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void waverace64kawasaki_vi_callback() {}
static void waverace64kawasaki_gfx_init_callback() {}

static std::string waverace64kawasaki_get_thread_name(const OSThread* t) {
    return "waverace64kawasaki_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Wave Race 64 - Kawasaki Jet Ski ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            waverace64kawasaki_show_error("ROM Required",
                "A Wave Race 64 - Kawasaki Jet Ski ROM is required.\n"
    ...
    // WAVERACE64KAWASAKI_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("WAVERACE64KAWASAKI_ROM");
    if (!rom_hint || !*rom_hint) {
        waverace64kawasaki_show_error("ROM Required",
            "Pass the Wave Race 64 - Kawasaki Jet Ski ROM path as the first argument,\n"
            "or set the WAVERACE64KAWASAKI_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"waverace64kawasaki_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        waverace64kawasaki_show_error("Wrong ROM",
            "The selected ROM does not match Wave Race 64 - Kawasaki Jet Ski.\n"
            "Expected SHA1: 887ab588c2ecc64c52fb2065f06b0a1ee4af13dc");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        waverace64kawasaki_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        waverace64kawasaki_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int waverace64kawasaki_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(waverace64kawasaki_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "waverace64kawasakipc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "waverace64kawasakipc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "waverace64kawasakipc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[waverace64kawasakipc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "waverace64kawasakipc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[waverace64kawasakipc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = WAVERACE64KAWASAKI_ROM_HASH;
    entry.internal_name      = "Wave Race 64 - Kawasaki Jet Ski";
    entry.game_id            = u8"waverace64kawasaki_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = WAVERACE64KAWASAKI_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"waverace64kawasaki_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            waverace64kawasaki_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    waverace64kawasaki_audio_init();
    waverace64kawasaki_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = waverace64kawasaki::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = waverace64kawasaki_vi_callback,
        .gfx_init_callback = waverace64kawasaki_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = waverace64kawasaki_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = waverace64kawasaki_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return waverace64kawasaki_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return waverace64kawasaki_app_main(argc, argv);
}
#endif
