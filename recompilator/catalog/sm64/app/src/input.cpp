/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for SM64PC.
 *
...
void sm64_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in sm64_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool sm64_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void sm64_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t sm64_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  sm64_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = sm64_poll_input,
        .get_input               = sm64_get_input,
        .set_rumble              = sm64_set_rumble,
        .get_connected_device_info = sm64_get_connected_device,
    };
}
