/**
 * rsp.cpp — RSP microcode dispatch for ROADRASH64PC (Roadrash64 64, USA). Game #4.
 *
 * AUDIO: Roadrash64 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void roadrash64_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* roadrash64_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[roadrash64pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = roadrash64_get_rsp_microcode,
        .capture_gfx = roadrash64_run_gfx_lle,
    };
}
