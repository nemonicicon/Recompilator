/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for SOUTHPARKCHEFSLUVSPC.
 *
...
void southparkchefsluvs_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in southparkchefsluvs_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool southparkchefsluvs_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void southparkchefsluvs_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t southparkchefsluvs_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  southparkchefsluvs_get_input() already supplies
...
void southparkchefsluvs_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = southparkchefsluvs_poll_input,
        .get_input               = southparkchefsluvs_get_input,
        .set_rumble              = southparkchefsluvs_set_rumble,
        .get_connected_device_info = southparkchefsluvs_get_connected_device,
    };
}
