 ...
 *     engine. Keyed to CV64 boot conditions; must never fire for ASTEROIDS.
 ...
 *     correct for static-only ASTEROIDS (overlay recompilation is PHASE 3).
 ...
 * The RT64-free console carries no estimator -- 0.0 = "no estimated cost". */
#ifdef RDPC_CONSOLE_ONLY
double g_rdp_estimated_cost_cycles = 0.0;
#endif

/* 1 — CV64 NI boot-stub hook; loudly visible if it ever fires for ASTEROIDS. */
RECOMP_FUNC void gamestate_change(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    fprintf(stderr, "[engine_shim] gamestate_change(%d) called — CV64 NI boot-stub path fired for ASTEROIDS?!\n",
            (int)ctx->r4);
    fflush(stderr);
}
