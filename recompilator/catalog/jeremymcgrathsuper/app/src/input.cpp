/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for JEREMYMCGRATHSUPERPC.
 *
...
void jeremymcgrathsuper_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in jeremymcgrathsuper_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool jeremymcgrathsuper_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void jeremymcgrathsuper_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t jeremymcgrathsuper_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  jeremymcgrathsuper_get_input() already supplies
...
void jeremymcgrathsuper_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = jeremymcgrathsuper_poll_input,
        .get_input               = jeremymcgrathsuper_get_input,
        .set_rumble              = jeremymcgrathsuper_set_rumble,
        .get_connected_device_info = jeremymcgrathsuper_get_connected_device,
    };
}
