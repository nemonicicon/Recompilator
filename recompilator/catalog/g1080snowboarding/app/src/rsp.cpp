/**
 * rsp.cpp — RSP microcode dispatch for G1080SNOWBOARDINGPC (G1080snowboarding 64, USA). Game #4.
 *
 * AUDIO: G1080snowboarding ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void g1080snowboarding_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* g1080snowboarding_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[g1080snowboardingpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = g1080snowboarding_get_rsp_microcode,
        .capture_gfx = g1080snowboarding_run_gfx_lle,
    };
}
