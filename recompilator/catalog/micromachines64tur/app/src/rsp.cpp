/**
 * rsp.cpp — RSP microcode dispatch for MICROMACHINES64TURPC (Micromachines64tur 64, USA). Game #4.
 *
 * AUDIO: Micromachines64tur ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void micromachines64tur_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* micromachines64tur_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[micromachines64turpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = micromachines64tur_get_rsp_microcode,
        .capture_gfx = micromachines64tur_run_gfx_lle,
    };
}
