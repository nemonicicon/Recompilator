/**
 * main.cpp — Turok 2 Seeds of Evil PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Turok 2 Seeds of Evil game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: fb0400f21e3f043939ab56500c7b12a3231006f1 · internal name "Turok 2 Seeds of Evil"
static constexpr uint64_t TUROK2_ROM_HASH        = 0xC8A153E0C80403AAULL;
static constexpr uint32_t TUROK2_ENTRYPOINT_VRAM = 0x80000400;
...
static void turok2_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        turok2_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        turok2_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Turok 2 Seeds of Evil PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void turok2_message_box(const char* msg) {
    turok2_show_error("Turok 2 Seeds of Evil — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI turok2_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in turok2pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in turok2pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Turok 2 Seeds of Evil — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void turok2_vi_callback() {}
static void turok2_gfx_init_callback() {}

static std::string turok2_get_thread_name(const OSThread* t) {
    return "turok2_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Turok 2 Seeds of Evil ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            turok2_show_error("ROM Required",
                "A Turok 2 Seeds of Evil ROM is required.\n"
    ...
    // TUROK2_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("TUROK2_ROM");
    if (!rom_hint || !*rom_hint) {
        turok2_show_error("ROM Required",
            "Pass the Turok 2 Seeds of Evil ROM path as the first argument,\n"
            "or set the TUROK2_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"turok2_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        turok2_show_error("Wrong ROM",
            "The selected ROM does not match Turok 2 Seeds of Evil.\n"
            "Expected SHA1: fb0400f21e3f043939ab56500c7b12a3231006f1");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        turok2_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        turok2_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int turok2_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(turok2_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "turok2pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "turok2pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "turok2pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[turok2pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "turok2pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[turok2pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = TUROK2_ROM_HASH;
    entry.internal_name      = "Turok 2 Seeds of Evil";
    entry.game_id            = u8"turok2_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = TUROK2_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"turok2_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            turok2_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = turok2::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = turok2_vi_callback,
        .gfx_init_callback = turok2_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = turok2_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = turok2_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return turok2_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return turok2_app_main(argc, argv);
}
#endif
