...
bool read_symbols(N64Recomp::Context& context, const ELFIO::elfio& elf_file, ELFIO::section* symtab_section, const N64Recomp::ElfParsingConfig& elf_config, bool dumping_context, std::unordered_map<uint16_t, std::vector<N64Recomp::DataSymbol>>& data_syms) {
    ...
    std::unordered_map<uint16_t, std::vector<uint32_t>> named_anchors_by_section;
    {
        ...
        for (int i = 0; i < symbols.get_symbols_num(); i++) {
            ...
            // TLB-WINDOW SECTIONS (vram bit 31 clear — the Acclaim London class, turok2/3):
            // EVERY declared FUNC symbol is an anchor there. Their func_ symbols are
            // census-derived seeds (real prologues / jal targets), not splat spurious splits,
            // and the fall-through fix (bed33ef) makes a genuinely-split function CORRECT via
            // its tail call — while absorption silently drops the symbol from the lookup
            // (turok2: 221 functions swallowed, including the kernel entry 0x0028D380 the
            // boot stub jalr's into — NO_FLIPS with a perfectly healthy TLB map). KSEG0
            // sections keep the old rule: auto-named symbols stay absorbable, preserving the
            // DK64 j-continuation and LoD/CV64 severed-split repairs byte-identically.
            bool low_vram_window = static_cast<uint32_t>(k_value) < 0x80000000u;
        ...
        if (section_index < context.sections.size()) {
            ...
            if (ignored || type == ELFIO::STT_FUNC || type == ELFIO::STT_NOTYPE || type == ELFIO::STT_OBJECT) {
                ...
                if (section_index < context.sections.size()) {
                    ...
                    // Suffix local symbols to prevent name conflicts - but NEVER one that has
                    // already been renamed to a host entry point above. An overlay that bundles its
                    // own libultra has duplicate names, and localize_overlay_dups.py deliberately
                    // makes those copies LOCAL "while staying NAMED (so N64Recomp still HLE-binds
                    // them by name)". Suffixing after the HLE rename defeats exactly that: the
                    // overlay's osCreateThread becomes osCreateThread_recomp_008129E0, whose body is
                    // never emitted (it is host-provided) and which nothing defines, so the link
                    // fails on an unresolved external. An HLE name must match the host's exactly.
                    // Saikyou Habu Shougi 2026-09-08, full-RAM snapshot overlay.
                    if (bind == ELFIO::STB_LOCAL && !ignored) {
                    ...
                    if (type == ELFIO::STT_FUNC && !ignored && !reimplemented && insn_words.size() >= 4) {
                        ...
                        if (detect) {
                            ...
                            auto j_target_is_continuation = [&](uint32_t tj) -> bool {
                                ...
                                // SESSION 45 (wheel #260, GameTek/IDO -O0 `j <epilogue>` idiom): unoptimized
                                // IDO compiles EVERY early exit as `j <epilogue>`, where the epilogue chunk is
                                // this same function's frame teardown — `addu $sp,$fp,$zero` / `lw $ra,N($sp)` /
                                // `lw $fp,N($sp)` / `addiu $sp,$sp,+N` / `jr $ra`. That chunk contains no branch
                                // at all, so the DK64 branches-back test above cannot see it; extension stops one
                                // instruction short of it and the emitter turns the in-function `j` into
                                // `LOOKUP_FUNC(<epilogue>)`. That target is a mid-function address with no entry,
                                // so at runtime it is a dispatch gap: the interpreter is entered at the epilogue,
                                // returns into the middle of a function whose native frame already unwound, and
                                // never gets back to recompiled code (wheel: 2 x 50 M interpreted instructions at
                                // 0x800289DC, then every guest thread is dead and the game freezes on its logo).
                                // Recognise the shape directly: a chunk that reaches `jr $ra` WITHOUT ever
                                // establishing a frame of its own (`addiu $sp,$sp,-N`) while restoring one on the
                                // way is the shared epilogue of the function being extended, never a tail call.
                                {
                                    ...
                                    for (size_t k = 0; k < 0x20; ++k) {
                                        ...
                                        if (o + 4 > context.rom.size()) break;
                                        uint32_t cw = bswap(*reinterpret_cast<const uint32_t*>(context.rom.data() + o));
                                        if ((cw & 0xFFFF8000u) == 0x27BD8000u) break;   // addiu $sp,$sp,-N: its OWN frame => a real function
                                        if (cw == 0x03E00008u) return saw_teardown;     // jr $ra ends the chunk
                                        if (cw == 0x03C0E821u) { saw_teardown = true; continue; }              // addu $sp, $fp, $zero
                                        if ((cw & 0xFFFF0000u) == 0x8FBF0000u) { saw_teardown = true; continue; } // lw $ra, N($sp)
                                        if ((cw >> 16) == 0x27BDu) { saw_teardown = true; continue; }          // addiu $sp, $sp, +N
                                        uint32_t eop = cw >> 26;
                                        if (eop == 2u || eop == 3u) break;              // j / jal: not a plain epilogue
                                    }
                                }
                                return false;
                            };
