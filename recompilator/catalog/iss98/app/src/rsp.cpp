/**
 * rsp.cpp — RSP microcode dispatch for ISS98PC (Iss98 64, USA). Game #4.
 *
 * AUDIO: Iss98 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void iss98_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* iss98_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[iss98pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = iss98_get_rsp_microcode,
        .capture_gfx = iss98_run_gfx_lle,
    };
}
