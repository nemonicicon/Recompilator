/**
 * rsp.cpp — RSP microcode dispatch for WAVERACE64KAWASAKIPC (Waverace64kawasaki 64, USA). Game #4.
 *
 * AUDIO: Waverace64kawasaki ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void waverace64kawasaki_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* waverace64kawasaki_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[waverace64kawasakipc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = waverace64kawasaki_get_rsp_microcode,
        .capture_gfx = waverace64kawasaki_run_gfx_lle,
    };
}
