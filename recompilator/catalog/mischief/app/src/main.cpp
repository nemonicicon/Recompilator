/**
 * main.cpp — Mischief Makers PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Mischief Makers game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 5eb5a9d6f413a379fe297fe1542d21f728393a27 · internal name "Mischief Makers"
static constexpr uint64_t MISCHIEF_ROM_HASH        = 0x10FDD74D41CE91B3ULL;
static constexpr uint32_t MISCHIEF_ENTRYPOINT_VRAM = 0x80000400;
...
static void mischief_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        mischief_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        mischief_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Mischief Makers PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void mischief_message_box(const char* msg) {
    mischief_show_error("Mischief Makers — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI mischief_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in mischiefpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in mischiefpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Mischief Makers — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void mischief_vi_callback() {}
static void mischief_gfx_init_callback() {}

static std::string mischief_get_thread_name(const OSThread* t) {
    return "mischief_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Mischief Makers ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            mischief_show_error("ROM Required",
                "A Mischief Makers ROM is required.\n"
    ...
    // MISCHIEF_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("MISCHIEF_ROM");
    if (!rom_hint || !*rom_hint) {
        mischief_show_error("ROM Required",
            "Pass the Mischief Makers ROM path as the first argument,\n"
            "or set the MISCHIEF_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"mischief_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        mischief_show_error("Wrong ROM",
            "The selected ROM does not match Mischief Makers.\n"
            "Expected SHA1: 5eb5a9d6f413a379fe297fe1542d21f728393a27");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        mischief_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        mischief_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int mischief_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(mischief_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "mischiefpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "mischiefpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "mischiefpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[mischiefpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "mischiefpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[mischiefpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = MISCHIEF_ROM_HASH;
    entry.internal_name      = "Mischief Makers";
    entry.game_id            = u8"mischief_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = MISCHIEF_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"mischief_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            mischief_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = mischief::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = mischief_vi_callback,
        .gfx_init_callback = mischief_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = mischief_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = mischief_get_thread_name };
    cfg.message_queue_control = {
        .requeue_timer = true,
        .requeue_sp    = false,   // [attempt 1 2026-09-02] a full depth-1 SP queue requeued forever live-locked the pump (research_mischief.md); libultra drops
        .requeue_si    = true,
        .requeue_ai    = false,
        .requeue_vi    = false,
        .requeue_pi    = false,
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return mischief_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return mischief_app_main(argc, argv);
}
#endif
