...
namespace RT64 {
    ...
    UserConfiguration::UserConfiguration() {
        ...
        idleWorkActive = true;
#ifdef N64PC_RELEASE
        developerMode = false; // release configuration (Recompilator step 4): no F1..F4 dev tools in a shipped build
#else
        developerMode = true;  // dev tools ON by default for every recompiled game (F1 inspector / F2 RT / F3 RDRAM / F4 replacements)
#endif
    }
