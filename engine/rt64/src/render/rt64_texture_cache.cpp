...
namespace RT64 {
    ...
    void TextureCache::setRGBA32(Texture *dstTexture, RenderDevice *device, RenderCommandList *commandList, const uint8_t *bytes, size_t byteCount, uint32_t width, uint32_t height, uint32_t rowPitch, std::unique_ptr<RenderBuffer> &dstUploadResource, RenderPool *uploadResourcePool, std::mutex *uploadResourcePoolMutex) {
        ...
        // single-texture mode: copy-uploaded textures (replacements, low-mip cache) also carry
        // their own 1-slot set so per-draw binding covers them (fallback set otherwise = white).
        if (!device->getCapabilities().descriptorIndexing) {
            const RenderDescriptorRange perTexRange(RenderDescriptorRangeType::TEXTURE, 0, 1);
            const RenderDescriptorSetDesc perTexDesc(&perTexRange, 1);
            dstTexture->textureSet = device->createDescriptorSet(perTexDesc);
            dstTexture->textureSet->setTexture(0, dstTexture->texture.get(), RenderTextureLayout::SHADER_READ);
        }
    }
            ...
            if (!queueCopy.empty() || !resolvedPathQueueCopy.empty() || !afterDecodeBarriers.empty()) {
                ...
                for (size_t i = 0; i < queueSize; i++) {
                    ...
                    if (upload.decodeTMEM) {
                        ...
                        // single-texture mode: give the texture its own 1-slot descriptor set,
                        // bound per-draw to set 1/2 by the framebuffer renderer (no bindless array
                        // on devices without descriptorIndexing).
                        if (!directWorker->device->getCapabilities().descriptorIndexing) {
                            const RenderDescriptorRange perTexRange(RenderDescriptorRangeType::TEXTURE, 0, 1);
                            const RenderDescriptorSetDesc perTexDesc(&perTexRange, 1);
                            dstTexture->textureSet = directWorker->device->createDescriptorSet(perTexDesc);
                            dstTexture->textureSet->setTexture(0, dstTexture->texture.get(), RenderTextureLayout::SHADER_READ);
                        }
                    }
