...
namespace RT64 {
    ...
    VI Application::Core::decodeVI() const {
        ...
        // Optional half-res present factor (osViSetXScale/YScale). Null pointer => 1.0 (unbound apps
        // are unaffected). Applied only in VI::viewRectangle(), never in framebuffer sizing.
        vi.xScaleFactor = (VI_X_SCALE_FACTOR != nullptr) ? *VI_X_SCALE_FACTOR : 1.0f;
        vi.yScaleFactor = (VI_Y_SCALE_FACTOR != nullptr) ? *VI_Y_SCALE_FACTOR : 1.0f;
        return vi;
    }
...
#       endif
        ...
        // single-texture profile (no descriptorIndexing, e.g. a low-end ARM board): force SDR — such devices mis-render the
        // R16G16B16A16 color target — and disable raw-TMEM sampling: single-texture mode binds
        // pre-decoded RGBA per draw, so every tile must take the decode path (rawTMEM tiles would
        // have no texture to bind and fall back to white).
        if (!device->getCapabilities().descriptorIndexing) {
            usesHDR = false;
            TMEMHasher::singleTextureMode = true;
        }
