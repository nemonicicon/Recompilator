#include <cassert>
#include <cstdlib>   // getenv: RECOMP_EMIT_WATCH store-watch diagnostic (baseline-safe, gated)
#include <fstream>
...
// store-watch func-attribution fix: recomp_func_mark is emitted only at function ENTRY, so after a
// call returns the mark still names the callee (often a leaf like sqrt/matrix), mis-attributing the
// NEXT watched store to that leaf instead of the real writer. Re-emit the mark after every call so a
// watched store names the function actually performing it. Gated on RECOMP_EMIT_WATCH => when the env
// is unset this is all dead (g_emit_func_mark==false) and the generated output is byte-identical.
static const bool g_emit_func_mark = std::getenv("RECOMP_EMIT_WATCH") != nullptr;
static std::string g_cur_func_hex; // vram suffix of the function currently being emitted (e.g. "8061DBD4")
...
void N64Recomp::CGenerator::get_operand_string(Operand operand, UnaryOpType operation, const InstructionContext& context, std::string& operand_string) const {
    switch (operand) {
        ...
        case Operand::Sa32:
            ...
            break;
        case Operand::Cop1cs:
            // 2026-08-28: TRIED ctx->c1cs (persist across calls, hardware-faithful) and it
            // MEASURED WORSE on KI Gold — see memory/overlay_boundary_rules.md. Left as a local
            // pending a clean single-variable A/B; the defect it targets is real but unproven here.
...
void N64Recomp::CGenerator::get_binary_expr_string(BinaryOpType type, const BinaryOperands& operands, const InstructionContext& ctx, const std::string& output, std::string& expr_string) const {
    ...
    // TODO encode these ops to avoid needing special handling.
        ...
        // Word loads (LW -> MEM_W) route through LOAD_W so reads of hardware device registers
        // (0xA4xxxxxx MMIO) return the live register value; ordinary RDRAM loads are unchanged.
        const char* load_func = (func_string == "MEM_W") ? "LOAD_W" : func_string.c_str();
        expr_string = fmt::format("{}({}, {})", load_func, input_a, input_b);
    }
...
void N64Recomp::CGenerator::emit_function_start(const std::string& function_name, size_t func_index) const {
    ...
    fmt::print(output_file,
        ...
        // 2026-08-28: a ctx-held c1cs (persist across calls, hardware-faithful — FCR31 bit 23 is
        // a real register) was TRIED and measured WORSE on KI Gold, so it stays local pending a
        // clean single-variable A/B. The tier split is real; see the c1cs field comment in recomp.h.
        "    int c1cs = 0;\n" // cop1 conditional signal
        "    uint32_t poll_guard = 0; (void)poll_guard;\n", // load-only poll-loop back-edge counter (emit_poll_yield_guard)
        function_name);
    // store-watch diagnostic: mark the current guest func vram so recomp_store_watch_h can name the writer
    // of a watched store. Gated on RECOMP_EMIT_WATCH at recomp time => OFF leaves the output byte-identical.
    static const bool emit_watch = std::getenv("RECOMP_EMIT_WATCH") != nullptr;
    g_cur_func_hex.clear();
    if (emit_watch && function_name.rfind("func_", 0) == 0) {
        g_cur_func_hex = function_name.substr(5);
        // 2026-08-30: keep only the LEADING HEX RUN. When two sections share a vram — which the
        // RAM-snapshot overlay recipe creates deliberately (the decompressed body is registered at
        // the same vram as the packed bytes) — N64Recomp disambiguates as `func_<vram>_<romoff>`.
        // Emitting that whole tail produced `recomp_func_mark(0x80001300_00800000u);` =>
        // `error C2059: syntax error: 'bad suffix on number'`, so EVERY overlay game failed to
        // build under RECOMP_EMIT_WATCH. The mark identifies the guest function by VRAM, and the
        // vram IS the first component, so truncating is correct rather than merely convenient.
        // Measured on turok (13 declared funcs -> 2403 via the overlay); see dossier_turok.
        const size_t nonhex = g_cur_func_hex.find_first_not_of("0123456789abcdefABCDEF");
        if (nonhex != std::string::npos) {
            g_cur_func_hex.resize(nonhex);
        }
        if (!g_cur_func_hex.empty()) {
            fmt::print(output_file, "    recomp_func_mark(0x{}u);\n", g_cur_func_hex);
        }
    }
}

// re-mark the current function after a call so a watched store isn't blamed on the leaf that call entered.
void N64Recomp::CGenerator::emit_func_remark() const {
    if (g_emit_func_mark && !g_cur_func_hex.empty()) {
        fmt::print(output_file, "    recomp_func_mark(0x{}u);\n", g_cur_func_hex);
    }
}
...
void N64Recomp::CGenerator::emit_jtbl_addend_assignment(const JumpTable& jtbl, int reg) const {
    // Same store as the declaration, without defining the temp again: it is already defined at function
    // scope. Used for every emission of a jump-table `addu` after the first (a delay-slot `addu` is
    // emitted twice by design). See Generator::emit_jtbl_addend_assignment.
...
void N64Recomp::CGenerator::emit_cop0_status_read(int reg) const {
    // GENERAL FIX (sweep, mk64): a cop read into $zero (gpr 0) is a hardware no-op (writes to $zero are
    // discarded). gpr_to_string(0) == "0", so the normal `{dest} = ...` form emits `0 = ...` which is not an
    // l-value (C2106). Mirror the operand path's $zero handling: void-cast so any read side effect still runs.
    if (reg == 0) { fmt::print(output_file, "(void)cop0_status_read(ctx);\n"); return; }
...
void N64Recomp::CGenerator::emit_cop0_tlb_read(int cop0_reg, int dest_gpr) const {
    // GENERAL FIX (sweep, mk64): mfc0 $zero discards the read (no-op write to $zero). See emit_cop0_status_read.
    if (dest_gpr == 0) { fmt::print(output_file, "(void)recomp_cop0_tlb_read({});\n", cop0_reg); return; }
    fmt::print(output_file, "{} = (gpr)(int32_t)recomp_cop0_tlb_read({});\n", gpr_to_string(dest_gpr), cop0_reg);
}

void N64Recomp::CGenerator::emit_cop0_tlb_write(int cop0_reg, int src_gpr) const {
    fmt::print(output_file, "recomp_cop0_tlb_write({}, (uint32_t){});", cop0_reg, gpr_to_string(src_gpr));
}

void N64Recomp::CGenerator::emit_tlb_op(int op) const {
    const char* fn = (op == 0) ? "recomp_tlbwi"
                   : (op == 1) ? "recomp_tlbwr"
                   : (op == 2) ? "recomp_tlbp"
                   :             "recomp_tlbr";
    fmt::print(output_file, "{}();", fn);
}

void N64Recomp::CGenerator::emit_conditional_move(int rd, int rs, int rt, bool eq_zero) const {
    // movn $rd,$rs,$rt -> if ($rt != 0) $rd = $rs;   movz -> if ($rt == 0) $rd = $rs;
    // $zero semantics fall out naturally: $rt==$zero makes the literal `if (0 ==/!= 0)` const-fold
    // correctly (movz always moves, movn never), and a write to $zero ($rd==0) is discarded.
    std::string dst = gpr_to_string(rd);
    if (dst == "0") { fmt::print(output_file, "// cond-move to $zero discarded\n"); return; }
    fmt::print(output_file, "if ({} {} 0) {{ {} = {}; }}\n", gpr_to_string(rt), eq_zero ? "==" : "!=", dst, gpr_to_string(rs));
}

void N64Recomp::CGenerator::emit_cop1_cs_read(int reg) const {
    // GENERAL FIX (sweep, mk64): cfc1 $zero discards the read (no-op write to $zero). See emit_cop0_status_read.
    if (reg == 0) { fmt::print(output_file, "(void)get_cop1_cs();\n"); return; }
...
void N64Recomp::CGenerator::emit_muldiv(InstrId instr_id, int reg1, int reg2) const {
    switch (instr_id) {
        ...
        case InstrId::cpu_div:
            ...
            // cv64 SESSION 38: VR4300 faithfulness — a zero divisor must NOT trap (the hardware defines
            // the result: LO = rs<0 ? 1 : -1, HI = rs). A raw C `/` raises 0xC0000094 on the host
            // (crashed CV64 at func_8005BF48 stepping outside the Villa). Same below for divu.
            fmt::print(output_file, "lo = (S32({1}) != 0) ? S32(S64(S32({0})) / S64(S32({1}))) : ((S32({0}) < 0) ? S32(1) : S32(-1)); hi = (S32({1}) != 0) ? S32(S64(S32({0})) % S64(S32({1}))) : S32({0});\n", gpr_to_string(reg1), gpr_to_string(reg2));
            break;
        case InstrId::cpu_ddiv:
            fmt::print(output_file, "DDIV(S64({}), S64({}), &lo, &hi);\n", gpr_to_string(reg1), gpr_to_string(reg2));
            break;
        case InstrId::cpu_divu:
            // cv64 SESSION 38: VR4300 zero-divisor semantics for DIVU: LO = 0xFFFFFFFF, HI = rs.
            fmt::print(output_file, "lo = (U32({1}) != 0) ? S32(U32({0}) / U32({1})) : S32(-1); hi = (U32({1}) != 0) ? S32(U32({0}) % U32({1})) : S32({0});\n", gpr_to_string(reg1), gpr_to_string(reg2));
            break;
...
void N64Recomp::CGenerator::emit_pause_self(uint32_t pc_vram) const {
    // pc_vram = the self-branch instruction — the bare-metal drain inside pause_self latches it
    // as EPC so an interrupt-driven context save parks the task AT its halt loop (hardware
    // semantics). Draining with a stale EPC resumed the task at a long-dead pc (SOTE wall 4).
    fmt::print(output_file, "pause_self(rdram, 0x{:08X}u);\n", pc_vram);
}

void N64Recomp::CGenerator::emit_poll_yield_guard(uint32_t pc_vram) const {
    // Cooperative-scheduler fidelity for load-only memory-poll loops (e.g. SM64's
    // turn_off_audio `while (sCurrentAudioSPTask != NULL) {}`): on hardware the polled value
    // changes when a higher-priority thread preempts the spinner; recompiled threads are
    // cooperative, so the back edge must yield or the spinner starves the entire scheduler.
    // The 100k-iteration threshold keeps finite load-only scans (strlen, checksums) at full
    // speed; past it, yield every 256th iteration (~1ms cadence while genuinely stuck).
    // pc_vram = this back-edge instruction — the yield venue latches it as EPC so a bare-metal
    // drain's handler saves the TRUE resume pc (a stale EPC re-entered a task at a wrong pc and
    // poisoned its saved SR — the SOTE residual tear).
    fmt::print(output_file, "if ((++poll_guard & 0x3F) == 0 && (recomp_baremetal_peek_pending() > 0 || poll_guard >= 100000)) {{ yield_self_1ms(rdram, 0x{:08X}u); }}\n", pc_vram);
}

void N64Recomp::CGenerator::emit_calling_poll_yield_guard(uint32_t func_vram, uint32_t pc_vram) const {
    // Same fidelity for a CALLING poll-loop (body = one function call + backward branch — the
    // software work-list-drain idiom an arcade/custom-runtime main thread uses: it spins calling
    // a drain function that early-returns while the list, fed by blocked worker threads, is
    // empty). The load-only guard rejects this because of the call. Uses the NON-BLOCKING yield
    // (deliver pending interrupts + reschedule, no wait) so a genuine >100k-iteration calling
    // work loop pays only a queue check, not a 1ms stall. pc_vram: see emit_poll_yield_guard.
    fmt::print(output_file, "if ((++poll_guard & 0x3F) == 0 && (recomp_baremetal_peek_pending() > 0 || poll_guard >= 100000)) {{ yield_self_poll(rdram, 0x{:08X}u, (uint32_t)ctx->r31, 0x{:08X}u); }}\n", func_vram, pc_vram);
}

void N64Recomp::CGenerator::emit_coswitch_prologue(bool saver, bool switcher) const {
    // Guest context save/switch (librecomp/src/coswitch.cpp): the context BLOCK addresses are captured
    // at the `sw $sp` / `lw $sp` instructions (emit_coswitch_capture) into these locals.
    if (saver) {
        fmt::print(output_file, "    uint32_t coswitch_save_block = 0; (void)coswitch_save_block;\n");
    }
    if (switcher) {
        fmt::print(output_file, "    uint32_t coswitch_load_block = 0; (void)coswitch_load_block;\n");
    }
}

void N64Recomp::CGenerator::emit_coswitch_capture(int base_reg, bool save) const {
    fmt::print(output_file, "{} = (uint32_t){};\n", save ? "coswitch_save_block" : "coswitch_load_block", gpr_to_string(base_reg));
}

void N64Recomp::CGenerator::emit_coswitch_capture_const(uint32_t block, bool save) const {
    fmt::print(output_file, "{} = (uint32_t)0x{:08X}U;\n", save ? "coswitch_save_block" : "coswitch_load_block", block);
}

void N64Recomp::CGenerator::emit_coswitch_savepoint(bool return_on_resume) const {
    // The savepoint lives in THIS frame: a later resume of the saved context longjmps here. Inside a
    // swap routine (save+switch in one function) the resume then RETURNS, continuing the guest at
    // the $ra the resumer loaded; at a setjmp CALL SITE the resume simply falls through to the code
    // after the call (the guest's own check of the returned value follows), so no branch is taken.
    if (return_on_resume) {
        fmt::print(output_file, "{{ jmp_buf* coswitch_jb = recomp_coswitch_savepoint(rdram, ctx, coswitch_save_block); if (coswitch_jb != NULL && setjmp(*coswitch_jb) != 0) {{ return; }} }}\n");
    }
    else {
        fmt::print(output_file, "{{ jmp_buf* coswitch_jb = recomp_coswitch_savepoint(rdram, ctx, coswitch_save_block); if (coswitch_jb != NULL) {{ (void)setjmp(*coswitch_jb); }} }}\n");
    }
}

void N64Recomp::CGenerator::emit_coswitch_jump(int reg) const {
    // The routine has loaded the entering context's registers; hand the host stack to that
    // context's fiber. Returns nonzero only if this context is resumed without a savepoint to
    // longjmp to (then the return continues at the guest's saved $ra, the swap-routine idiom).
    fmt::print(output_file, "if (recomp_coswitch(rdram, ctx, coswitch_load_block, (uint32_t){})) {{ return; }}\n", gpr_to_string(reg));
}
...
void N64Recomp::CGenerator::process_binary_op(const BinaryOp& op, const InstructionContext& ctx) const {
    ...
    // sm64 SESSION 45: VR4300 faithfulness — writes to $zero are architecturally
    // discarded (the operand printer renders r0 as literal "0", which is not an
    // lvalue). SM64 ships `lw $zero, 0x0($t6)` dummy reads; perform the access,
    // discard the result, exactly like hardware.
    if (output == "0") {
...
void N64Recomp::CGenerator::process_unary_op(const UnaryOp& op, const InstructionContext& ctx) const {
    ...
    // sm64 SESSION 45: same $zero-destination discard as process_binary_op.
    if (output == "0") {
        fmt::print(output_file, "(void)({});\n", input);
    } else {
        fmt::print(output_file, "{} = {};\n", output, input);
...
void N64Recomp::CGenerator::process_store_op(const StoreOp& op, const InstructionContext& ctx) const {
    ...
    switch (syntax) {
        ...
        case StoreSyntax::Assignment:
            // Word stores (SW/SWC1) route through STORE_W so writes that land in the device-register
            // window (0xA4xxxxxx MMIO — e.g. raw PI DMA) reach the hardware-emulation handler instead
            // of plain RDRAM. Non-word stores never target the (word-only) device registers, and the
            // STORE_W fast path is identical to the old `MEM_W(...) =` for ordinary RDRAM addresses.
            if (func_text == "MEM_W") {
                fmt::print(output_file, "STORE_W({}, {}, {});\n", imm_str, base_str, value_input);
            } else {
                fmt::print(output_file, "{}({}, {}) = {};\n", func_text, imm_str, base_str, value_input);
            }
            {
                // store-watch diagnostic: additive (the store above is unchanged) — attribute a watched
                // write to its writer func. Gated on RECOMP_EMIT_SWATCH (separate from the lighter
                // per-FUNCTION mark) => OFF leaves output byte-identical. The per-store form is HEAVY
                // (one call per store) and can tip a fragile boot; prefer RECOMP_EMIT_WATCH (mark only).
                static const bool emit_watch = std::getenv("RECOMP_EMIT_SWATCH") != nullptr;
                if (emit_watch) {
                    fmt::print(output_file, "    recomp_store_watch_h(rdram, ({}) + ({}), (uint32_t)({}));\n",
                               base_str, imm_str, value_input);
                }
            }
            break;
    }
