/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for SWRACERPC.
 *
...
void swracer_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in swracer_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool swracer_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void swracer_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t swracer_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  swracer_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = swracer_poll_input,
        .get_input               = swracer_get_input,
        .set_rumble              = swracer_set_rumble,
        .get_connected_device_info = swracer_get_connected_device,
    };
}
