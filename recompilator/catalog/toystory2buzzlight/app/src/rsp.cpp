/**
 * rsp.cpp — RSP microcode dispatch for TOYSTORY2BUZZLIGHTPC (Toystory2buzzlight 64, USA). Game #4.
 *
 * AUDIO: Toystory2buzzlight ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void toystory2buzzlight_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* toystory2buzzlight_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[toystory2buzzlightpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = toystory2buzzlight_get_rsp_microcode,
        .capture_gfx = toystory2buzzlight_run_gfx_lle,
    };
}
