...
namespace RT64 {
    ...
    // Forward-declared so we can record the active microcode on the RSP without
    // pulling in gbi/rt64_gbi.h (which includes rt64_state.h -> circular). Keeps
    // the explicit underlying type matching the definition in rt64_gbi.h.
    ...
    struct RSP {
        ...
        // The microcode the active GBI belongs to. Set by setGBI() on every ucode
        // change. Lets branchZ/branchW gate the global forceBranch enhancement to
        // the F3DEX2 family only (see branchZ/branchW), so the per-app cv64 hardcode
        // can't mis-route DL branches on a non-F3DEX2 game.
        GBIUCode currentUCode;
        ...
        uint32_t fromSegmentedMaskedDKR(uint32_t segAddress);
        ...
        // [F3DDKR 2026-09-04] Rare's DMA Fast3D (Diddy Kong Racing / Jet Force Gemini / Mickey's
        // Speedway). Vertices are 10-byte records without texture coordinates; triangles are 16-byte
        // records that carry s/t for their three corners; the game keeps four model-view matrices
        // (each already multiplied by the projection) and selects one by move-word. See rt64_gbi_f3ddkr.
        void setVertexDKR(uint32_t address, uint32_t vtxCount, uint32_t dstIndex);
        void drawTrianglesDKR(uint32_t address, uint32_t triCount, uint32_t textureOn);
        // dstIndex / srcIndex / multiply: the 1998 revision composes slot[dst] = loaded x slot[src];
        // the 1997 one always loads (srcIndex 0, multiply false). Eight slots: DKR uses the low four,
        // Jet Force Gemini's table at DMEM 0x220 is 8 x 64 bytes.
        void dkrLoadMatrix(uint32_t address, uint32_t dstIndex, uint32_t srcIndex, bool multiply);
        void dkrSelectMatrix(uint32_t index);
        std::array<hlslpp::float4x4, 8> dkrMatrix = { hlslpp::float4x4::identity(), hlslpp::float4x4::identity(), hlslpp::float4x4::identity(), hlslpp::float4x4::identity(),
                                                     hlslpp::float4x4::identity(), hlslpp::float4x4::identity(), hlslpp::float4x4::identity(), hlslpp::float4x4::identity() };
        uint32_t dkrMatrixOffset = 0;
        uint32_t dkrVertexOffset = 0;
        uint32_t dkrTexOffset = 0;
        uint32_t dkrVertexIndex = 0;
        uint32_t dkrMatrixIndex = 0;
        bool dkrBillboard = false;
        // [dkrcensus 2026-09-06] instrument counters only (printed by [dkrcensus-tri2]).
        unsigned long long dkrSelectFromLoad = 0;
        unsigned long long dkrSelectFromMoveWord = 0;
        unsigned long long dkrMoveWordIndex[256] = {};
        unsigned long long dkrTexOffsetCmds = 0;
        unsigned long long dkrOffsetsCmds = 0;
        ...
        // True when a forced (unconditional) DL branch should be taken: the global
        // forceBranch enhancement only when the active microcode is F3DEX2-family, OR
        // the game's own extended-GBI G_EX_FORCEBRANCH_V1 request. See branchZ/branchW.
        bool forceBranchEnabled() const;
