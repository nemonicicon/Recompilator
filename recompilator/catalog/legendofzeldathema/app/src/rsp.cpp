/**
 * rsp.cpp — RSP microcode dispatch for LEGENDOFZELDATHEMAPC (Legendofzeldathema 64, USA). Game #4.
 *
 * AUDIO: Legendofzeldathema ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void legendofzeldathema_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* legendofzeldathema_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[legendofzeldathemapc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = legendofzeldathema_get_rsp_microcode,
        .capture_gfx = legendofzeldathema_run_gfx_lle,
    };
}
