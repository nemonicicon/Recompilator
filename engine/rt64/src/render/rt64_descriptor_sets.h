...
namespace RT64 {
    ...
    // capability gate (single-texture mode, low-end class): devices without descriptorIndexing
    // cannot bind the boundless gTextures[] array (PARTIALLY_BOUND/VARIABLE_COUNT/runtime arrays
    // unsupported) — the bindless set above SIGSEGVs the driver at first draw. In single-texture
    // mode each draw binds its tile textures individually (sets 1 and 2); every cached texture
    // carries one of these sets, plus one instance as the white/copy fallback. Selected at
    // renderer init via RenderDevice::getCapabilities().descriptorIndexing — desktop (D3D12 and
    // indexing-capable Vulkan) keeps the bindless path byte-identical.
    struct FramebufferRendererDescriptorSingleTextureSet : RenderDescriptorSetBase {
        uint32_t gTexture;

        FramebufferRendererDescriptorSingleTextureSet(RenderDevice *device = nullptr) {
            builder.begin();
            gTexture = builder.addTexture(0);
            builder.end();
    ...
    struct FramebufferRendererDescriptorFramebufferSet : RenderDescriptorSetBase {
        ...
        // CV64 cont.23 Brick 1 (Option D accurate-RDP depth gate): the programmable N64 depth store, bound
        // as a rasterizer-ordered UAV at u3,space3. Opaque draws read-modify-write it with the real RDP
        // coplanar/dz compare (RasterPS() under CV64_AB_RDPZ) instead of the GPU's fixed-function
        // LESS_EQUAL. Always bound to the depth target's n64DepthTextureView; only READ by the shader when
        // CV64_AB_RDPZ is enabled (otherwise an unused root param = inert).
        uint32_t gN64Depth;
        ...
        FramebufferRendererDescriptorFramebufferSet(RenderDevice *device = nullptr) {
            ...
            gN64Depth = builder.addReadWriteTexture(3);
            builder.end();
    ...
    // CV64 cont.23 Brick 1: descriptor set for the N64-depth UAV clear compute pass (ClearN64DepthCS).
    struct ClearN64DepthDescriptorSet : RenderDescriptorSetBase {
        uint32_t gN64Depth;

        ClearN64DepthDescriptorSet(RenderDevice *device = nullptr) {
            builder.begin();
            gN64Depth = builder.addReadWriteTexture(0);
            builder.end();
