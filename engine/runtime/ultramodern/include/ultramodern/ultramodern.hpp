...
// Which queue a thread currently sits on (NULLPTR if none). The host-side membership map is the
// single authority — replaces reading the game-writable t->queue TCB field (see threadqueue.cpp).
PTR(PTR(OSThread)) thread_queue_membership(PTR(OSThread) t_);
// Drop all members of a queue (osCreateMesgQueue re-init parity; see threadqueue.cpp).
void thread_queue_clear(RDRAM_ARG PTR(PTR(OSThread)) queue_);
// Host-cached scheduling priority (NC RUN 63): snapshot at the libultra API surface
// (osCreateThread/osSetThreadPri); ordering and preemption never read the game-writable TCB field.
void thread_queue_note_priority(PTR(OSThread) t_, s32 pri);
s32 thread_queue_priority(RDRAM_ARG PTR(OSThread) t_);
...
// cv64 (session 18, Reinhardt): for driving the AI interrupt at the DAC consumption rate.
uint32_t get_audio_sample_rate();
uint32_t get_audio_buffer_frames();
// Models the N64 AI's 2-deep DMA FIFO for raw-MMIO-AI games that poll AI_STATUS_FIFO_FULL to pace
// synthesis (Army Men: Sarge's Heroes). True when the device still holds >= 2 buffers of un-drained
// audio. See mmio.cpp AI_STATUS read.
bool audio_fifo_full();
