/**
 * rsp.cpp — RSP microcode dispatch for CHAMELEONTWIST_REV1PC (Chameleontwist_rev1 64, USA). Game #4.
 *
 * AUDIO: Chameleontwist_rev1 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void chameleontwist_rev1_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* chameleontwist_rev1_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[chameleontwist_rev1pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = chameleontwist_rev1_get_rsp_microcode,
        .capture_gfx = chameleontwist_rev1_run_gfx_lle,
    };
}
