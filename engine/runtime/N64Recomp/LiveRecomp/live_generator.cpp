...
// Guest-memory op table for JIT'd code (see live_recompiler.h). Zero-init here; librecomp
// installs the recomp.h-macro shims. Left empty (e.g. LiveRecompTest, which doesn't link
// librecomp), every memory op fails codegen and the trampoline's interpreter fallback runs.
extern "C" RecompLiveJitMemOps recomp_live_jit_mem_ops = {};
...
void N64Recomp::LiveGenerator::process_binary_op(const BinaryOp& op, const InstructionContext& ctx) const {
    ...
    auto do_load_op = [dst, dstw, src1, src1w, src2, src2w, this](recomp_jit_load_fn fn) {
        // Guest loads call the runtime's memory shims so JIT'd code gets the exact recomp.h
        // semantics (CV64_PHYS virtual->physical translation, LOAD_W MMIO window, open-bus,
        // byte-order XOR). A raw rdram+vaddr index is only correct for KSEG0. No shim
        // installed -> refuse to codegen; the trampoline's interpreter fallback runs instead.
        if (fn == nullptr) {
            errored = true;
            return;
        }

        // vaddr = base + immediate -> second call argument (R1).
        sljit_emit_op2(compiler, SLJIT_ADD, Registers::arithmetic_temp2, 0, src1, src1w, src2, src2w);

        // Real rdram pointer (undo the KSEG0 bias baked into the rdram register) -> first argument (R0).
        sljit_emit_op2(compiler, SLJIT_ADD, Registers::arithmetic_temp1, 0, Registers::rdram, 0, SLJIT_IMM, (sljit_sw)rdram_offset);

        // value = fn(rdram, vaddr); the shim returns the fully-formed sign/zero-extended gpr.
        sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS2(W, P, W), SLJIT_IMM, sljit_sw(fn));

        // Move the returned value into the destination.
    ...
    auto do_unaligned_load_op = [dst, dstw, src1, src1w, src2, src2w, this](recomp_jit_load_merge_fn fn) {
        // lwl/lwr/ldl/ldr merge loaded bytes into the destination register's current value.
        // The shim (do_lwl/do_lwr/do_ldl/do_ldr from recomp.h) owns the whole merge, with
        // correct CV64_PHYS address translation. See do_load_op for the null contract.
        if (fn == nullptr) {
            errored = true;
            return;
        }

        // vaddr = base + offset -> second call argument (R1).
        sljit_emit_op2(compiler, SLJIT_ADD, Registers::arithmetic_temp2, 0, src1, src1w, src2, src2w);

        // initial value = the destination register's current value -> third argument (R2).
        sljit_emit_op1(compiler, SLJIT_MOV, Registers::arithmetic_temp3, 0, dst, dstw);

        // Real rdram pointer (undo the KSEG0 bias) -> first argument (R0).
        sljit_emit_op2(compiler, SLJIT_ADD, Registers::arithmetic_temp1, 0, Registers::rdram, 0, SLJIT_IMM, (sljit_sw)rdram_offset);

        // value = fn(rdram, vaddr, initial).
        sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS3(W, P, W, W), SLJIT_IMM, sljit_sw(fn));

        // Move the merged value into the destination.
        sljit_emit_op1(compiler, SLJIT_MOV, dst, dstw, SLJIT_RETURN_REG, 0);
    };
    ...
    switch (op.type) {
        ...
        // Loads
        case BinaryOpType::LD:
            do_load_op(recomp_live_jit_mem_ops.ld);
            break;
        case BinaryOpType::LW:
            do_load_op(recomp_live_jit_mem_ops.lw);
            break;
        case BinaryOpType::LWU:
            do_load_op(recomp_live_jit_mem_ops.lwu);
            break;
        case BinaryOpType::LH:
            do_load_op(recomp_live_jit_mem_ops.lh);
            break;
        case BinaryOpType::LHU:
            do_load_op(recomp_live_jit_mem_ops.lhu);
            break;
        case BinaryOpType::LB:
            do_load_op(recomp_live_jit_mem_ops.lb);
            break;
        case BinaryOpType::LBU:
            do_load_op(recomp_live_jit_mem_ops.lbu);
            break;
        case BinaryOpType::LDL:
            do_unaligned_load_op(recomp_live_jit_mem_ops.ldl);
            break;
        case BinaryOpType::LDR:
            do_unaligned_load_op(recomp_live_jit_mem_ops.ldr);
            break;
        case BinaryOpType::LWL:
            do_unaligned_load_op(recomp_live_jit_mem_ops.lwl);
            break;
        case BinaryOpType::LWR:
            do_unaligned_load_op(recomp_live_jit_mem_ops.lwr);
            break;
        default:
            assert(false);
            errored = true;
...
void N64Recomp::LiveGenerator::process_store_op(const StoreOp& op, const InstructionContext& ctx) const {
    ...
    // All guest stores call the runtime's memory shims so JIT'd code gets the exact recomp.h
    // semantics (CV64_PHYS virtual->physical translation, STORE_W MMIO window, open-bus,
    // byte-order XOR, and the do_swl/do_swr/do_sdl/do_sdr unaligned merges). The address is
    // already in temp1 (both the reloc and the plain path above). Argument setup order
    // matters: the value may live in temp2 (R1, from get_operand_values), so read it into R2
    // BEFORE the address overwrites R1, and un-bias rdram into R0 last.
    auto do_store_call = [src, srcw, this](recomp_jit_store_fn fn) {
        // No shim installed -> refuse to codegen; the interpreter fallback runs this function.
        if (fn == nullptr) {
            errored = true;
            return;
        }

        // value -> third call argument (R2).
        sljit_emit_op1(compiler, SLJIT_MOV, Registers::arithmetic_temp3, 0, src, srcw);

        // vaddr (currently in temp1/R0) -> second argument (R1).
        sljit_emit_op1(compiler, SLJIT_MOV, Registers::arithmetic_temp2, 0, Registers::arithmetic_temp1, 0);

        // Real rdram pointer (undo the KSEG0 bias) -> first argument (R0).
        sljit_emit_op2(compiler, SLJIT_ADD, Registers::arithmetic_temp1, 0, Registers::rdram, 0, SLJIT_IMM, (sljit_sw)rdram_offset);

        // fn(rdram, vaddr, value)
        sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS3V(P, W, W), SLJIT_IMM, sljit_sw(fn));
    };

    switch (op.type) {
        case StoreOpType::SD:
        case StoreOpType::SDC1:
            do_store_call(recomp_live_jit_mem_ops.sd);
            break;
        case StoreOpType::SDL:
            do_store_call(recomp_live_jit_mem_ops.sdl);
            break;
        case StoreOpType::SDR:
            do_store_call(recomp_live_jit_mem_ops.sdr);
            break;
        case StoreOpType::SW:
        case StoreOpType::SWC1:
            do_store_call(recomp_live_jit_mem_ops.sw);
            break;
        case StoreOpType::SWL:
            do_store_call(recomp_live_jit_mem_ops.swl);
            break;
        case StoreOpType::SWR:
            do_store_call(recomp_live_jit_mem_ops.swr);
            break;
        case StoreOpType::SH:
            do_store_call(recomp_live_jit_mem_ops.sh);
            break;
        case StoreOpType::SB:
            do_store_call(recomp_live_jit_mem_ops.sb);
            break;
    }
}
...
// Emit the call to an already-resolved function pointer sitting in R0. With the runtime's
// termination bridge installed (game exes — see live_recompiler.h), the call goes through it
// so a thread_terminated thrown by the callee never tries to unwind through this sljit frame
// (no unwind info -> process death). Without a bridge (LiveRecompTest: no ultramodern, nothing
// throws), fall back to the original direct call.
static void emit_resolved_call(sljit_compiler* compiler) {
    if (recomp_live_jit_mem_ops.call_native != nullptr) {
        // The resolved function is already in R0 (the bridge's first argument).
        // Load rdram and ctx into R1 and R2, then call through the bridge.
        sljit_emit_op2(compiler, SLJIT_ADD, SLJIT_R1, 0, Registers::rdram, 0, SLJIT_IMM, (sljit_sw)rdram_offset);
        sljit_emit_op1(compiler, SLJIT_MOV, SLJIT_R2, 0, Registers::ctx, 0);
        sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS3V(P, P, P), SLJIT_IMM, sljit_sw(recomp_live_jit_mem_ops.call_native));
        return;
    }
    ...
    // Load rdram and ctx into R0 and R1.
    sljit_emit_op2(compiler, SLJIT_ADD, SLJIT_R0, 0, Registers::rdram, 0, SLJIT_IMM, (sljit_sw)rdram_offset);
    sljit_emit_op1(compiler, SLJIT_MOV, SLJIT_R1, 0, Registers::ctx, 0);
    // Call the function.
...
void N64Recomp::LiveGenerator::emit_syscall(uint32_t instr_vram) const {
    // No syscall handler installed (the live-gap engine leaves it null) -> the emitted icall
    // would land on address 0 when this function RUNS (compiles fine, dies at IP=0 with
    // "module unknown"). Refuse to codegen instead; the interpreter fallback executes the
    // syscall with full semantics. Same null contract as the guest-memory ops.
    if (inputs.syscall_handler == nullptr) {
        errored = true;
        return;
    }
    // Load rdram and ctx into R0 and R1.
...
void N64Recomp::LiveGenerator::emit_pause_self(uint32_t pc_vram) const {
    // See emit_syscall: a null pause_self (live-gap engine) must fail codegen, not emit call-0.
    if (inputs.pause_self == nullptr) {
        errored = true;
        return;
    }
    // Load rdram into R0 and the halt-loop pc into R1 (latched as EPC by bare-metal drains).
    sljit_emit_op2(compiler, SLJIT_ADD, SLJIT_R0, 0, Registers::rdram, 0, SLJIT_IMM, rdram_offset);
    sljit_emit_op1(compiler, SLJIT_MOV32, SLJIT_R1, 0, SLJIT_IMM, pc_vram);
    // Call pause_self.
    sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS2V(P, 32), SLJIT_IMM, sljit_sw(inputs.pause_self));
}
