/**
 * rsp.cpp — RSP microcode dispatch for RESIDENTEVIL2PC (Residentevil2 64, USA). Game #4.
 *
 * AUDIO: Residentevil2 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void residentevil2_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* residentevil2_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[residentevil2pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = residentevil2_get_rsp_microcode,
        .capture_gfx = residentevil2_run_gfx_lle,
    };
}
