/**
 * rsp.cpp — RSP microcode dispatch for CV64BLINDPC (Cv64blind 64, USA). Console seam ported
 * from snowkidspc (2026-07-31 batch, starfox64pc port pattern).
 *
 * AUDIO: Cv64blind ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static RspExitReason cv64blind_gfx_ucode_missing(uint8_t* /*rdram*/, uint32_t /*ucode_addr*/) {
    static int logs = 0;
    if (logs < 8) {
        logs++;
...
static RspUcodeFunc* cv64blind_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
    RspUcodeFunc* fn = recomp::rsp::dispatch_ucode(rdram, task);
    return fn != nullptr ? fn : cv64blind_gfx_ucode_missing;
}

// Desktop/HLE tier: no LLE capture (RT64 renders via send_dl).
static void cv64blind_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* cv64blind_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry serves
        // via cv64blind_run_gfx_capture; no recompiled artifact exists for this family yet.
        return rsp_noop_stub;
    default:
        fprintf(stderr, "[cv64blindpc] Unknown RSP task type %u — no-op stub\n",
...
static void cv64blind_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void cv64blind_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in cv64blind_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
void cv64blind_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    cv64blind_lle_render(rdram, job, n);
                }
...
static void cv64blind_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled gfx ucode (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = cv64blind_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            cv64blind_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            cv64blind_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) cv64blind_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/cv64blind_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=cv64blind tree=N64PC src=registry-LLE(no-artifact) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    ...
    // [0xD80,+0xA0) FNV discriminates: SK1 = sm64 = cv64blind = 19D507EA3E34D5F6 (proven
    ...
    return {
        .get_rsp_microcode = cv64blind_get_rsp_microcode,
...
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = cv64blind_run_gfx_capture,
#else
        .capture_gfx = cv64blind_run_gfx_lle,
#endif
    };
}
