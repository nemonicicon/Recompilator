/**
 * rsp.cpp — RSP microcode dispatch for PENNYRACERSPC (Pennyracers 64, USA). Game #4.
 *
 * AUDIO: Pennyracers ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void pennyracers_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* pennyracers_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[pennyracerspc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = pennyracers_get_rsp_microcode,
        .capture_gfx = pennyracers_run_gfx_lle,
    };
}
