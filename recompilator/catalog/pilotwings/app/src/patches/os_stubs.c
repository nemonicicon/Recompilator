 ...
 *
 * SESSION 44: identified in PILOTWINGS by signature (tools/match_libultra.py); these
 * four are the ones PILOTWINGS's recompiled code actually calls.
 *
 ...
 * mapOverlay HLE), pilotwingspc has no game patches — the game's own recompiled
...
/* [linkfix 2026-08-28] __osSiRawStartDma stub REMOVED. pilotwings.toml:735 lists
 * __osSiRawStartDma as a function to recompile, so a real body is emitted into RecompiledFuncs --
 * and this hand-written stub collided with it: LNK2005 "already defined in os_stubs.obj".
 * shadowspc keeps its stub because shadows.toml never lists the function at all, so nothing is
 * emitted there; the two trees disagreed on which side provides it. Kept the RECOMPILED body: the
 * stub only returned 0 and its own comment called it dead at runtime, while the recompiled code
 * does the real SI work through librecomp's MMIO handlers. */
