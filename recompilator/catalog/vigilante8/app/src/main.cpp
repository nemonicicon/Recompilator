/**
 * main.cpp — Vigilante 8 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Vigilante 8 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 22b7d7b4f1722efd52d3c8beba8cf5d07340569b · internal name "Vigilante 8"
static constexpr uint64_t VIGILANTE8_ROM_HASH        = 0x43C254629928B027ULL;
static constexpr uint32_t VIGILANTE8_ENTRYPOINT_VRAM = 0x80125800;
...
static void vigilante8_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        vigilante8_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        vigilante8_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Vigilante 8 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void vigilante8_message_box(const char* msg) {
    vigilante8_show_error("Vigilante 8 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI vigilante8_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in vigilante8pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in vigilante8pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Vigilante 8 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void vigilante8_vi_callback() {}
static void vigilante8_gfx_init_callback() {}

static std::string vigilante8_get_thread_name(const OSThread* t) {
    return "vigilante8_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Vigilante 8 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            vigilante8_show_error("ROM Required",
                "A Vigilante 8 ROM is required.\n"
    ...
    // VIGILANTE8_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("VIGILANTE8_ROM");
    if (!rom_hint || !*rom_hint) {
        vigilante8_show_error("ROM Required",
            "Pass the Vigilante 8 ROM path as the first argument,\n"
            "or set the VIGILANTE8_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"vigilante8_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        vigilante8_show_error("Wrong ROM",
            "The selected ROM does not match Vigilante 8.\n"
            "Expected SHA1: 22b7d7b4f1722efd52d3c8beba8cf5d07340569b");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        vigilante8_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        vigilante8_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int vigilante8_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(vigilante8_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "vigilante8pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "vigilante8pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "vigilante8pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[vigilante8pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "vigilante8pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[vigilante8pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = VIGILANTE8_ROM_HASH;
    entry.internal_name      = "Vigilante 8";
    entry.game_id            = u8"vigilante8_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = VIGILANTE8_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"vigilante8_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            vigilante8_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    vigilante8_audio_init();
    vigilante8_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = vigilante8::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = vigilante8_vi_callback,
        .gfx_init_callback = vigilante8_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = vigilante8_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = vigilante8_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return vigilante8_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return vigilante8_app_main(argc, argv);
}
#endif
