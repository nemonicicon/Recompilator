/**
 * rsp.cpp — RSP microcode dispatch for F1WGPPC (F1wgp 64, USA). Game #4.
 *
 * AUDIO: F1wgp ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void f1wgp_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* f1wgp_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[f1wgppc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = f1wgp_get_rsp_microcode,
        .capture_gfx = f1wgp_run_gfx_lle,
    };
}
