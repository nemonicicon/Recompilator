...
namespace RT64 {
    ...
    void RenderTarget::releaseTextures() {
        ...
        dummyTexture.reset();
        n64DepthTextureView.reset();   // CV64 cont.23 Brick 1
        n64DepthTexture.reset();
    ...
    void RenderTarget::setupDepth(RenderWorker *worker, uint32_t width, uint32_t height) {
        ...
        // CV64 cont.23 Brick 1: allocate the programmable N64-depth UAV alongside the HW depth target.
        // R32_FLOAT, STORAGE|UNORDERED_ACCESS. INERT for now (no reader/writer until the ROV path + shader
        // compare land). Non-MSAA for this first increment; MSAA reconciliation is a later increment.
        n64DepthTexture = worker->device->createTexture(RenderTextureDesc::Texture2D(width, height, 1, RenderFormat::R32_FLOAT, RenderTextureFlag::STORAGE | RenderTextureFlag::UNORDERED_ACCESS));
        n64DepthTextureView = n64DepthTexture->createTextureView(RenderTextureViewDesc::Texture2D(RenderFormat::R32_FLOAT));
        n64DepthTexture->setName("Render Target N64 Depth #" + std::to_string(addressForName));
        textureRevision++;
    }
