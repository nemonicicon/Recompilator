...
namespace RT64 {
    namespace GBI_RDP {
        ...
        void setColorImage(State *state, DisplayList **dl) {
            ...
            // [cimgraw] KI Gold 2026-08-29. RT64 was rejecting a flood of setColorImage with
            // width 1000-3900 (valid <=1024) while fmt/siz decoded PERFECTLY SANE (0 / 2 =
            // RGBA/16-bit). A wholly corrupt word would corrupt all three fields, so either the
            // low 12 bits are being read from the wrong place (a pre-SDK ucode that moved the
            // parameter bits -- see [[ucode_param_layouts]], which cracked SOTE the same way), or
            // the word is real and something else is wrong. Print the RAW w0/w1 so the field
            // layout can be checked by hand instead of guessed at.
            // Fires ONLY on an out-of-range width, capped -- but the cap is REPORTED, so a count
            // at the cap is never mistaken for a measurement.
            if (width == 0u || width > 1024u) {
                static int _n = 0;
                if (_n++ < 24) {
                    fprintf(stderr, "[cimgraw] #%d RAW w0=0x%08X w1=0x%08X -> fmt=%u siz=%u width=%u addr=0x%08X"
                                    "  | bitslice w0: [31:24]=0x%02X [23:16]=0x%02X [15:8]=0x%02X [7:0]=0x%02X%s",
                            _n, (*dl)->w0, (*dl)->w1, (unsigned)fmt, (unsigned)siz, (unsigned)width, address,
                            (unsigned)((*dl)->w0 >> 24 & 0xFF), (unsigned)((*dl)->w0 >> 16 & 0xFF),
                            (unsigned)((*dl)->w0 >> 8 & 0xFF), (unsigned)((*dl)->w0 & 0xFF),
