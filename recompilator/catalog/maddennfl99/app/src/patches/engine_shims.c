 ...
 *     engine. Keyed to CV64 boot conditions; must never fire for MADDENNFL99.
 ...
 *     correct for static-only MADDENNFL99 (overlay recompilation is PHASE 3).
...
/* 1 — CV64 NI boot-stub hook; loudly visible if it ever fires for MADDENNFL99. */
RECOMP_FUNC void gamestate_change(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    fprintf(stderr, "[engine_shim] gamestate_change(%d) called — CV64 NI boot-stub path fired for MADDENNFL99?!\n",
            (int)ctx->r4);
    fflush(stderr);
}
