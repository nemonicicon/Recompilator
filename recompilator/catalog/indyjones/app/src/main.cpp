/**
 * main.cpp — Indiana Jones Infernal Machine PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Indiana Jones Infernal Machine game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 93300d63412d6c7432109b5ad2e4a8b9348e9538 · internal name "Indiana Jones Infernal Machine"
static constexpr uint64_t INDYJONES_ROM_HASH        = 0xAA0401123B477996ULL;
static constexpr uint32_t INDYJONES_ENTRYPOINT_VRAM = 0x80000400;
...
static void indyjones_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        indyjones_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Indiana Jones Infernal Machine PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        indyjones_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Indiana Jones Infernal Machine PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void indyjones_message_box(const char* msg) {
    indyjones_show_error("Indiana Jones Infernal Machine — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI indyjones_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in indyjonespc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in indyjonespc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Indiana Jones Infernal Machine — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void indyjones_vi_callback() {}
static void indyjones_gfx_init_callback() {}

static std::string indyjones_get_thread_name(const OSThread* t) {
    return "indyjones_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Indiana Jones Infernal Machine ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            indyjones_show_error("ROM Required",
                "A Indiana Jones Infernal Machine ROM is required.\n"
    ...
    // INDYJONES_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("INDYJONES_ROM");
    if (!rom_hint || !*rom_hint) {
        indyjones_show_error("ROM Required",
            "Pass the Indiana Jones Infernal Machine ROM path as the first argument,\n"
            "or set the INDYJONES_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"indyjones_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        indyjones_show_error("Wrong ROM",
            "The selected ROM does not match Indiana Jones Infernal Machine.\n"
            "Expected SHA1: 93300d63412d6c7432109b5ad2e4a8b9348e9538");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        indyjones_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        indyjones_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int indyjones_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(indyjones_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "indyjonespc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "indyjonespc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "indyjonespc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[indyjonespc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "indyjonespc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[indyjonespc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = INDYJONES_ROM_HASH;
    entry.internal_name      = "Indiana Jones Infernal Machine";
    entry.game_id            = u8"indyjones_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = INDYJONES_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"indyjones_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            indyjones_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    indyjones_audio_init();
    indyjones_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = indyjones::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = indyjones_vi_callback,
        .gfx_init_callback = indyjones_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = indyjones_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = indyjones_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return indyjones_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return indyjones_app_main(argc, argv);
}
#endif
