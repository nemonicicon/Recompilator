/**
 * rsp.cpp — RSP microcode dispatch for NABOOPC (Naboo 64, USA). Game #4.
 *
 * AUDIO: Naboo ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void naboo_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* naboo_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[naboopc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = naboo_get_rsp_microcode,
        .capture_gfx = naboo_run_gfx_lle,
    };
}
