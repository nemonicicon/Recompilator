/**
 * rsp.cpp — RSP microcode dispatch for DUALHEROESPC (Dualheroes 64, USA). Game #4.
 *
 * AUDIO: Dualheroes ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void dualheroes_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* dualheroes_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[dualheroespc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = dualheroes_get_rsp_microcode,
        .capture_gfx = dualheroes_run_gfx_lle,
    };
}
