...
void waverace_queue_samples(int16_t* audio_data, size_t sample_count) {
...
size_t waverace_get_frames_remaining() {
...
void waverace_set_frequency(uint32_t freq) {
    fprintf(stderr, "[audio_freq] waverace_set_frequency(%u) (was %u)\n", freq, current_freq);
    fflush(stderr);
...
void waverace_audio_init() {
    waverace_set_frequency(current_freq);
}

// Public callback struct to pass to recomp::start().
ultramodern::audio_callbacks_t get_audio_callbacks() {
    return {
        .queue_samples      = waverace_queue_samples,
        .get_frames_remaining = waverace_get_frames_remaining,
        .set_frequency      = waverace_set_frequency,
    };
}
