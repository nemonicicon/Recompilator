...
N64Recomp::Config::Config(const char* path) {
    ...
    try {
        ...
        // Exact-size functions (optional): names whose manual size is ground truth — never auto-extended,
        // and a hard boundary for neighbor auto-extension. (Carved bare-metal exception dispatch tails.)
        toml::node_view exact_size_data = input_data["exact_size_funcs"];
        if (exact_size_data.is_array()) {
            exact_size_data.as_array()->for_each([this](auto&& el) {
                if constexpr (toml::is_string<decltype(el)>) {
                    exact_size_funcs.push_back(*el);
                }
            });
        }
