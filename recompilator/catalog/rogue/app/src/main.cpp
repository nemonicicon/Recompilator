/**
 * main.cpp — Star Wars Rogue Squadron PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Star Wars Rogue Squadron game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: ed42eed1ee2db646ff7ef94ba8c5421d164a4f0d · internal name "Star Wars Rogue Squadron"
static constexpr uint64_t ROGUE_ROM_HASH        = 0x6B66A44153594DEAULL;
static constexpr uint32_t ROGUE_ENTRYPOINT_VRAM = 0x80000400;
...
static void rogue_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        rogue_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        rogue_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Star Wars Rogue Squadron PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void rogue_message_box(const char* msg) {
    rogue_show_error("Star Wars Rogue Squadron — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI rogue_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in roguepc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in roguepc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Star Wars Rogue Squadron — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void rogue_vi_callback() {}
static void rogue_gfx_init_callback() {}

static std::string rogue_get_thread_name(const OSThread* t) {
    return "rogue_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Star Wars Rogue Squadron ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            rogue_show_error("ROM Required",
                "A Star Wars Rogue Squadron ROM is required.\n"
    ...
    // ROGUE_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("ROGUE_ROM");
    if (!rom_hint || !*rom_hint) {
        rogue_show_error("ROM Required",
            "Pass the Star Wars Rogue Squadron ROM path as the first argument,\n"
            "or set the ROGUE_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"rogue_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        rogue_show_error("Wrong ROM",
            "The selected ROM does not match Star Wars Rogue Squadron.\n"
            "Expected SHA1: ed42eed1ee2db646ff7ef94ba8c5421d164a4f0d");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        rogue_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        rogue_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int rogue_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(rogue_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "roguepc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "roguepc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "roguepc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[roguepc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "roguepc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[roguepc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = ROGUE_ROM_HASH;
    entry.internal_name      = "Star Wars Rogue Squadron";
    entry.game_id            = u8"rogue_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = ROGUE_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"rogue_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            rogue_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = rogue::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = rogue_vi_callback,
        .gfx_init_callback = rogue_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = rogue_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = rogue_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return rogue_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return rogue_app_main(argc, argv);
}
#endif
