/**
 * rsp.cpp — RSP microcode dispatch for RAGEWARSR1PC (Ragewarsr1 64, USA). Game #4.
 *
 * AUDIO: Ragewarsr1 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void ragewarsr1_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* ragewarsr1_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[ragewarsr1pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = ragewarsr1_get_rsp_microcode,
        .capture_gfx = ragewarsr1_run_gfx_lle,
    };
}
