/**
 * rsp.cpp — RSP microcode dispatch for WORLDCUP98PC (Worldcup98 64, USA). Game #4.
 *
 * AUDIO: Worldcup98 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void worldcup98_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* worldcup98_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[worldcup98pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = worldcup98_get_rsp_microcode,
        .capture_gfx = worldcup98_run_gfx_lle,
    };
}
