 ...
 *     engine. Keyed to CV64 boot conditions; must never fire for NFLQUARTERBACKCLUB.
 ...
 *     correct for static-only NFLQUARTERBACKCLUB (overlay recompilation is PHASE 3).
...
/* 1 — CV64 NI boot-stub hook; loudly visible if it ever fires for NFLQUARTERBACKCLUB. */
RECOMP_FUNC void gamestate_change(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    fprintf(stderr, "[engine_shim] gamestate_change(%d) called — CV64 NI boot-stub path fired for NFLQUARTERBACKCLUB?!\n",
            (int)ctx->r4);
    fflush(stderr);
}
