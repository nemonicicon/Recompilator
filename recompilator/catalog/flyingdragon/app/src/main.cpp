/**
 * main.cpp — Flying Dragon PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Flying Dragon game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 5472fe1dcaaacaecba8d1bed8eab4a2146e58665 · internal name "Flying Dragon"
static constexpr uint64_t FLYINGDRAGON_ROM_HASH        = 0x8E8732EA69FE2D1EULL;
static constexpr uint32_t FLYINGDRAGON_ENTRYPOINT_VRAM = 0x80096400;
...
static void flyingdragon_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        flyingdragon_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        flyingdragon_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Flying Dragon PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void flyingdragon_message_box(const char* msg) {
    flyingdragon_show_error("Flying Dragon — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI flyingdragon_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in flyingdragonpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in flyingdragonpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Flying Dragon — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void flyingdragon_vi_callback() {}
static void flyingdragon_gfx_init_callback() {}

static std::string flyingdragon_get_thread_name(const OSThread* t) {
    return "flyingdragon_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Flying Dragon ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            flyingdragon_show_error("ROM Required",
                "A Flying Dragon ROM is required.\n"
    ...
    // FLYINGDRAGON_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("FLYINGDRAGON_ROM");
    if (!rom_hint || !*rom_hint) {
        flyingdragon_show_error("ROM Required",
            "Pass the Flying Dragon ROM path as the first argument,\n"
            "or set the FLYINGDRAGON_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"flyingdragon_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        flyingdragon_show_error("Wrong ROM",
            "The selected ROM does not match Flying Dragon.\n"
            "Expected SHA1: 5472fe1dcaaacaecba8d1bed8eab4a2146e58665");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        flyingdragon_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        flyingdragon_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int flyingdragon_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(flyingdragon_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "flyingdragonpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "flyingdragonpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "flyingdragonpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[flyingdragonpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "flyingdragonpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[flyingdragonpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = FLYINGDRAGON_ROM_HASH;
    entry.internal_name      = "Flying Dragon";
    entry.game_id            = u8"flyingdragon_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = FLYINGDRAGON_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"flyingdragon_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            flyingdragon_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    flyingdragon_audio_init();
    flyingdragon_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = flyingdragon::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = flyingdragon_vi_callback,
        .gfx_init_callback = flyingdragon_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = flyingdragon_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = flyingdragon_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return flyingdragon_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return flyingdragon_app_main(argc, argv);
}
#endif
