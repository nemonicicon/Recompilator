...
namespace interop {
    ...
    struct Blender {
        ...
        // Additive framebuffer blend: M = framebuffer color and B = ONE, so M*B keeps the FULL framebuffer
        // (out = src*A + framebuffer*1). This is additive on N64 hardware — a black source adds nothing
        // (transparent), a bright source glows. RT64's default alpha-blend path uses dstBlend =
        // INV_SRC1_ALPHA (alpha-over), which cannot express the "+ framebuffer*1" term and instead
        // overwrites the framebuffer wherever the source alpha is 1.0 — turning e.g. Space Invaders 64's
        // shield (a CI8 effect whose index-0 background decodes to opaque black, alpha 1.0) into an opaque
        // black box. createPipeline uses dstBlend = ONE for these draws so the framebuffer is preserved and
        // the source is added. Distinct from standard XLU alpha-over (B = B_ONE_MINUS_A) and fog
        // (M != framebuffer), so those classes are unaffected.
        static bool usesFramebufferAdditiveBlend(const OtherMode otherMode) {
            ...
            for (uint c = 0; c < cycles; c++) {
                ...
                const InputPM M = decodeInputM(blenderInputs, secondCycle);
                const InputB B = decodeInputB(blenderInputs, secondCycle);
                if ((M == PM_FRAMEBUFFER_COLOR) && (B == B_ONE)) {
                    return true;
                }
            }
