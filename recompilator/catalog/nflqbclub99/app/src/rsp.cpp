/**
 * rsp.cpp — RSP microcode dispatch for NFLQBCLUB99PC (Nflqbclub99 64, USA). Game #4.
 *
 * AUDIO: Nflqbclub99 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nflqbclub99_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nflqbclub99_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nflqbclub99pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nflqbclub99_get_rsp_microcode,
        .capture_gfx = nflqbclub99_run_gfx_lle,
    };
}
