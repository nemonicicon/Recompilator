/**
 * rsp.cpp — RSP microcode dispatch for ASTEROIDSPC (Asteroids Hyper 64, USA). Game #6 on a framebuffer target tier (starfox64pc port pattern).
 *
 * AUDIO: Asteroids ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
// ── LLE CONSOLE (2026-07-24, Asteroids Pi port — the desktop-broken-game drive) ──────────────
// gfx = rsp/f3dex2.cpp: the cv64-family F3DEX fifo 2.06 artifact REUSED VERBATIM — Asteroids
// ships the SAME byte-build (125671d5b05e, BUILD_CENSUS: 24 games incl. every Castlevania),
// so the 64/64 bit-exact judgment (f3dex2_ab, cv64 capture bank) carries whole. Registered
// under cv64's organ constants (identical bytes = identical key).
// Asteroids ALSO ships L3DEX fifo 2.06 (LINE ucode, build 70838c201560 — NOT in the
// catalog). If the game fires it, [ucode-miss] names the key = the next family to recompile;
// the f3dex2 fallback would render its tasks WRONG — watch for it before trusting visuals.
RspExitReason f3dex2(uint8_t* rdram, uint32_t ucode_addr);

static RspUcodeFunc* asteroids_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
...
static void asteroids_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* asteroids_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry/artifact
        // serve via asteroids_run_gfx_capture; this documents the mapping and keeps the artifact linked.
        return f3dex2;
    default:
        fprintf(stderr, "[asteroidspc] Unknown RSP task type %u — no-op stub\n",
...
// F3DEX2 (asteroids_recomp, f3dex2.cpp) runs per gfx task; DO_DP_END feeds the librecomp walker whose
...
static void asteroids_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void asteroids_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in asteroids_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
// Name kept as asteroids_lle_console_submit — lle_console.cpp (copied verbatim) references it.
void asteroids_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    asteroids_lle_render(rdram, job, n);
                }
...
static void asteroids_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled F3DEX2 (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = asteroids_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            asteroids_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            asteroids_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) asteroids_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/asteroids_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=asteroids tree=N64PC src=f3dex2-LLE(RSPRecomp) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    ...
    // offline, SERVED_ROSTER rung, ENGINE_LEDGER 07-24).
    ...
    return {
        .get_rsp_microcode = asteroids_get_rsp_microcode,
...
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = asteroids_run_gfx_capture,
#else
        .capture_gfx = asteroids_run_gfx_lle,
#endif
    };
}
