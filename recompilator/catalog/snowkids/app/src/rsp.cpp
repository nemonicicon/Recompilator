/**
 * rsp.cpp — RSP microcode dispatch for SNOWKIDSPC (Snowkids 64, USA). Game #6 on a framebuffer target tier (starfox64pc port pattern).
 *
 * AUDIO: Snowkids ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
// ── LLE CONSOLE (2026-07-24, SK1 Pi port — starfox64pc pattern): F3DLX 1.23 recompiled ───────
// RecompiledFuncs/f3dlx.cpp — RSPRecomp artifact (f3dlx.toml), JUDGED 16/16 live-capture
// bit-exact vs the real ucode (oracle_rig f3dlx_ab, catalog hash C2A86DF957DA9470). The console
// tier consumes the UCODE REGISTRY; fallback = the direct artifact call. gfx registration is
// deferred to the organ's own [ucode-miss] lines from the first Pi run (bootstrap by design;
// F3DLX 1.x keys collide across sibling builds ⇒ window constants must come from a live run).
RspExitReason f3dlx(uint8_t* rdram, uint32_t ucode_addr);

static RspUcodeFunc* snowkids_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
    RspUcodeFunc* fn = recomp::rsp::dispatch_ucode(rdram, task);
    return fn != nullptr ? fn : f3dlx;
}

// Desktop/HLE tier: no LLE capture (RT64 renders via send_dl).
static void snowkids_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* snowkids_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry/artifact
        // serve via snowkids_run_gfx_capture; this documents the mapping and keeps the artifact linked.
        return f3dlx;
    default:
        fprintf(stderr, "[snowkidspc] Unknown RSP task type %u — no-op stub\n",
...
// F3DEX2 (snowkids_recomp, f3dex2.cpp) runs per gfx task; DO_DP_END feeds the librecomp walker whose
...
static void snowkids_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void snowkids_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in snowkids_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
// Name kept as snowkids_lle_console_submit — lle_console.cpp (copied verbatim) references it.
void snowkids_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    snowkids_lle_render(rdram, job, n);
                }
...
static void snowkids_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled F3DEX2 (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = snowkids_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            snowkids_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            snowkids_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) snowkids_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/snowkids_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=snowkids tree=N64PC src=f3dex2-LLE(RSPRecomp) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    ...
    return {
        .get_rsp_microcode = snowkids_get_rsp_microcode,
...
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = snowkids_run_gfx_capture,
#else
        .capture_gfx = snowkids_run_gfx_lle,
#endif
    };
}
