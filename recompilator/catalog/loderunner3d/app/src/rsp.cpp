/**
 * rsp.cpp — RSP microcode dispatch for LODERUNNER3DPC (Loderunner3d 64, USA). Game #4.
 *
 * AUDIO: Loderunner3d ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void loderunner3d_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* loderunner3d_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[loderunner3dpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = loderunner3d_get_rsp_microcode,
        .capture_gfx = loderunner3d_run_gfx_lle,
    };
}
