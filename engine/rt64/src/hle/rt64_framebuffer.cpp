...
namespace RT64 {
    ...
    uint32_t Framebuffer::copyRAMToNativeAndChanges(RenderWorker *worker, FramebufferChange &fbChange, const uint8_t *src, uint32_t rowStart, uint32_t rowCount, uint8_t fmt, bool invalidateTargets, const ShaderLibrary *shaderLibrary) {
        ...
        // GUARD (session 28 cont.19, KEEP): reject a degenerate/corrupt framebuffer reaching the
        // read-back path. A bad gsDPSetColorImage / framebuffer-change can yield width==0, width>1024
        // (N64 framebuffer width maxes at 1024), or 0 rows → nativeSwappedRAM is short/empty while
        // copyFromRAM's bufferSize is nonzero → a wild READ inside copyFromRAM (0xC0000005 READ 0x0 on
        // the RENDER thread → graphics deadlock / the [mq_ext] VI-retrace flood). Same invariant-guard
        // class as the cont.17 setColorImage / copyNativeToRAM guards. No real CV64 framebuffer hits this.
        if ((src == nullptr) || (width == 0) || (width > 1024) || (rowCount == 0)) {
            static int _fbcopyGuard = 0;
            if (++_fbcopyGuard <= 32) {
                fprintf(stderr, "[fbcopy] skip degenerate readback: width=%u rowCount=%u siz=%u src=%p\n",
                        width, rowCount, (unsigned)siz, (const void *)src);
                fflush(stderr);
            }
            return 0;
        }
