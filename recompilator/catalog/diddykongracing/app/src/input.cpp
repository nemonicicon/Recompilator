/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for DIDDYKONGRACINGPC.
 *
...
void diddykongracing_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in diddykongracing_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool diddykongracing_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void diddykongracing_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t diddykongracing_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  diddykongracing_get_input() already supplies
...
void diddykongracing_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = diddykongracing_poll_input,
        .get_input               = diddykongracing_get_input,
        .set_rumble              = diddykongracing_set_rumble,
        .get_connected_device_info = diddykongracing_get_connected_device,
    };
}
