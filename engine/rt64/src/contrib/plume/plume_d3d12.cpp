...
namespace plume {
    // cv64 S38: the Villa-courtyard streaming session EXHAUSTED a descriptor heap ("Allocator was unable
    // to find free space") → the set kept INVALID_OFFSET → wild descriptor write → driver AV (the
    // "stepping outside the Villa" crash, previously misattributed to a recompiled game fn via a wrong-module
    // RVA lookup). Raise capacities to real D3D12 limits' neighborhood: CBV/SRV/UAV shader-visible max is 1M
    // (512K leaves margin, ~16MB of descriptors); sampler shader-visible max is 2048; RTV/DSV heaps are
    // non-shader-visible (no hard cap). Telemetry in allocate() reports occupancy so a leak still surfaces.
    static const uint32_t ShaderDescriptorHeapSize = 524288;
    static const uint32_t SamplerDescriptorHeapSize = 2048;
    static const uint32_t TargetDescriptorHeapSize = 65536;
...
#   endif
    ...
    D3D12DescriptorHeapAllocator::D3D12DescriptorHeapAllocator(D3D12Device *device, uint32_t heapSize, D3D12_DESCRIPTOR_HEAP_TYPE heapType) {
        ...
        this->allocatorHeapType = heapType;   // cv64 S38: for log labels
    ...
    // cv64 S38: heap-type label for the exhaustion/occupancy logs.
    static const char *cv64_heapTypeName(D3D12_DESCRIPTOR_HEAP_TYPE t) {
        switch (t) {
        case D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV: return "view(CBV/SRV/UAV)";
    ...
    uint32_t D3D12DescriptorHeapAllocator::allocate(uint32_t size) {
        ...
        freeSize -= size;
        // cv64 S38 telemetry: log once per occupancy threshold crossed (50/75/90/95%) so a leak vs a
        // legit working-set growth is visible in the diag log long BEFORE exhaustion. lowSpaceMark only
        // ratchets up (no per-frame spam); freed-then-refilled heaps don't re-log.
        {
            uint32_t usedPct = (heapSize > 0) ? (uint32_t)(100ull * (heapSize - freeSize) / heapSize) : 0;
            static const uint32_t marks[] = { 50, 75, 90, 95 };
            for (uint32_t m : marks) {
                if (usedPct >= m && lowSpaceMark < m) {
                    lowSpaceMark = m;
                    fprintf(stderr, "[descheap] %s heap at %u%% occupancy (used=%u/%u)\n",
                            cv64_heapTypeName(allocatorHeapType), usedPct, heapSize - freeSize, heapSize);
                    fflush(stderr);
                }
            }
        }
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE D3D12DescriptorHeapAllocator::getCPUHandleAt(uint32_t index) const {
        ...
        // cv64 S38: a failed allocation leaves offset = INVALID_OFFSET; downstream handle math then lands
        // FAR outside the heap and the descriptor write AVs inside D3D12/the driver (the Villa crash).
        // Asserts are compiled out in Release — clamp to slot 0 instead: a wrong-but-valid descriptor
        // (visual glitch at worst) beats a process crash. Single choke point covers every misuse path.
        if (index >= heapSize) {
            static int _n = 0;
            if (_n++ < 16) { fprintf(stderr, "[descheap] OOB CPU handle idx=%u on %s heap (size=%u) — clamped to 0\n", index, cv64_heapTypeName(allocatorHeapType), heapSize); fflush(stderr); }
            index = 0;
        }
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE D3D12DescriptorHeapAllocator::getGPUHandleAt(uint32_t index) const {
        ...
        if (index >= heapSize) {   // cv64 S38: see getCPUHandleAt
            static int _n = 0;
            if (_n++ < 16) { fprintf(stderr, "[descheap] OOB GPU handle idx=%u on %s heap (size=%u) — clamped to 0\n", index, cv64_heapTypeName(allocatorHeapType), heapSize); fflush(stderr); }
            index = 0;
        }
    ...
    D3D12DescriptorSet::D3D12DescriptorSet(D3D12Device *device, const RenderDescriptorSetDesc &desc) {
        ...
        if (viewDescriptorCount > 0) {
            ...
            if (viewAllocation.offset == D3D12DescriptorHeapAllocator::INVALID_OFFSET) {
                fprintf(stderr, "[descheap] EXHAUSTED view(CBV/SRV/UAV) heap (set wanted %u, free=%u/%u)\n",
                        viewDescriptorCount, device->viewHeapAllocator->freeSize, device->viewHeapAllocator->heapSize);
                fflush(stderr);
                return;
            }
        ...
        if (samplerDescriptorCount > 0) {
            ...
            if (samplerAllocation.offset == D3D12DescriptorHeapAllocator::INVALID_OFFSET) {
                fprintf(stderr, "[descheap] EXHAUSTED sampler heap (set wanted %u, free=%u/%u)\n",
                        samplerDescriptorCount, device->samplerHeapAllocator->freeSize, device->samplerHeapAllocator->heapSize);
                fflush(stderr);
                return;
            }
    ...
    void D3D12Framebuffer::createRenderTargetHeap(const D3D12Texture* texture, const D3D12TextureView* textureView) {
        ...
        if (targetAllocatorOffset == D3D12DescriptorHeapAllocator::INVALID_OFFSET) {
            fprintf(stderr, "[descheap] EXHAUSTED RTV heap (free=%u/%u)\n",
                    device->colorTargetHeapAllocator->freeSize, device->colorTargetHeapAllocator->heapSize);
            fflush(stderr);
            return;
        }
    ...
    void D3D12Framebuffer::createDepthStencilHeap(const D3D12Texture* texture, const D3D12TextureView* textureView, const bool readOnly) {
        ...
        if (targetAllocatorOffset == D3D12DescriptorHeapAllocator::INVALID_OFFSET) {
            fprintf(stderr, "[descheap] EXHAUSTED DSV heap (free=%u/%u)\n",
                    device->depthTargetHeapAllocator->freeSize, device->depthTargetHeapAllocator->heapSize);
            fflush(stderr);
            return;
        }
...
#   endif
    ...
    void D3D12CommandList::barriers(RenderBarrierStages stages, const RenderBufferBarrier *bufferBarriers, uint32_t bufferBarriersCount, const RenderTextureBarrier *textureBarriers, uint32_t textureBarriersCount) {
        ...
        for (uint32_t i = 0; i < bufferBarriersCount; i++) {
            ...
            // CV64 invariant-guard (cont.17 render-crash class): a corrupt/collided overlay DL can drive
            // RT64 to submit a barrier on a resource that was never created -> null interface -> crash
            // dereferencing ->resourceStates (+0x10). Skip it (a null resource has nothing to transition).
            // Deeper root = the 0x0F overlay-collision corrupt DL (badcolorimg/dlguard nonzero), not fixed here.
            if (interfaceBuffer == nullptr) {
                static int _badbb = 0;
                if (_badbb++ < 32) { fprintf(stderr, "[badbarrier] SKIP null BUFFER barrier i=%u/%u\n", i, bufferBarriersCount); fflush(stderr); }
                continue;
            }
        ...
        for (uint32_t i = 0; i < textureBarriersCount; i++) {
            ...
            // CV64 invariant-guard (cont.17 render-crash class): null interface from a corrupt overlay DL
            // -> skip. This is the OBSERVED crash: barriers+0x202 READ 0x10 = interfaceTexture->resourceStates
            // on a null RenderTexture. A null resource has nothing to transition. Deeper root = overlay collision.
            if (interfaceTexture == nullptr) {
                static int _badtb = 0;
                if (_badtb++ < 32) { fprintf(stderr, "[badbarrier] SKIP null TEXTURE barrier i=%u/%u\n", i, textureBarriersCount); fflush(stderr); }
                continue;
            }
...
#   endif
    ...
    D3D12Buffer::D3D12Buffer(D3D12Device *device, D3D12Pool *pool, const RenderBufferDesc &desc) {
        ...
        if (FAILED(res)) {
            // SM64PC S45 boot#6: name the failing BUFFER desc — an E_INVALIDARG here (e.g. size 0 from
            // a corrupt workload) leaves a null mapping that BufferUploader later memcpy's into (the
            // boot#6 process crash). The desc identifies the producer.
            fprintf(stderr, "CreateResource (buffer) failed with error code 0x%lX. size=%llu heapType=%d flags=0x%X committed=%d\n",
                    res, (unsigned long long)desc.size, (int)desc.heapType, desc.flags, desc.committed ? 1 : 0);
            fflush(stderr);
            return;
        }
    }
    ...
    void *D3D12Buffer::map(uint32_t subresource, const RenderRange *readRange) {
        // SM64PC S45 boot#6: a buffer whose CreateResource failed has a null d3d — mapping it must
        // return null (callers check) instead of dereferencing null.
        if (d3d == nullptr) {
            return nullptr;
        }
    ...
    D3D12Texture::D3D12Texture(D3D12Device *device, D3D12Pool *pool, const RenderTextureDesc &desc) {
        ...
        if (FAILED(res)) {
            // SM64PC S45 boot#6: name the failing TEXTURE desc — a degenerate dimension (0-width
            // framebuffer from a corrupt setColorImage) makes E_INVALIDARG here. The desc names it.
            fprintf(stderr, "CreateResource (texture) failed with error code 0x%lX. dim=%d w=%llu h=%u depth/array=%u mips=%u fmt=%d samples=%u flags=0x%X\n",
                    res, (int)desc.dimension, (unsigned long long)desc.width, desc.height,
                    (desc.dimension == RenderTextureDimension::TEXTURE_3D) ? desc.depth : desc.arraySize,
                    desc.mipLevels, (int)desc.format, desc.multisampling.sampleCount, desc.flags);
            fflush(stderr);
            return;
        }
    }
...
#   endif
        ...
        capabilities.dualSrcBlend = true; // D3D12 always supports dual-source blending.
    ...
    std::unique_ptr<RenderBuffer> D3D12Device::createBuffer(const RenderBufferDesc &desc) {
        ...
        // SM64PC S45 boot#6: surface creation failure as a null return (the ctor already logged the
        // desc) so callers can drop the work instead of using a dead buffer (null d3d) later.
