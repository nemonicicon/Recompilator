/**
 * rsp.cpp — RSP microcode dispatch for MP1PC (Mp1 64, USA). Game #4.
 *
 * AUDIO: Mp1 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void mp1_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* mp1_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[mp1pc] Unknown RSP task type %u — no-op stub\n",
