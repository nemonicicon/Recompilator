/**
 * rsp.cpp — RSP microcode dispatch for GEX64PC (Gex64 64, USA). Game #4.
 *
 * AUDIO: Gex64 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void gex64_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* gex64_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[gex64pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = gex64_get_rsp_microcode,
        .capture_gfx = gex64_run_gfx_lle,
    };
}
