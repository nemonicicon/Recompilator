...
float4 clampWrapMirrorSample(RT64_TEX_PARAM const RDPTile rdpTile, const GPUTile gpuTile, float2 tcScale, int2 texelInt, uint tlut, bool gpuTileUsesTMEM, uint mipLevel) {
...
#if defined(RT64_SINGLE_TEXTURE)
    // single-texture mode: the tile's pre-decoded RGBA texture is bound per-draw (no bindless
    // array, no raw-TMEM path — requiresRawTMEM() is false in this mode).
    return stTexture.Load(int3(texelInt, mipLevel));
#else
...
float4 sampleTextureLevel(RT64_TEX_PARAM const RDPTile rdpTile, const GPUTile gpuTile, bool filterBilerp, bool filterAverage, bool linearFiltering, float2 uvCoord, uint tlut, bool canDecodeTMEM, uint mipLevel, bool usesHDR) {
...
#if defined(RT64_SINGLE_TEXTURE)
    bool gpuTileUsesTMEM = false; // pre-decoded RGBA only in this mode
#else
    ...
    if ((nativeSampler == NATIVE_SAMPLER_NONE) || gpuTileUsesTMEM) {
        ...
        for (int i = 0; i < numSamples; i++) {
            samples[i] = clampWrapMirrorSample(RT64_TEX_FWD rdpTile, gpuTile, tcScale, texelBaseInt + int2(i >> 1, i & 1), tlut, gpuTileUsesTMEM, mipLevel);
        }
#else
        samples[0] = clampWrapMirrorSample(RT64_TEX_FWD rdpTile, gpuTile, tcScale, texelBaseInt, tlut, gpuTileUsesTMEM, mipLevel);

        if (filtering) {
            samples[1] = clampWrapMirrorSample(RT64_TEX_FWD rdpTile, gpuTile, tcScale, texelBaseInt + int2(0, 1), tlut, gpuTileUsesTMEM, mipLevel);
            samples[2] = clampWrapMirrorSample(RT64_TEX_FWD rdpTile, gpuTile, tcScale, texelBaseInt + int2(1, 0), tlut, gpuTileUsesTMEM, mipLevel);
            samples[3] = clampWrapMirrorSample(RT64_TEX_FWD rdpTile, gpuTile, tcScale, texelBaseInt + int2(1, 1), tlut, gpuTileUsesTMEM, mipLevel);
        }
#endif
    }
    else {
#if defined(RT64_SINGLE_TEXTURE)
        Texture2D texture = stTexture;
#else
...
float4 sampleTexture(RT64_TEX_PARAM OtherMode otherMode, RenderFlags renderFlags, float2 inputUV, float2 ddxUV, float2 ddyUV, const RDPTile rdpTile, const GPUTile gpuTile, bool nextPixelBug) {
    ...
    for (uint i = 0; i < numRDPSamples; i++) {
        textureSamples[i] = sampleTextureLevel(RT64_TEX_FWD rdpTile, gpuTile, filterBilerp, filterAverage, linearFiltering, uvCoord, tlut, canDecodeTMEM, RDPMipLevels[i], usesHDR);
    }
#else
    textureSamples[0] = sampleTextureLevel(RT64_TEX_FWD rdpTile, gpuTile, filterBilerp, filterAverage, linearFiltering, uvCoord, tlut, canDecodeTMEM, RDPMipLevels[0], usesHDR);

    if (numRDPSamples > 1) {
        textureSamples[1] = sampleTextureLevel(RT64_TEX_FWD rdpTile, gpuTile, filterBilerp, filterAverage, linearFiltering, uvCoord, tlut, canDecodeTMEM, RDPMipLevels[1], usesHDR);
    }
#endif
