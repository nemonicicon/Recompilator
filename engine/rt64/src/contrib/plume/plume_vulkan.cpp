...
#define _CRT_SECURE_NO_WARNINGS // std::getenv for the pipeline-cache path
#endif
...
namespace plume {
    ...
    // --- Persistent VkPipelineCache (disk-backed) -------------------------------------------------
    // Speeds up pipeline creation on every run after the first and is the load-bearing mitigation for
    // GPUs that compile graphics pipelines slowly (e.g. Raspberry Pi V3D). Vulkan-backend only. The
    // driver validates the blob header against the device; we additionally check vendor/device/UUID
    // before trusting it as initial data, and write atomically (temp + rename) so a crash can't corrupt it.
    static std::filesystem::path vulkanPipelineCachePath() {
        std::error_code ec;
        std::filesystem::path dir;
#if defined(_WIN32)
        const char *base = std::getenv("LOCALAPPDATA");
        dir = (base != nullptr) ? std::filesystem::path(base) : std::filesystem::temp_directory_path(ec);
#else
...
#endif
        return dir / "rt64" / "pipeline_cache.bin";
    }

    static VkPipelineCache vulkanCreatePipelineCache(VkDevice device, const VkPhysicalDeviceProperties &properties) {
        std::vector<uint8_t> initialData;
        std::ifstream file(vulkanPipelineCachePath(), std::ios::binary);
        if (file) {
            initialData.assign(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>());
        }

        // Pipeline cache header layout (Vulkan spec): u32 length, u32 version, u32 vendorID, u32 deviceID,
        // u8 pipelineCacheUUID[VK_UUID_SIZE]. Only feed the blob as initial data if it matches THIS device,
        // otherwise a foreign/stale blob risks driver rejection. On mismatch we start an empty cache.
        bool usable = false;
        if (initialData.size() >= 32) {
            uint32_t vendorID = 0;
            uint32_t deviceID = 0;
            std::memcpy(&vendorID, initialData.data() + 8, sizeof(uint32_t));
            std::memcpy(&deviceID, initialData.data() + 12, sizeof(uint32_t));
            usable = (vendorID == properties.vendorID) && (deviceID == properties.deviceID) &&
                (std::memcmp(initialData.data() + 16, properties.pipelineCacheUUID, VK_UUID_SIZE) == 0);
        }

        VkPipelineCacheCreateInfo cacheInfo = {};
        cacheInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
        cacheInfo.initialDataSize = usable ? initialData.size() : 0;
        cacheInfo.pInitialData = usable ? initialData.data() : nullptr;
    ...
    static void vulkanSavePipelineCache(VkDevice device, VkPipelineCache cache) {
        if (cache == VK_NULL_HANDLE) {
            return;
        }

        size_t dataSize = 0;
        if (vkGetPipelineCacheData(device, cache, &dataSize, nullptr) != VK_SUCCESS || dataSize == 0) {
            return;
        }

        std::vector<uint8_t> data(dataSize);
        if (vkGetPipelineCacheData(device, cache, &dataSize, data.data()) != VK_SUCCESS) {
            return;
        }
        ...
        tmp += ".tmp";
        {
            ...
            file.write(reinterpret_cast<const char *>(data.data()), std::streamsize(dataSize));
        }
...
#   endif
    ...
    VulkanDescriptorSetLayout::VulkanDescriptorSetLayout(VulkanDevice *device, const RenderDescriptorSetDesc &descriptorSetDesc) {
        ...
        // Devices without descriptorIndexing (V3D/Pi 4 class) can't accept the boundless binding
        // flags — creating such a layout is invalid API. Drop the flags there: the layout becomes a
        // plain fixed-count layout, legal to create; renderers must not USE boundless sets on such
        // devices (rt64's single-texture mode), this only keeps incidental creation from faulting.
    ...
    VulkanDescriptorSet::VulkanDescriptorSet(VulkanDevice *device, const RenderDescriptorSetDesc &desc) {
        ...
        descriptorPool = createDescriptorPool(device, typeCounts, desc.lastRangeIsBoundless && device->capabilities.descriptorIndexing);
        ...
        // Variable-count allocation requires descriptorIndexing; without it the layout was created
        // as a plain fixed-count layout (see VulkanDescriptorSetLayout) — allocate it plainly too.
...
#   endif
    ...
    uint32_t VulkanSwapChain::getRefreshRate() const {
        // V3D/v3dv guard: the display-timing extension can be absent (or the loader resolves a NULL
        // function pointer) — calling through it segfaults on the Pi. Report "unknown" instead; the
        // caller falls back to its default rate.
        if (!commandQueue->device->capabilities.displayTiming || (vkGetRefreshCycleDurationGOOGLE == nullptr)) {
            return 0;
        }
...
#       endif
        ...
        // Create the persistent pipeline cache as early as possible so every pipeline creation benefits.
        pipelineCache = vulkanCreatePipelineCache(vk, physicalDeviceProperties);
...
#   endif
    ...
    void VulkanDevice::release() {
        ...
        // Persist the pipeline cache to disk before tearing down the device. Worker threads that create
        // pipelines have already been joined by this point (the shader cache is destroyed before the
        // device), so this read is single-threaded.
        if (pipelineCache != VK_NULL_HANDLE) {
            vulkanSavePipelineCache(vk, pipelineCache);
