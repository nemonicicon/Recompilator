/**
 * main.cpp — Micro Machines 64 Turbo PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Micro Machines 64 Turbo game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 9672fa010a06adf01074c9720ee46031f5d9e101 · internal name "Micro Machines 64 Turbo"
static constexpr uint64_t MICROMACHINES64TUR_ROM_HASH        = 0xB74F82D15363B6E2ULL;
static constexpr uint32_t MICROMACHINES64TUR_ENTRYPOINT_VRAM = 0x80025C00;
...
static void micromachines64tur_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        micromachines64tur_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Micro Machines 64 Turbo PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        micromachines64tur_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Micro Machines 64 Turbo PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void micromachines64tur_message_box(const char* msg) {
    micromachines64tur_show_error("Micro Machines 64 Turbo — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI micromachines64tur_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in micromachines64turpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in micromachines64turpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Micro Machines 64 Turbo — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void micromachines64tur_vi_callback() {}
static void micromachines64tur_gfx_init_callback() {}

static std::string micromachines64tur_get_thread_name(const OSThread* t) {
    return "micromachines64tur_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Micro Machines 64 Turbo ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            micromachines64tur_show_error("ROM Required",
                "A Micro Machines 64 Turbo ROM is required.\n"
    ...
    // MICROMACHINES64TUR_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("MICROMACHINES64TUR_ROM");
    if (!rom_hint || !*rom_hint) {
        micromachines64tur_show_error("ROM Required",
            "Pass the Micro Machines 64 Turbo ROM path as the first argument,\n"
            "or set the MICROMACHINES64TUR_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"micromachines64tur_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        micromachines64tur_show_error("Wrong ROM",
            "The selected ROM does not match Micro Machines 64 Turbo.\n"
            "Expected SHA1: 9672fa010a06adf01074c9720ee46031f5d9e101");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        micromachines64tur_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        micromachines64tur_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int micromachines64tur_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(micromachines64tur_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "micromachines64turpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "micromachines64turpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "micromachines64turpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[micromachines64turpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "micromachines64turpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[micromachines64turpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = MICROMACHINES64TUR_ROM_HASH;
    entry.internal_name      = "Micro Machines 64 Turbo";
    entry.game_id            = u8"micromachines64tur_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = MICROMACHINES64TUR_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"micromachines64tur_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            micromachines64tur_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    micromachines64tur_audio_init();
    micromachines64tur_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = micromachines64tur::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = micromachines64tur_vi_callback,
        .gfx_init_callback = micromachines64tur_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = micromachines64tur_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = micromachines64tur_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return micromachines64tur_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return micromachines64tur_app_main(argc, argv);
}
#endif
