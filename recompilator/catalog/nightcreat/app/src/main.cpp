...
static constexpr uint64_t NIGHTCREAT_ROM_HASH        = 0x260851F60FE541D8ULL;
static constexpr uint32_t NIGHTCREAT_ENTRYPOINT_VRAM = 0x80000400;
...
static void nightcreat_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        nightcreat_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        nightcreat_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void nightcreat_message_box(const char* msg) {
    nightcreat_show_error("Nightmare Creatures — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI nightcreat_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in nightcreatpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in nightcreatpc.exe (base=0x%p), nearest first:\n",
...
static void nightcreat_vi_callback() {}
static void nightcreat_gfx_init_callback() {}

static std::string nightcreat_get_thread_name(const OSThread* t) {
    return "nightcreat_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        if (!GetOpenFileNameA(&ofn)) {
            nightcreat_show_error("ROM Required",
    ...
    // NIGHTCREAT_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("NIGHTCREAT_ROM");
    if (!rom_hint || !*rom_hint) {
        nightcreat_show_error("ROM Required",
            "Pass the Nightmare Creatures ROM path as the first argument,\n"
            "or set the NIGHTCREAT_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"nightcreat_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        nightcreat_show_error("Wrong ROM",
            "The selected ROM does not match Nightmare Creatures.\n"
            "Expected SHA1: bea6c21b3ef5950bdf8cdb255955da6c4746eea8");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        nightcreat_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        nightcreat_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int nightcreat_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(nightcreat_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "nightcreatpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "nightcreatpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "nightcreatpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[nightcreatpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "nightcreatpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[nightcreatpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = NIGHTCREAT_ROM_HASH;
    entry.internal_name      = "Nightmare Creatures";
    entry.game_id            = u8"nightcreat_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = NIGHTCREAT_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"nightcreat_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            nightcreat_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    nightcreat_audio_init();
    nightcreat_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = nightcreat::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = nightcreat_vi_callback,
        .gfx_init_callback = nightcreat_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = nightcreat_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = nightcreat_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return nightcreat_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return nightcreat_app_main(argc, argv);
}
#endif
