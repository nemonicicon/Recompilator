/**
 * main.cpp — Lode Runner 3-D PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Lode Runner 3-D game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: d3a13c0cfdff835fdf87d5dc7c5149fba564877f · internal name "Lode Runner 3-D"
static constexpr uint64_t LODERUNNER3D_ROM_HASH        = 0x0A6CD389087117A6ULL;
static constexpr uint32_t LODERUNNER3D_ENTRYPOINT_VRAM = 0x80096400;
...
static void loderunner3d_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        loderunner3d_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        loderunner3d_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Lode Runner 3-D PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void loderunner3d_message_box(const char* msg) {
    loderunner3d_show_error("Lode Runner 3-D — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI loderunner3d_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in loderunner3dpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in loderunner3dpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Lode Runner 3-D — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void loderunner3d_vi_callback() {}
static void loderunner3d_gfx_init_callback() {}

static std::string loderunner3d_get_thread_name(const OSThread* t) {
    return "loderunner3d_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Lode Runner 3-D ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            loderunner3d_show_error("ROM Required",
                "A Lode Runner 3-D ROM is required.\n"
    ...
    // LODERUNNER3D_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("LODERUNNER3D_ROM");
    if (!rom_hint || !*rom_hint) {
        loderunner3d_show_error("ROM Required",
            "Pass the Lode Runner 3-D ROM path as the first argument,\n"
            "or set the LODERUNNER3D_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"loderunner3d_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        loderunner3d_show_error("Wrong ROM",
            "The selected ROM does not match Lode Runner 3-D.\n"
            "Expected SHA1: d3a13c0cfdff835fdf87d5dc7c5149fba564877f");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        loderunner3d_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        loderunner3d_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int loderunner3d_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(loderunner3d_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "loderunner3dpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "loderunner3dpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "loderunner3dpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[loderunner3dpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "loderunner3dpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[loderunner3dpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = LODERUNNER3D_ROM_HASH;
    entry.internal_name      = "Lode Runner 3-D";
    entry.game_id            = u8"loderunner3d_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = LODERUNNER3D_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"loderunner3d_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            loderunner3d_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    loderunner3d_audio_init();
    loderunner3d_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = loderunner3d::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = loderunner3d_vi_callback,
        .gfx_init_callback = loderunner3d_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = loderunner3d_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = loderunner3d_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return loderunner3d_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return loderunner3d_app_main(argc, argv);
}
#endif
