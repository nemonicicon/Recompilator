/**
 * main.cpp — Chameleon Twist (Rev 1) PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Chameleon Twist (Rev 1) game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 4ca6d563131b4809fe1748335182816a024999d4 · internal name "Chameleon Twist (Rev 1)"
static constexpr uint64_t CHAMELEONTWIST_REV1_ROM_HASH        = 0xCDC00B191D4F1862ULL;
static constexpr uint32_t CHAMELEONTWIST_REV1_ENTRYPOINT_VRAM = 0x80025C00;
...
static void chameleontwist_rev1_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        chameleontwist_rev1_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Chameleon Twist (Rev 1) PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        chameleontwist_rev1_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Chameleon Twist (Rev 1) PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void chameleontwist_rev1_message_box(const char* msg) {
    chameleontwist_rev1_show_error("Chameleon Twist (Rev 1) — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI chameleontwist_rev1_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in chameleontwist_rev1pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in chameleontwist_rev1pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Chameleon Twist (Rev 1) — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void chameleontwist_rev1_vi_callback() {}
static void chameleontwist_rev1_gfx_init_callback() {}

static std::string chameleontwist_rev1_get_thread_name(const OSThread* t) {
    return "chameleontwist_rev1_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Chameleon Twist (Rev 1) ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            chameleontwist_rev1_show_error("ROM Required",
                "A Chameleon Twist (Rev 1) ROM is required.\n"
    ...
    // CHAMELEONTWIST_REV1_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("CHAMELEONTWIST_REV1_ROM");
    if (!rom_hint || !*rom_hint) {
        chameleontwist_rev1_show_error("ROM Required",
            "Pass the Chameleon Twist (Rev 1) ROM path as the first argument,\n"
            "or set the CHAMELEONTWIST_REV1_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"chameleontwist_rev1_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        chameleontwist_rev1_show_error("Wrong ROM",
            "The selected ROM does not match Chameleon Twist (Rev 1).\n"
            "Expected SHA1: 4ca6d563131b4809fe1748335182816a024999d4");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        chameleontwist_rev1_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        chameleontwist_rev1_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int chameleontwist_rev1_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(chameleontwist_rev1_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "chameleontwist_rev1pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "chameleontwist_rev1pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "chameleontwist_rev1pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[chameleontwist_rev1pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "chameleontwist_rev1pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[chameleontwist_rev1pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = CHAMELEONTWIST_REV1_ROM_HASH;
    entry.internal_name      = "Chameleon Twist (Rev 1)";
    entry.game_id            = u8"chameleontwist_rev1_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = CHAMELEONTWIST_REV1_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"chameleontwist_rev1_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            chameleontwist_rev1_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    chameleontwist_rev1_audio_init();
    chameleontwist_rev1_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = chameleontwist_rev1::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = chameleontwist_rev1_vi_callback,
        .gfx_init_callback = chameleontwist_rev1_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = chameleontwist_rev1_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = chameleontwist_rev1_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return chameleontwist_rev1_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return chameleontwist_rev1_app_main(argc, argv);
}
#endif
