/**
 * main.cpp — pilotwings PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
#endif
// console seam ported from snowkidspc (2026-07-31 batch): the shared organ replaces the
// per-game sm64::lleconsole copy (moved aside as src/lle_console.*.pre_organ_20260731).
...
// ── pilotwings game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: (unknown) · internal name "PILOTWINGS"
// XXH3 recomputed from pilotwings/baserom.z64 2026-07-01 (the 6/18 scaffold regen wrote a 0x0
// placeholder here -> runtime "Wrong ROM" modal -> the game never booted in two sweeps).
static constexpr uint64_t PILOTWINGS_ROM_HASH        = 0x887AB02583C90111ULL;
static constexpr uint32_t PILOTWINGS_ENTRYPOINT_VRAM = 0x80200050;   // per pilotwings.toml (PIF-relocated boot)
...
static void pilotwings_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
// console seam ported from snowkidspc (2026-07-31 batch)
    ...
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        pilotwings_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
// console seam ported from snowkidspc (2026-07-31 batch)
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        pilotwings_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    // console seam ported from snowkidspc (2026-07-31 batch): organ namespace.
...
#endif
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "pilotwings PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void pilotwings_message_box(const char* msg) {
    pilotwings_show_error("pilotwings — Error", msg);
}

// console seam ported from snowkidspc (2026-07-31 batch)
static std::unique_ptr<ultramodern::renderer::RendererContext>
pilotwings_create_render_context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode) {
#ifdef RDPC_CAPTURE_CHAIN
    ...
    if (preservation_core::lleconsole::active()) {
        ...
        extern void pilotwings_lle_console_submit();
        preservation_core::lleconsole::set_submit_callback(&pilotwings_lle_console_submit);
    ...
    pilotwings_show_error("Renderer", "Console-only build: set RDPC_LLE_CONSOLE=1.");
    std::exit(1);
#else
    return pilotwings::renderer::create_render_context(rdram, window_handle, developer_mode);
#endif
}

#if defined(_WIN32)
static LONG WINAPI pilotwings_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in pilotwingspc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in pilotwingspc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "pilotwings — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void pilotwings_vi_callback() {}
static void pilotwings_gfx_init_callback() {}

static std::string pilotwings_get_thread_name(const OSThread* t) {
    return "pilotwings_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select pilotwings ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            pilotwings_show_error("ROM Required",
                "A pilotwings ROM is required.\n"
    ...
    // PILOTWINGS_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("PILOTWINGS_ROM");
    if (!rom_hint || !*rom_hint) {
        pilotwings_show_error("ROM Required",
            "Pass the pilotwings ROM path as the first argument,\n"
            "or set the PILOTWINGS_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"pilotwings_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        pilotwings_show_error("Wrong ROM",
            "The selected ROM does not match pilotwings.\n"
            "Expected SHA1: (unknown)");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        pilotwings_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        pilotwings_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int pilotwings_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(pilotwings_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "pilotwingspc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "pilotwingspc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "pilotwingspc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[pilotwingspc] diagnostic log started\n"); }
        recomp::diag_log_cap(log_path.string());   // console seam ported from snowkidspc (2026-07-31 batch)
        std::filesystem::path stdout_path = config_path / "pilotwingspc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[pilotwingspc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = PILOTWINGS_ROM_HASH;
    entry.internal_name      = "PILOTWINGS";
    entry.game_id            = u8"pilotwings_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = PILOTWINGS_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"pilotwings_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            pilotwings_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    pilotwings_audio_init();
    pilotwings_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = pilotwings_create_render_context };   // console seam ported from snowkidspc (2026-07-31 batch)
    ...
    cfg.events_callbacks     = {
        .vi_callback       = pilotwings_vi_callback,
        .gfx_init_callback = pilotwings_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = pilotwings_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = pilotwings_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return pilotwings_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return pilotwings_app_main(argc, argv);
}
#endif
