/**
 * rsp.cpp — RSP microcode dispatch for NBAZONE98PC (Nbazone98 64, USA). Game #4.
 *
 * AUDIO: Nbazone98 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nbazone98_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nbazone98_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nbazone98pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nbazone98_get_rsp_microcode,
        .capture_gfx = nbazone98_run_gfx_lle,
    };
}
