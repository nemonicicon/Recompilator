/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for SUPERBOWLINGPC.
 *
...
void superbowling_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in superbowling_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool superbowling_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void superbowling_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t superbowling_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  superbowling_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = superbowling_poll_input,
        .get_input               = superbowling_get_input,
        .set_rumble              = superbowling_set_rumble,
        .get_connected_device_info = superbowling_get_connected_device,
    };
}
