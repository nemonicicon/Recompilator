...
// Render-true DP-done hook (SOTE shimmer, 2026-07-19): the runtime installs a callback that
// delivers the guest's DP interrupt at ACTUAL workload-render completion — the hardware meaning
// of DP-done (the RDP has finished writing the framebuffer) — instead of a floor offset from
// submission. Null when no runtime registered it; called on the workload thread, so the callback
// must be cheap and lock-leaf only.
extern "C" void (*rt64_workload_rendered_hook)(void) = nullptr;
...
namespace RT64 {
    ...
    void WorkloadQueue::threadConfigurationUpdate(hlslpp::uint2 viFbSize, WorkloadConfiguration &workloadConfig) {
        ...
        auto refreshRate = ext.sharedResources->userConfig.refreshRate;
        // [interp test] RECOMP_INTERP=1 forces Display refresh = enable RT64's frame interpolation
        // (smooth display at the swapchain rate, interpolating from the game's native viOriginalRate).
        // The app's GraphicsConfig.rr_option isn't bridged to this UserConfiguration yet; this proves the
        // path before wiring it. Keeps the governor pacing logic faithfully -> interp just smooths the picture.
        static const bool _force_interp = (std::getenv("RECOMP_INTERP") != nullptr);
        if (_force_interp) refreshRate = UserConfiguration::RefreshRate::Display;
        switch (refreshRate) {
        ...
        while (threadsRunning) {
            ...
            if (processCursor >= 0) {
                ...
                if (generateInterpolatedFrames) {
                    ...
                    // HOLD THE TWO INVARIANTS IN RELEASE TOO. Both were asserts only (just above),
                    // and an assert is compiled out of the build people actually run. displayFrames
                    // then feeds two lines that cannot survive a bad value: (displayFrames - 1) on an
                    // unsigned, handed straight to interpolatedTargets.resize(), and a division by
                    // displayFrames. A tick gap under one frame makes it 0, so the subtraction is
                    // 4,294,967,295 and the resize asks for 34 GB of unique_ptr<RenderTarget>; a gap
                    // that has gone NEGATIVE (displayTicks overshooting logicalTicks - these are
                    // int64_t) divides to a negative and casts to the same class of number. Either
                    // takes the process heap down (STATUS_HEAP_CORRUPTION 0xC0000374).
                    //
                    // Only reachable at RefreshRate::Display, the one mode that turns interpolation
                    // on, and only when the display is FASTER than the game (displayRateAboveOriginal)
                    // - so it never fires on a 60 Hz panel and does on a 99 Hz one. It was found by
                    // setting refresh rate to Display and losing the program twice, 2026-09-07.
                    const int64_t tickGap = logicalTicks - displayTicks;
                    const int64_t framesFromGap = (tickGap > 0) ? (tickGap / int64_t(workload.viOriginalRate)) : 0;
                    // The ceiling is the accounting's own bound: one logical frame can never be worth
                    // more than targetRate/viOriginalRate display frames, plus one for the remainder.
                    const int64_t framesCeiling = int64_t(workloadConfig.targetRate) / int64_t(workload.viOriginalRate) + 1;
                    displayFrames = uint32_t(std::max<int64_t>(1, std::min<int64_t>(framesFromGap, framesCeiling)));
                ...
                // displayFrames is >= 1 on both paths above, but this subtraction is the line that
                // detonates if that ever stops being true, so it does not rely on it.
                uint32_t requiredFrames = (usingMSAA && generateInterpolatedFrames) ? displayFrames : ((displayFrames > 0) ? (displayFrames - 1) : 0);
                ...
                for (uint32_t frame = 0; (frame < displayFrames) && !skipWorkloadNow; frame++) {
                    ...
                    if (frame == 0) {
                        ...
                        // Render-true DP-done: this is the moment the "RDP" genuinely finished
                        // writing this workload's framebuffer (see the hook comment at file top).
                        if (rt64_workload_rendered_hook != nullptr) {
                            rt64_workload_rendered_hook();
                        }
                    }
