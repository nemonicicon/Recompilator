/**
 * rsp.cpp — RSP microcode dispatch for EXTREMEGXG2PC (Extremegxg2 64, USA). Game #4.
 *
 * AUDIO: Extremegxg2 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void extremegxg2_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* extremegxg2_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[extremegxg2pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = extremegxg2_get_rsp_microcode,
        .capture_gfx = extremegxg2_run_gfx_lle,
    };
}
