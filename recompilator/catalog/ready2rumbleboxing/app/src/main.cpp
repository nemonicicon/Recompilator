/**
 * main.cpp — Ready 2 Rumble Boxing PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Ready 2 Rumble Boxing game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: e60fedd535a72fda5c424b846421bbdad17c6caa · internal name "Ready 2 Rumble Boxing"
static constexpr uint64_t READY2RUMBLEBOXING_ROM_HASH        = 0x3E334DC31FCDD402ULL;
static constexpr uint32_t READY2RUMBLEBOXING_ENTRYPOINT_VRAM = 0x80029400;
...
static void ready2rumbleboxing_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        ready2rumbleboxing_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Ready 2 Rumble Boxing PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        ready2rumbleboxing_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Ready 2 Rumble Boxing PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void ready2rumbleboxing_message_box(const char* msg) {
    ready2rumbleboxing_show_error("Ready 2 Rumble Boxing — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI ready2rumbleboxing_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in ready2rumbleboxingpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in ready2rumbleboxingpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Ready 2 Rumble Boxing — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void ready2rumbleboxing_vi_callback() {}
static void ready2rumbleboxing_gfx_init_callback() {}

static std::string ready2rumbleboxing_get_thread_name(const OSThread* t) {
    return "ready2rumbleboxing_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Ready 2 Rumble Boxing ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            ready2rumbleboxing_show_error("ROM Required",
                "A Ready 2 Rumble Boxing ROM is required.\n"
    ...
    // READY2RUMBLEBOXING_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("READY2RUMBLEBOXING_ROM");
    if (!rom_hint || !*rom_hint) {
        ready2rumbleboxing_show_error("ROM Required",
            "Pass the Ready 2 Rumble Boxing ROM path as the first argument,\n"
            "or set the READY2RUMBLEBOXING_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"ready2rumbleboxing_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        ready2rumbleboxing_show_error("Wrong ROM",
            "The selected ROM does not match Ready 2 Rumble Boxing.\n"
            "Expected SHA1: e60fedd535a72fda5c424b846421bbdad17c6caa");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        ready2rumbleboxing_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        ready2rumbleboxing_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int ready2rumbleboxing_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(ready2rumbleboxing_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "ready2rumbleboxingpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "ready2rumbleboxingpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "ready2rumbleboxingpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[ready2rumbleboxingpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "ready2rumbleboxingpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[ready2rumbleboxingpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = READY2RUMBLEBOXING_ROM_HASH;
    entry.internal_name      = "Ready 2 Rumble Boxing";
    entry.game_id            = u8"ready2rumbleboxing_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = READY2RUMBLEBOXING_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"ready2rumbleboxing_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            ready2rumbleboxing_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    ready2rumbleboxing_audio_init();
    ready2rumbleboxing_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = ready2rumbleboxing::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = ready2rumbleboxing_vi_callback,
        .gfx_init_callback = ready2rumbleboxing_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = ready2rumbleboxing_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = ready2rumbleboxing_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return ready2rumbleboxing_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return ready2rumbleboxing_app_main(argc, argv);
}
#endif
