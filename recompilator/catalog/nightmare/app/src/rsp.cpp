/**
 * rsp.cpp — RSP microcode dispatch for NIGHTMAREPC (Nightmare 64, USA). Console seam ported
 * from snowkidspc (2026-07-31 batch, starfox64pc port pattern).
 *
 * AUDIO: Nightmare ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
 ...
 * GFX: desktop tier HLE-rendered by RT64 via send_dl; console tier LLE via the cv64-family
 * F3DEX fifo 2.06 artifact reused verbatim (rsp/f3dex2.cpp).
 */
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
// ── LLE CONSOLE (console seam ported from snowkidspc, 2026-07-31 batch; artifact per the
// asteroidspc precedent) ─────────────────────────────────────────────────────────────────────
// gfx = rsp/f3dex2.cpp: the cv64-family F3DEX fifo 2.06 artifact REUSED VERBATIM — Nightmare
// Creatures ships the SAME byte-build (125671d5b05e; BUILD_CENSUS: 1 build, 24 games incl.
// every Castlevania and Asteroids), so the 64/64 bit-exact judgment (f3dex2_ab, cv64 capture
// bank) carries whole. Registered under cv64's organ constants (identical bytes = identical
// key). Census: F3DEX fifo 2.06 is Nightmare Creatures' ONLY gfx ucode family.
RspExitReason f3dex2(uint8_t* rdram, uint32_t ucode_addr);

static RspUcodeFunc* nightmare_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
    RspUcodeFunc* fn = recomp::rsp::dispatch_ucode(rdram, task);
    return fn != nullptr ? fn : f3dex2;
}

// Desktop/HLE tier: no LLE capture (RT64 renders via send_dl).
static void nightmare_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nightmare_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry/artifact
        // serve via nightmare_run_gfx_capture; this documents the mapping and keeps the artifact linked.
        return f3dex2;
    default:
        fprintf(stderr, "[nightmarepc] Unknown RSP task type %u — no-op stub\n",
...
// gfx ucode (nightmare_recomp, f3dex2.cpp) runs per gfx task; DO_DP_END feeds the librecomp
...
static void nightmare_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void nightmare_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in nightmare_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
void nightmare_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    nightmare_lle_render(rdram, job, n);
                }
...
static void nightmare_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled gfx ucode (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = nightmare_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            nightmare_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            nightmare_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) nightmare_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/nightmare_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=nightmare tree=N64PC src=f3dex2-LLE(RSPRecomp) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    ...
    // offline, SERVED_ROSTER rung, ENGINE_LEDGER 07-24). Nightmare ships the byte-identical
    ...
    // gfx: the cv64-family artifact under cv64's organ-bootstrap constants (identical build;
    // asteroidspc registration copied verbatim). Key hit + verify mismatch = safe MISS to the
    // direct f3dex2 fallback — mis-dispatch impossible.
    recomp::rsp::register_ucode(0xAA49D4D253D339CFull, "f3dex2", f3dex2,
                                0x8C0, 0x40, 0x257B1C35D63E35CDull);
    return {
        .get_rsp_microcode = nightmare_get_rsp_microcode,
        // Console tier (-DRDPC_CAPTURE=ON): the preservation-core chain — recompiled F3DEX2
        // via the registry -> rdpcore -> console VI. Desktop tier: inert no-op alongside RT64.
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = nightmare_run_gfx_capture,
#else
        .capture_gfx = nightmare_run_gfx_lle,
#endif
    };
}
