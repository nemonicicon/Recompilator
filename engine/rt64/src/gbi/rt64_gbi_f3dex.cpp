...
namespace RT64 {
    namespace GBI_F3DEX {
        ...
        void loadUCode(State *state, DisplayList **dl) {
            // cv64 SESSION 37: in-DL microcode switch (gSPLoadUcode) — log the requested text (w1) and
            // data (preceding G_RDPHALF_1) addresses; pairs with [gbiload]/[gbifail] in the interpreter.
            static int _luc = 0;
            if (_luc++ < 64) {
                fprintf(stderr, "[gbi_loaducode] G_LOAD_UCODE w1=0x%08X half1=0x%08X\n",
                        (*dl)->w1, state->microcode.half1);
                fflush(stderr);
            }
