...
namespace plume {
    ...
    struct D3D12DescriptorHeapAllocator {
        ...
        uint32_t lowSpaceMark = 0;        // cv64 S38: highest occupancy threshold logged (leak-vs-workingset telemetry)
        D3D12_DESCRIPTOR_HEAP_TYPE allocatorHeapType = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;   // cv64 S38: for log labels
