/**
 * main.cpp — F-1 World Grand Prix PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── F-1 World Grand Prix game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: dd17545adc8fe2af2a713ab48c5f71801222edaf · internal name "F-1 World Grand Prix"
static constexpr uint64_t F1WGP_ROM_HASH        = 0x8022946728878263ULL;
static constexpr uint32_t F1WGP_ENTRYPOINT_VRAM = 0x80000400;
...
static void f1wgp_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        f1wgp_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "F-1 World Grand Prix PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        f1wgp_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "F-1 World Grand Prix PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void f1wgp_message_box(const char* msg) {
    f1wgp_show_error("F-1 World Grand Prix — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI f1wgp_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in f1wgppc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in f1wgppc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "F-1 World Grand Prix — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void f1wgp_vi_callback() {}
static void f1wgp_gfx_init_callback() {}

static std::string f1wgp_get_thread_name(const OSThread* t) {
    return "f1wgp_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select F-1 World Grand Prix ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            f1wgp_show_error("ROM Required",
                "A F-1 World Grand Prix ROM is required.\n"
    ...
    // F1WGP_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("F1WGP_ROM");
    if (!rom_hint || !*rom_hint) {
        f1wgp_show_error("ROM Required",
            "Pass the F-1 World Grand Prix ROM path as the first argument,\n"
            "or set the F1WGP_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"f1wgp_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        f1wgp_show_error("Wrong ROM",
            "The selected ROM does not match F-1 World Grand Prix.\n"
            "Expected SHA1: dd17545adc8fe2af2a713ab48c5f71801222edaf");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        f1wgp_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        f1wgp_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int f1wgp_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(f1wgp_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "f1wgppc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "f1wgppc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "f1wgppc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[f1wgppc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "f1wgppc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[f1wgppc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = F1WGP_ROM_HASH;
    entry.internal_name      = "F-1 World Grand Prix";
    entry.game_id            = u8"f1wgp_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = F1WGP_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"f1wgp_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            f1wgp_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = f1wgp::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = f1wgp_vi_callback,
        .gfx_init_callback = f1wgp_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = f1wgp_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = f1wgp_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return f1wgp_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return f1wgp_app_main(argc, argv);
}
#endif
