 ...
 *     engine. Keyed to CV64 boot conditions; must never fire for GT64.
 ...
 *     correct for static-only GT64 (overlay recompilation is PHASE 3).
...
/* 1 — CV64 NI boot-stub hook; loudly visible if it ever fires for GT64. */
RECOMP_FUNC void gamestate_change(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    fprintf(stderr, "[engine_shim] gamestate_change(%d) called — CV64 NI boot-stub path fired for GT64?!\n",
            (int)ctx->r4);
    fflush(stderr);
}
