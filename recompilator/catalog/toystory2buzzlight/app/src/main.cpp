/**
 * main.cpp — Toy Story 2 - Buzz Lightyear to the Rescue! PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Toy Story 2 - Buzz Lightyear to the Rescue! game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 982ad2e1e44c6662c88a77367bc5df91c51531bf · internal name "Toy Story 2 - Buzz Lightyear to the Rescue!"
static constexpr uint64_t TOYSTORY2BUZZLIGHT_ROM_HASH        = 0xFD53C5EC8CE2530AULL;
static constexpr uint32_t TOYSTORY2BUZZLIGHT_ENTRYPOINT_VRAM = 0x80006000;
...
static void toystory2buzzlight_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        toystory2buzzlight_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        toystory2buzzlight_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Toy Story 2 - Buzz Lightyear to the Rescue! PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void toystory2buzzlight_message_box(const char* msg) {
    toystory2buzzlight_show_error("Toy Story 2 - Buzz Lightyear to the Rescue! — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI toystory2buzzlight_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in toystory2buzzlightpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in toystory2buzzlightpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Toy Story 2 - Buzz Lightyear to the Rescue! — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void toystory2buzzlight_vi_callback() {}
static void toystory2buzzlight_gfx_init_callback() {}

static std::string toystory2buzzlight_get_thread_name(const OSThread* t) {
    return "toystory2buzzlight_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Toy Story 2 - Buzz Lightyear to the Rescue! ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            toystory2buzzlight_show_error("ROM Required",
                "A Toy Story 2 - Buzz Lightyear to the Rescue! ROM is required.\n"
    ...
    // TOYSTORY2BUZZLIGHT_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("TOYSTORY2BUZZLIGHT_ROM");
    if (!rom_hint || !*rom_hint) {
        toystory2buzzlight_show_error("ROM Required",
            "Pass the Toy Story 2 - Buzz Lightyear to the Rescue! ROM path as the first argument,\n"
            "or set the TOYSTORY2BUZZLIGHT_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"toystory2buzzlight_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        toystory2buzzlight_show_error("Wrong ROM",
            "The selected ROM does not match Toy Story 2 - Buzz Lightyear to the Rescue!.\n"
            "Expected SHA1: 982ad2e1e44c6662c88a77367bc5df91c51531bf");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        toystory2buzzlight_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        toystory2buzzlight_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int toystory2buzzlight_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(toystory2buzzlight_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "toystory2buzzlightpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "toystory2buzzlightpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "toystory2buzzlightpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[toystory2buzzlightpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "toystory2buzzlightpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[toystory2buzzlightpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = TOYSTORY2BUZZLIGHT_ROM_HASH;
    entry.internal_name      = "Toy Story 2 - Buzz Lightyear to the Rescue!";
    entry.game_id            = u8"toystory2buzzlight_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = TOYSTORY2BUZZLIGHT_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"toystory2buzzlight_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            toystory2buzzlight_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    toystory2buzzlight_audio_init();
    toystory2buzzlight_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = toystory2buzzlight::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = toystory2buzzlight_vi_callback,
        .gfx_init_callback = toystory2buzzlight_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = toystory2buzzlight_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = toystory2buzzlight_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return toystory2buzzlight_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return toystory2buzzlight_app_main(argc, argv);
}
#endif
