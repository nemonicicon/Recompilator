/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for EXTREMEGXG2PC.
 *
...
void extremegxg2_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in extremegxg2_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool extremegxg2_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void extremegxg2_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t extremegxg2_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  extremegxg2_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = extremegxg2_poll_input,
        .get_input               = extremegxg2_get_input,
        .set_rumble              = extremegxg2_set_rumble,
        .get_connected_device_info = extremegxg2_get_connected_device,
    };
}
