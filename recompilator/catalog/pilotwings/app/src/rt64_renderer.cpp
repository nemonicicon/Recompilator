/**
 * rt64_renderer.cpp — RT64Context implementation for PILOTWINGSPC.
 *
 * Same wrapper as cv64pc's (the engine-proven shape), minus the CV64-specific
 * DL-dump probes. PILOTWINGS uses the same F3DEX2 fifo 2.06 microcode as CV64.
 */
...
// console seam ported from snowkidspc (2026-07-31 batch): the console switch moved to
// main.cpp's pilotwings_create_render_context wrapper (organ Move 1); this file is now
// RT64-only and is excluded entirely from RDPC_CONSOLE_ONLY builds.
                         ...
                         bool developer_mode)
{
...
#else
    // Engine drift (renderer rework): WindowHandle (SDL_Window*) no longer assignable to the plume
    // RenderWindow. On the console path RT64 is unused, so a default window is fine. (2026-07-14)
    core.window = {};
#endif
    ...
    appConfig.appId = "pilotwingspc";

    app_ = std::make_unique<RT64::Application>(core, appConfig);

    // NOTE: enhancementConfig.f3dex.forceBranch intentionally left at the
...
create_render_context(uint8_t* rdram,
                      ...
                      bool developer_mode)
{
    // console seam ported from snowkidspc (2026-07-31 batch): the old in-renderer console
    // switch (sm64::lleconsole, the copied file) is gone — main.cpp's wrapper intercepts
    // console mode before this function is ever reached.
