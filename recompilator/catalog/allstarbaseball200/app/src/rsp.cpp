/**
 * rsp.cpp — RSP microcode dispatch for ALLSTARBASEBALL200PC (Allstarbaseball200 64, USA). Game #4.
 *
 * AUDIO: Allstarbaseball200 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void allstarbaseball200_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* allstarbaseball200_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[allstarbaseball200pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = allstarbaseball200_get_rsp_microcode,
        .capture_gfx = allstarbaseball200_run_gfx_lle,
    };
}
