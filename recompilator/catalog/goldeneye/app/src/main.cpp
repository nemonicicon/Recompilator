/**
 * main.cpp — GoldenEye 007 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── GoldenEye 007 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: abe01e4aeb033b6c0836819f549c791b26cfde83 · internal name "GoldenEye 007"
static constexpr uint64_t GOLDENEYE_ROM_HASH        = 0x639ECE0BC88C6E4AULL;
static constexpr uint32_t GOLDENEYE_ENTRYPOINT_VRAM = 0x80000400;
...
static void goldeneye_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        goldeneye_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        goldeneye_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "GoldenEye 007 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void goldeneye_message_box(const char* msg) {
    goldeneye_show_error("GoldenEye 007 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI goldeneye_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in goldeneyepc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in goldeneyepc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "GoldenEye 007 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void goldeneye_vi_callback() {}
static void goldeneye_gfx_init_callback() {}

static std::string goldeneye_get_thread_name(const OSThread* t) {
    return "goldeneye_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select GoldenEye 007 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            goldeneye_show_error("ROM Required",
                "A GoldenEye 007 ROM is required.\n"
    ...
    // GOLDENEYE_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("GOLDENEYE_ROM");
    if (!rom_hint || !*rom_hint) {
        goldeneye_show_error("ROM Required",
            "Pass the GoldenEye 007 ROM path as the first argument,\n"
            "or set the GOLDENEYE_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"goldeneye_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        goldeneye_show_error("Wrong ROM",
            "The selected ROM does not match GoldenEye 007.\n"
            "Expected SHA1: abe01e4aeb033b6c0836819f549c791b26cfde83");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        goldeneye_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        goldeneye_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int goldeneye_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(goldeneye_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "goldeneyepc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "goldeneyepc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "goldeneyepc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[goldeneyepc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "goldeneyepc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[goldeneyepc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = GOLDENEYE_ROM_HASH;
    entry.internal_name      = "GoldenEye 007";
    entry.game_id            = u8"goldeneye_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = GOLDENEYE_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"goldeneye_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            goldeneye_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    goldeneye_audio_init();
    goldeneye_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = goldeneye::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = goldeneye_vi_callback,
        .gfx_init_callback = goldeneye_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = goldeneye_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = goldeneye_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return goldeneye_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return goldeneye_app_main(argc, argv);
}
#endif
