/**
 * rsp.cpp — RSP microcode dispatch for SUPERSMASHBROSPC (Supersmashbros 64, USA). Game #4.
 *
 * AUDIO: Supersmashbros ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void supersmashbros_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* supersmashbros_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[supersmashbrospc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = supersmashbros_get_rsp_microcode,
        .capture_gfx = supersmashbros_run_gfx_lle,
    };
}
