/**
 * rsp.cpp — RSP microcode dispatch for NBALIVE99PC (Nbalive99 64, USA). Game #4.
 *
 * AUDIO: Nbalive99 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nbalive99_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nbalive99_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nbalive99pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nbalive99_get_rsp_microcode,
        .capture_gfx = nbalive99_run_gfx_lle,
    };
}
