/**
 * rsp.cpp — RSP microcode dispatch for OLYMPIC98PC (Olympic98 64, USA). Game #4.
 *
 * AUDIO: Olympic98 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void olympic98_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* olympic98_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[olympic98pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = olympic98_get_rsp_microcode,
        .capture_gfx = olympic98_run_gfx_lle,
    };
}
