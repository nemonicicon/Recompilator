/**
 * rsp.cpp — RSP microcode dispatch for FIGHTERSDESTINYPC (Fightersdestiny 64, USA). Game #4.
 *
 * AUDIO: Fightersdestiny ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void fightersdestiny_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* fightersdestiny_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[fightersdestinypc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = fightersdestiny_get_rsp_microcode,
        .capture_gfx = fightersdestiny_run_gfx_lle,
    };
}
