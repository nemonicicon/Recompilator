...
#   endif
    ...
    struct FramebufferRenderer {
        ...
        // single-texture mode (device lacks descriptorIndexing): per-draw texture binding
        // replaces the bindless array. Fallback 1-slot set + the per-tile texture-index table
        // recorded by createGPUTiles for the draw loop's set-1/set-2 binds.
        bool singleTextureMode = false;
        std::unique_ptr<FramebufferRendererDescriptorSingleTextureSet> descSingleTextureFallbackSet;
        std::vector<uint32_t> gpuTileTextureIndices;
...
#   endif
        ...
        struct Framebuffer {
            ...
            std::unique_ptr<ClearN64DepthDescriptorSet> clearN64DepthSet; // CV64 cont.23 Brick 1: N64-depth UAV clear.
