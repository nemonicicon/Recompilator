...
#include "shaders/ClearN64DepthCS.hlsl.spirv.h"
...
#   include "shaders/ClearN64DepthCS.hlsl.dxil.h"
...
#   include "shaders/ClearN64DepthCS.hlsl.metal.h"
...
namespace RT64 {
    ...
    void ShaderLibrary::setupCommonShaders(RenderInterface *rhi, RenderDevice *device) {
        ...
        // CV64 cont.23 Brick 1: N64-depth UAV clear (Option D accurate-RDP depth gate). 32-byte push constant
        // = { float depthValue, uint left, top, right, bottom, pad0..2 }, matching ClearN64DepthCS.hlsl.
        {
            ClearN64DepthDescriptorSet descriptorSet;
            layoutBuilder.begin();
            layoutBuilder.addPushConstant(0, 0, sizeof(uint32_t) * 8, RenderShaderStageFlag::COMPUTE);
            layoutBuilder.addDescriptorSet(descriptorSet);
            layoutBuilder.end();
            clearN64Depth.pipelineLayout = layoutBuilder.create(device);

            std::unique_ptr<RenderShader> computeShader = device->createShader(CREATE_SHADER_INPUTS(ClearN64DepthCSBlobDXIL, ClearN64DepthCSBlobSPIRV, ClearN64DepthCSBlobMSL, "CSMain", shaderFormat));
            RenderComputePipelineDesc pipelineDesc(clearN64Depth.pipelineLayout.get(), computeShader.get(), 8, 8, 1);
            clearN64Depth.pipeline = device->createComputePipeline(pipelineDesc);
        }
