/**
 * rsp.cpp — RSP microcode dispatch for MADDENNFL2002PC (Maddennfl2002 64, USA). Game #4.
 *
 * AUDIO: Maddennfl2002 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void maddennfl2002_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* maddennfl2002_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[maddennfl2002pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = maddennfl2002_get_rsp_microcode,
        .capture_gfx = maddennfl2002_run_gfx_lle,
    };
}
