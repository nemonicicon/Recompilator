/**
 * rsp.cpp — RSP microcode dispatch for TUROKPC (Turok 64, USA). Game #4.
 *
 * AUDIO: Turok ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void turok_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* turok_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[turokpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = turok_get_rsp_microcode,
        .capture_gfx = turok_run_gfx_lle,
    };
}
