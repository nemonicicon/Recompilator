/**
 * main.cpp — FIFA Road to World Cup 98 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── FIFA Road to World Cup 98 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 148d6424d2135afefecde74092701c491027c23e · internal name "FIFA Road to World Cup 98"
static constexpr uint64_t FIFA98RD_ROM_HASH        = 0x7C9D14DF3E56ECA6ULL;
static constexpr uint32_t FIFA98RD_ENTRYPOINT_VRAM = 0x80000400;
...
static void fifa98rd_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        fifa98rd_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        fifa98rd_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "FIFA Road to World Cup 98 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void fifa98rd_message_box(const char* msg) {
    fifa98rd_show_error("FIFA Road to World Cup 98 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI fifa98rd_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in fifa98rdpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in fifa98rdpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "FIFA Road to World Cup 98 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void fifa98rd_vi_callback() {}
static void fifa98rd_gfx_init_callback() {}

static std::string fifa98rd_get_thread_name(const OSThread* t) {
    return "fifa98rd_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select FIFA Road to World Cup 98 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            fifa98rd_show_error("ROM Required",
                "A FIFA Road to World Cup 98 ROM is required.\n"
    ...
    // FIFA98RD_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("FIFA98RD_ROM");
    if (!rom_hint || !*rom_hint) {
        fifa98rd_show_error("ROM Required",
            "Pass the FIFA Road to World Cup 98 ROM path as the first argument,\n"
            "or set the FIFA98RD_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"fifa98rd_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        fifa98rd_show_error("Wrong ROM",
            "The selected ROM does not match FIFA Road to World Cup 98.\n"
            "Expected SHA1: 148d6424d2135afefecde74092701c491027c23e");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        fifa98rd_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        fifa98rd_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int fifa98rd_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(fifa98rd_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "fifa98rdpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "fifa98rdpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "fifa98rdpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[fifa98rdpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "fifa98rdpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[fifa98rdpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = FIFA98RD_ROM_HASH;
    entry.internal_name      = "FIFA Road to World Cup 98";
    entry.game_id            = u8"fifa98rd_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = FIFA98RD_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"fifa98rd_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            fifa98rd_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = fifa98rd::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = fifa98rd_vi_callback,
        .gfx_init_callback = fifa98rd_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = fifa98rd_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = fifa98rd_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return fifa98rd_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return fifa98rd_app_main(argc, argv);
}
#endif
