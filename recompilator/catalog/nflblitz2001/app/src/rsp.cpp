/**
 * rsp.cpp — RSP microcode dispatch for NFLBLITZ2001PC (Nflblitz2001 64, USA). Game #4.
 *
 * AUDIO: Nflblitz2001 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nflblitz2001_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nflblitz2001_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nflblitz2001pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nflblitz2001_get_rsp_microcode,
        .capture_gfx = nflblitz2001_run_gfx_lle,
    };
}
