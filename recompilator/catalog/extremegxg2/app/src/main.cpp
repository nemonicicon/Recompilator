/**
 * main.cpp — Extreme-G XG2 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Extreme-G XG2 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: ed0a50086ef9a89f5b445c20ab6f365165959630 · internal name "Extreme-G XG2"
static constexpr uint64_t EXTREMEGXG2_ROM_HASH        = 0xAAC7E75AD89A1B15ULL;
static constexpr uint32_t EXTREMEGXG2_ENTRYPOINT_VRAM = 0x8004B400;
...
static void extremegxg2_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        extremegxg2_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        extremegxg2_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Extreme-G XG2 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void extremegxg2_message_box(const char* msg) {
    extremegxg2_show_error("Extreme-G XG2 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI extremegxg2_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in extremegxg2pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in extremegxg2pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Extreme-G XG2 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void extremegxg2_vi_callback() {}
static void extremegxg2_gfx_init_callback() {}

static std::string extremegxg2_get_thread_name(const OSThread* t) {
    return "extremegxg2_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Extreme-G XG2 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            extremegxg2_show_error("ROM Required",
                "A Extreme-G XG2 ROM is required.\n"
    ...
    // EXTREMEGXG2_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("EXTREMEGXG2_ROM");
    if (!rom_hint || !*rom_hint) {
        extremegxg2_show_error("ROM Required",
            "Pass the Extreme-G XG2 ROM path as the first argument,\n"
            "or set the EXTREMEGXG2_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"extremegxg2_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        extremegxg2_show_error("Wrong ROM",
            "The selected ROM does not match Extreme-G XG2.\n"
            "Expected SHA1: ed0a50086ef9a89f5b445c20ab6f365165959630");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        extremegxg2_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        extremegxg2_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int extremegxg2_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(extremegxg2_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "extremegxg2pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "extremegxg2pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "extremegxg2pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[extremegxg2pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "extremegxg2pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[extremegxg2pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = EXTREMEGXG2_ROM_HASH;
    entry.internal_name      = "Extreme-G XG2";
    entry.game_id            = u8"extremegxg2_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = EXTREMEGXG2_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"extremegxg2_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            extremegxg2_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    extremegxg2_audio_init();
    extremegxg2_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = extremegxg2::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = extremegxg2_vi_callback,
        .gfx_init_callback = extremegxg2_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = extremegxg2_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = extremegxg2_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return extremegxg2_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return extremegxg2_app_main(argc, argv);
}
#endif
