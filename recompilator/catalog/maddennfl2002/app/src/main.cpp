/**
 * main.cpp — Madden NFL 2002 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Madden NFL 2002 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: de51147a238158adc059d0cc75fd39bbb08dcfc6 · internal name "Madden NFL 2002"
static constexpr uint64_t MADDENNFL2002_ROM_HASH        = 0x41DCFA163758D9B8ULL;
static constexpr uint32_t MADDENNFL2002_ENTRYPOINT_VRAM = 0x80200400;
...
static void maddennfl2002_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        maddennfl2002_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        maddennfl2002_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Madden NFL 2002 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void maddennfl2002_message_box(const char* msg) {
    maddennfl2002_show_error("Madden NFL 2002 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI maddennfl2002_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in maddennfl2002pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in maddennfl2002pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Madden NFL 2002 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void maddennfl2002_vi_callback() {}
static void maddennfl2002_gfx_init_callback() {}

static std::string maddennfl2002_get_thread_name(const OSThread* t) {
    return "maddennfl2002_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Madden NFL 2002 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            maddennfl2002_show_error("ROM Required",
                "A Madden NFL 2002 ROM is required.\n"
    ...
    // MADDENNFL2002_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("MADDENNFL2002_ROM");
    if (!rom_hint || !*rom_hint) {
        maddennfl2002_show_error("ROM Required",
            "Pass the Madden NFL 2002 ROM path as the first argument,\n"
            "or set the MADDENNFL2002_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"maddennfl2002_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        maddennfl2002_show_error("Wrong ROM",
            "The selected ROM does not match Madden NFL 2002.\n"
            "Expected SHA1: de51147a238158adc059d0cc75fd39bbb08dcfc6");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        maddennfl2002_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        maddennfl2002_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int maddennfl2002_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(maddennfl2002_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "maddennfl2002pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "maddennfl2002pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "maddennfl2002pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[maddennfl2002pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "maddennfl2002pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[maddennfl2002pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = MADDENNFL2002_ROM_HASH;
    entry.internal_name      = "Madden NFL 2002";
    entry.game_id            = u8"maddennfl2002_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = MADDENNFL2002_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"maddennfl2002_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            maddennfl2002_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    maddennfl2002_audio_init();
    maddennfl2002_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = maddennfl2002::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = maddennfl2002_vi_callback,
        .gfx_init_callback = maddennfl2002_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = maddennfl2002_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = maddennfl2002_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return maddennfl2002_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return maddennfl2002_app_main(argc, argv);
}
#endif
