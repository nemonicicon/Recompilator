/**
 * rsp.cpp — RSP microcode dispatch for QUAKE2PC (Quake2 64, USA). Game #4.
 *
 * AUDIO: Quake2 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void quake2_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* quake2_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[quake2pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = quake2_get_rsp_microcode,
        .capture_gfx = quake2_run_gfx_lle,
    };
}
