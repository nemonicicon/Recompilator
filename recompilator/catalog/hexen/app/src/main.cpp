/**
 * main.cpp — Hexen PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Hexen game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: a602839132f6cca71f175fb72039897705ba4661 · internal name "Hexen"
static constexpr uint64_t HEXEN_ROM_HASH        = 0x6F6953DC73EFB8E6ULL;
static constexpr uint32_t HEXEN_ENTRYPOINT_VRAM = 0x8004B400;
...
static void hexen_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        hexen_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        hexen_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Hexen PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void hexen_message_box(const char* msg) {
    hexen_show_error("Hexen — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI hexen_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in hexenpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in hexenpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Hexen — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void hexen_vi_callback() {}
static void hexen_gfx_init_callback() {}

static std::string hexen_get_thread_name(const OSThread* t) {
    return "hexen_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Hexen ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            hexen_show_error("ROM Required",
                "A Hexen ROM is required.\n"
    ...
    // HEXEN_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("HEXEN_ROM");
    if (!rom_hint || !*rom_hint) {
        hexen_show_error("ROM Required",
            "Pass the Hexen ROM path as the first argument,\n"
            "or set the HEXEN_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"hexen_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        hexen_show_error("Wrong ROM",
            "The selected ROM does not match Hexen.\n"
            "Expected SHA1: a602839132f6cca71f175fb72039897705ba4661");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        hexen_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        hexen_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int hexen_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(hexen_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "hexenpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "hexenpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "hexenpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[hexenpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "hexenpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[hexenpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = HEXEN_ROM_HASH;
    entry.internal_name      = "Hexen";
    entry.game_id            = u8"hexen_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = HEXEN_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"hexen_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            hexen_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = hexen::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = hexen_vi_callback,
        .gfx_init_callback = hexen_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = hexen_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = hexen_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return hexen_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return hexen_app_main(argc, argv);
}
#endif
