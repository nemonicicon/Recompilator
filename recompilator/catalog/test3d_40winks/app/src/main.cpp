/**
 * main.cpp — 3D Model RGB Test Program (40 Winks engine) PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── 3D Model RGB Test Program (40 Winks engine) game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: c9522561b622d2eb22e57aadbb43da6cf71d458d · internal name "3D Model RGB Test Program (40 Winks engine)"
static constexpr uint64_t TEST3D_40WINKS_ROM_HASH        = 0x567A9769B21418EBULL;
static constexpr uint32_t TEST3D_40WINKS_ENTRYPOINT_VRAM = 0x80210000;
...
static void test3d_40winks_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        test3d_40winks_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "3D Model RGB Test Program (40 Winks engine) PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        test3d_40winks_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "3D Model RGB Test Program (40 Winks engine) PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void test3d_40winks_message_box(const char* msg) {
    test3d_40winks_show_error("3D Model RGB Test Program (40 Winks engine) — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI test3d_40winks_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in test3d_40winkspc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in test3d_40winkspc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "3D Model RGB Test Program (40 Winks engine) — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void test3d_40winks_vi_callback() {}
static void test3d_40winks_gfx_init_callback() {}

static std::string test3d_40winks_get_thread_name(const OSThread* t) {
    return "test3d_40winks_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select 3D Model RGB Test Program (40 Winks engine) ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            test3d_40winks_show_error("ROM Required",
                "A 3D Model RGB Test Program (40 Winks engine) ROM is required.\n"
    ...
    // TEST3D_40WINKS_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("TEST3D_40WINKS_ROM");
    if (!rom_hint || !*rom_hint) {
        test3d_40winks_show_error("ROM Required",
            "Pass the 3D Model RGB Test Program (40 Winks engine) ROM path as the first argument,\n"
            "or set the TEST3D_40WINKS_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"test3d_40winks_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        test3d_40winks_show_error("Wrong ROM",
            "The selected ROM does not match 3D Model RGB Test Program (40 Winks engine).\n"
            "Expected SHA1: c9522561b622d2eb22e57aadbb43da6cf71d458d");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        test3d_40winks_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        test3d_40winks_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int test3d_40winks_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(test3d_40winks_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "test3d_40winkspc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "test3d_40winkspc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "test3d_40winkspc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[test3d_40winkspc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "test3d_40winkspc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[test3d_40winkspc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = TEST3D_40WINKS_ROM_HASH;
    entry.internal_name      = "3D Model RGB Test Program (40 Winks engine)";
    entry.game_id            = u8"test3d_40winks_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::None;
    entry.is_enabled         = true;
    entry.entrypoint_address = TEST3D_40WINKS_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"test3d_40winks_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            test3d_40winks_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    test3d_40winks_audio_init();
    test3d_40winks_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = test3d_40winks::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = test3d_40winks_vi_callback,
        .gfx_init_callback = test3d_40winks_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = test3d_40winks_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = test3d_40winks_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return test3d_40winks_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return test3d_40winks_app_main(argc, argv);
}
#endif
