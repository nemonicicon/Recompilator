...
#   endif
    ...
    {
        // single-texture mode: record each tile's texture-cache index so the draw loop can
        // bind that texture's own descriptor set per-draw (0xFFFFFFFF = none -> fallback set).
        gpuTileTextureIndices.assign(callTileCount, 0xFFFFFFFFu);
        for (uint32_t i = 0; i < callTileCount; i++) {
            ...
            if (callTile.tileCopyUsed) {
                ...
                gpuTileTextureIndices[i] = textureIndex; // single-texture mode: per-draw binding source
    ...
    void FramebufferRenderer::updateShaderDescriptorSet(RenderWorker *worker, const DrawBuffers *drawBuffers, const OutputBuffers *outputBuffers, const bool raytracingEnabled) {
        ...
        // single-texture mode: no bindless array exists. Keep a 1-slot fallback set (white/copy
        // draws and any texture without its own set); per-draw binding happens in submitRasterScene.
        singleTextureMode = !worker->device->getCapabilities().descriptorIndexing;
        if (singleTextureMode) {
            if (descSingleTextureFallbackSet == nullptr) {
                descSingleTextureFallbackSet = std::make_unique<FramebufferRendererDescriptorSingleTextureSet>(worker->device);
            }
        }
        else {
...
#   endif
        ...
        // single-texture mode: point the fallback set at the first valid cached texture and
        // skip the bindless-array maintenance entirely (per-draw sets carry the real bindings).
        if (singleTextureMode) {
            for (uint32_t i = 0; i < textureCacheSize; i++) {
                if ((textureCacheTextures[i] != nullptr) && (textureCacheTextures[i]->texture != nullptr)) {
                    descSingleTextureFallbackSet->setTexture(0, textureCacheTextures[i]->texture.get(), RenderTextureLayout::SHADER_READ);
                    break;
                }
            }
            return;
...
#   endif
    ...
    void FramebufferRenderer::submitRasterScene(RenderWorker *worker, const Framebuffer &framebuffer, RenderFramebufferStorage *fbStorage, const RasterScene &rasterScene, bool &depthState) {
        ...
        auto switchToGraphicsPipeline = [&]() {
            ...
            if (singleTextureMode) {
                // single-texture mode: safe defaults; the draw loop rebinds per-draw.
                worker->commandList->setGraphicsDescriptorSet(descSingleTextureFallbackSet->get(), 1);
                worker->commandList->setGraphicsDescriptorSet(descSingleTextureFallbackSet->get(), 2);
            }
            else {
        ...
        for (uint32_t i : rasterScene.instanceIndices) {
            ...
            case InstanceDrawCall::Type::RegularRect: {
                ...
                // single-texture mode: bind this draw's tile0/tile1 textures to sets 1/2
                // (each cached texture carries its own 1-slot set; fallback covers everything else,
                // including framebuffer tile copies — a known bring-up approximation).
                if (singleTextureMode) {
                    uint32_t tile0Index = 0xFFFFFFFFu;
                    uint32_t tile1Index = 0xFFFFFFFFu;
                    if (i < renderIndicesVector.size()) {
                        const uint32_t baseTile = renderIndicesVector[i].rdpTileIndex;
                        if (baseTile < gpuTileTextureIndices.size()) {
                            tile0Index = gpuTileTextureIndices[baseTile];
                        }
                        if ((baseTile + 1) < gpuTileTextureIndices.size()) {
                            tile1Index = gpuTileTextureIndices[baseTile + 1];
                        }
                    }

                    auto tileSet = [&](uint32_t idx) -> RenderDescriptorSet * {
                        if ((idx < textureCacheTextures.size()) && (textureCacheTextures[idx] != nullptr) && (textureCacheTextures[idx]->textureSet != nullptr)) {
                            return textureCacheTextures[idx]->textureSet.get();
                        }
                        return descSingleTextureFallbackSet->get();
                    };
                    worker->commandList->setGraphicsDescriptorSet(tileSet(tile0Index), 1);
                    worker->commandList->setGraphicsDescriptorSet(tileSet(tile1Index), 2);
                }
            ...
            case InstanceDrawCall::Type::FillRect: {
                ...
                if (fbStorage->colorTarget != nullptr) {
                    ...
                    // CV64 cont.23 Brick 1: mirror the HW depth clear into the programmable N64 depth store so
                    // the ROV-arbitrated opaque compare (RasterPS() under CV64_AB_RDPZ) starts each frame from
                    // the same far value the game cleared the Z buffer to. A compute dispatch is
                    // framebuffer-agnostic, so it is safe in this depth-only clear context; it only clobbers
                    // the shared PSO slot, which we reset for the next graphics draw. Inert when the shader
                    // doesn't read gN64Depth (CV64_AB_RDPZ off) — it just clears an unused texture.
                    RenderTarget *n64Target = fbStorage->depthTarget;
                    if ((n64Target != nullptr) && (n64Target->n64DepthTexture != nullptr)) {
                        int32_t rl = rectCoversWholeTarget ? 0 : int32_t(clearRect.rect.left);
                        int32_t rt = rectCoversWholeTarget ? 0 : int32_t(clearRect.rect.top);
                        int32_t rr = rectCoversWholeTarget ? int32_t(n64Target->width)  : int32_t(clearRect.rect.right);
                        int32_t rb = rectCoversWholeTarget ? int32_t(n64Target->height) : int32_t(clearRect.rect.bottom);
                        if (rl < 0) rl = 0;
                        if (rt < 0) rt = 0;
                        if (rr > int32_t(n64Target->width))  rr = int32_t(n64Target->width);
                        if (rb > int32_t(n64Target->height)) rb = int32_t(n64Target->height);
                        if ((rr > rl) && (rb > rt)) {
                            struct ClearN64DepthCB {
                                float depthValue;
                                uint32_t left, top, right, bottom, pad0, pad1, pad2;
                            } clearCB = {};
                            clearCB.depthValue = clearRect.depth;
                            clearCB.left = uint32_t(rl);
                            clearCB.top = uint32_t(rt);
                            clearCB.right = uint32_t(rr);
                            clearCB.bottom = uint32_t(rb);

                            const ShaderRecord &clearShader = shaderLibrary->clearN64Depth;
                            const uint32_t BlockSize = 8;
                            const uint32_t dispatchX = (uint32_t(rr - rl) + BlockSize - 1) / BlockSize;
                            const uint32_t dispatchY = (uint32_t(rb - rt) + BlockSize - 1) / BlockSize;
                            worker->commandList->setPipeline(clearShader.pipeline.get());
                            worker->commandList->setComputePipelineLayout(clearShader.pipelineLayout.get());
                            worker->commandList->setComputePushConstants(0, &clearCB);
                            worker->commandList->setComputeDescriptorSet(framebuffer.clearN64DepthSet->get(), 0);
                            worker->commandList->dispatch(dispatchX, dispatchY, 1);
                            // Make the clear visible to the ROV reads in the upcoming opaque draws.
                            worker->commandList->barriers(RenderBarrierStage::GRAPHICS, RenderTextureBarrier(n64Target->n64DepthTexture.get(), RenderTextureLayout::GENERAL));
                            // The compute dispatch overwrote the shared pipeline-state slot; force a re-bind.
                            previousPipeline = nullptr;
                        }
                    }
                }
...
#   endif
    ...
    void FramebufferRenderer::recordFramebuffer(RenderWorker *worker, uint32_t framebufferIndex) {
        ...
        static int _b1rf = 0;
        if (_b1rf++ < 40) {
            fprintf(stderr, "[brick1rf] #%d fbIdx=%u colorTgt=%p depthTgt=%p n64tex=%p\n",
                _b1rf, framebufferIndex, (void*)colorTarget, (void*)depthTarget,
                (void*)(depthTarget ? depthTarget->n64DepthTexture.get() : nullptr));
            fflush(stderr);
        }
        ...
        // CV64 cont.23 Brick 1: put the programmable N64 depth UAV into GENERAL once per frame. It stays
        // there for the whole framebuffer; ROV semantics order opaque read-modify-writes across draws without
        // per-draw UAV barriers (the clear→first-draw hazard is covered by a UAV barrier at the clear site).
        if (depthTarget->n64DepthTexture != nullptr) {
            startBarriers.emplace_back(RenderTextureBarrier(depthTarget->n64DepthTexture.get(), RenderTextureLayout::GENERAL));
        }
        worker->commandList->barriers(RenderBarrierStage::GRAPHICS, startBarriers);

        // CV64 cont.24 Brick 1: reset the programmable N64 depth store to FAR (1.0) over the WHOLE depth target
        // at frame start. The data-driven FillRect mirror (submitRasterScene) only fires on depth-ONLY clears;
        // the castle's MAIN framebuffer clears Z via the color+depth (clearColor) path, so without this reset
        // the store is NEVER cleared and the ROV opaque compare accumulates last-frame depth (cont.24's
        // broken-clear bug — "compare = no change" was an artifact of stored ≈ live depth). n64DepthTexture is
        // already in GENERAL from startBarriers above; the UAV barrier after the dispatch makes the cleared
        // values visible to the opaque ROV reads in the scene draws below. Inert when CV64_AB_RDPZ is off (the
        // shader never reads gN64Depth — this just clears an unused texture). Far = 1.0 because the viewport
        // maps screenZ = ndc_z*0.499 + 0.499 (castle far ≈ 0.97–0.99), so 1.0 is beyond all geometry.
        if (depthTarget->n64DepthTexture != nullptr) {
            struct ClearN64DepthCB {
                float depthValue;
                uint32_t left, top, right, bottom, pad0, pad1, pad2;
            } clearCB = {};
            clearCB.depthValue = 1.0f; // FAR. (cont.25 proved the clear lands via a 0.5 diagnostic -> screen went black under permissive.)
            clearCB.left = 0;
            clearCB.top = 0;
            clearCB.right = depthTarget->width;
            clearCB.bottom = depthTarget->height;

            const ShaderRecord &clearShader = shaderLibrary->clearN64Depth;
            const uint32_t BlockSize = 8;
            const uint32_t dispatchX = (depthTarget->width + BlockSize - 1) / BlockSize;
            const uint32_t dispatchY = (depthTarget->height + BlockSize - 1) / BlockSize;
            static int _b1clr = 0;
            if (_b1clr++ < 40) {
                fprintf(stderr, "[brick1clr] #%d fbIdx=%u tgt=%p tex=%p %ux%u disp=%u,%u pipe=%p layout=%p set=%p\n",
                    _b1clr, framebufferIndex, (void*)depthTarget, (void*)depthTarget->n64DepthTexture.get(),
                    depthTarget->width, depthTarget->height, dispatchX, dispatchY,
                    (void*)clearShader.pipeline.get(), (void*)clearShader.pipelineLayout.get(),
                    (void*)framebuffer.clearN64DepthSet->get());
                fflush(stderr);
            }
            worker->commandList->setPipeline(clearShader.pipeline.get());
            worker->commandList->setComputePipelineLayout(clearShader.pipelineLayout.get());
            ...
            worker->commandList->setComputeDescriptorSet(framebuffer.clearN64DepthSet->get(), 0);
            worker->commandList->dispatch(dispatchX, dispatchY, 1);
            // Make the cleared store visible to the ROV reads in the opaque draws below (compute write -> graphics read).
            worker->commandList->barriers(RenderBarrierStage::GRAPHICS, RenderTextureBarrier(depthTarget->n64DepthTexture.get(), RenderTextureLayout::GENERAL));
        }
...
#       endif
    ...
    void FramebufferRenderer::addFramebuffer(const DrawParams &p) {
        ...
        while (framebufferCount > framebufferVector.size()) {
            ...
            framebufferVector.back().clearN64DepthSet = std::make_unique<ClearN64DepthDescriptorSet>(p.worker->device); // CV64 cont.23 Brick 1
        }
        ...
        // CV64 cont.23 Brick 1: bind the programmable N64 depth UAV on BOTH descriptor sets. Unlike the HW
        // depth (which the real/dummy split exists for — it can't be a render-target attachment and an SRV at
        // once), this is a SEPARATE rasterizer-ordered UAV, so the SAME real texture is valid in both the
        // depth-read and depth-write states. The shader only touches it when CV64_AB_RDPZ is enabled.
        RenderTexture *n64DepthTexture = p.fbStorage->depthTarget->n64DepthTexture.get();
        RenderTextureView *n64DepthTextureView = p.fbStorage->depthTarget->n64DepthTextureView.get();
        framebuffer.descRealFbSet->setTexture(framebuffer.descRealFbSet->gN64Depth, n64DepthTexture, RenderTextureLayout::GENERAL, n64DepthTextureView);
        framebuffer.descDummyFbSet->setTexture(framebuffer.descDummyFbSet->gN64Depth, n64DepthTexture, RenderTextureLayout::GENERAL, n64DepthTextureView);
        framebuffer.clearN64DepthSet->setTexture(framebuffer.clearN64DepthSet->gN64Depth, n64DepthTexture, RenderTextureLayout::GENERAL, n64DepthTextureView);
