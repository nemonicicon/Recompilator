...
// cv64 S41 timing layer: the per-DL estimated real-RDP cycle accumulator (defined in rt64_rsp.cpp).
...
namespace RT64 {
    ...
    void RDP::reset() {
        /* cv64 session 19: pinpoint the +0x171 crash that fires once player ticks */
        static int _rst = 0; ++_rst;
        if (_rst <= 10) { fprintf(stderr, "[rdp_reset] #%d this=%p state=%p\n", _rst, (void*)this, (void*)state); fflush(stderr); }
        ...
        convertK[4] = 0;
        convertK[5] = 0;
        if (_rst <= 10) { fprintf(stderr, "[rdp_reset] #%d before scissor\n", _rst); fflush(stderr); }
        scissorRectStack[0] = { 0, 0, 8192, 8192 };
        scissorModeStack[0] = 0;
        pendingCommandCurrentBytes = 0;
        pendingCommandRemainingBytes = 0;
        if (_rst <= 10) { fprintf(stderr, "[rdp_reset] #%d before clearExtended\n", _rst); fflush(stderr); }
        clearExtended();
        if (_rst <= 10) { fprintf(stderr, "[rdp_reset] #%d done\n", _rst); fflush(stderr); }
    }
    ...
    void RDP::setColorImage(uint8_t fmt, uint8_t siz, uint16_t width, uint32_t address) {
        ...
        // SESSION 28 cont.16+ (Reinhardt): reject a CORRUPT color-image command. The weretiger scene
        // submits a garbage gsDPSetColorImage (addr in low/OS RDRAM ~0x244, width 1890-2006 — the N64 max
        // is 1024). Tracking it makes RT64 create/render a bogus framebuffer over the engine scheduler
        // struct → the hang/crash (pinned via the Dr0 watchpoint to Framebuffer::copyNativeToRAM). Ignore
        // the invalid command and keep the previous valid color image. (Deeper root = corrupt weretiger DL.)
        // SM64PC S45 boot#7 CORRECTION: the `addr < 0x100000` floor was a CV64-SHAPED HEURISTIC, not
        // a hardware invariant — SM64's Z-BUFFER LIVES AT PHYSICAL 0x400 (gZBuffer = 0x80000400,
        // verified byte-exact in the ROM's setup_game_memory) and is cleared as a color image EVERY
        // frame; the floor rejected it → no valid color image on frame 1 → fullsync AV + null
        // framebuffer cascade = SM64's "no video". The RDP has no concept of an invalid address; the
        // real invariant is WIDTH (CV64's corrupt 1890-wide images exceed the encodable max). Width only.
        if (width == 0u || width > 1024u) {
            static int _badci = 0;
            if (_badci++ < 32) {
                fprintf(stderr, "[badcolorimg] IGNORED corrupt setColorImage addr=0x%08X width=%u siz=%u fmt=%u (kept previous)\n",
                        newAddress, (unsigned)width, (unsigned)siz, (unsigned)fmt);
                fflush(stderr);
            }
            return;
        }
        // [cimg-census 2026-08-28] instrument only, env RECOMP_CIMG_CENSUS=1. THE QUESTION IT
        // ANSWERS: SOTE renders 3.1M triangles per census window, yet every framebuffer pair RT64
        // reports has colorImage=0x00164480 -- the SAME address [zclear] calls the DEPTH buffer --
        // while the VI presents 0x38F800/0x3B5000/0x3DA800. Either the game is using the standard
        // "clear Z by pointing the color image at it" idiom and the real scene pair is going
        // missing, or the address is being MIS-RESOLVED (F3D routes SETCIMG through
        // RSP::setColorImage -> fromSegmented, so a wrong segment table lands the scene in the
        // wrong buffer). Logging every DISTINCT accepted color image separates those two: if
        // 0x38F800/0x3B5000/0x3DA800 never appear here, the game never asks to draw into the
        // buffers the VI shows, and the 3D has nowhere to land.
        {
            static const bool on = [] { const char *e = std::getenv("RECOMP_CIMG_CENSUS"); return (e != nullptr) && (e[0] == '1'); }();
            if (on) {
                static uint32_t seenAddr[64]; static uint64_t seenCnt[64]; static int seenN = 0;
                int idx = -1;
                for (int k = 0; k < seenN; k++) if (seenAddr[k] == newAddress) { idx = k; break; }
                if (idx < 0 && seenN < 64) { idx = seenN++; seenAddr[idx] = newAddress; seenCnt[idx] = 0; }
                if (idx >= 0) {
                    seenCnt[idx]++;
                    if (seenCnt[idx] == 1 || (seenCnt[idx] % 20000) == 0) {
                        fprintf(stderr, "[cimg] set colorImage=0x%08X w=%u siz=%u fmt=%u (count %llu) | distinct so far:",
                                newAddress, (unsigned)width, (unsigned)siz, (unsigned)fmt, (unsigned long long)seenCnt[idx]);
                        for (int k = 0; k < seenN; k++) fprintf(stderr, " 0x%08X=%llu", seenAddr[k], (unsigned long long)seenCnt[k]);
...
#       endif
    ...
    void RDP::setDepthImage(uint32_t address) {
        const uint32_t newAddress = maskAddress(address);
        // [zimg] DOES THE GAME EVER SET A DEPTH IMAGE? (2026-08-29: the 3D stayed on screen, seen
        // across several brought-up games but NOT all of them.) KI Gold's [zclear] reports
        // depth=0x00000000 depthFb=NULL for an entire run, so either SETZIMG never arrives or it
        // arrives as 0. Those two have completely different fixes, and nothing distinguished them:
        // the only existing log here is behind a compile-time #ifdef that is off. Runtime-gated,
        // deduped by address, silent unless RECOMP_ZIMG_LOG=1.
        {
            static const bool on = [] { const char *e = std::getenv("RECOMP_ZIMG_LOG"); return (e != nullptr) && (e[0] == '1'); }();
            if (on) {
                static uint32_t seen[16]; static int nseen = 0; static uint64_t calls = 0;
                calls++;
                bool known = false;
                for (int k = 0; k < nseen; k++) if (seen[k] == newAddress) { known = true; break; }
                if (!known && nseen < 16) {
                    seen[nseen++] = newAddress;
                    fprintf(stderr, "[zimg] setDepthImage raw=0x%08X masked=0x%08X (call #%llu) distinct:",
                            address, newAddress, (unsigned long long)calls);
                    for (int k = 0; k < nseen; k++) fprintf(stderr, " 0x%08X", seen[k]);
                    fprintf(stderr, "\n");
                    fflush(stderr);
                }
            }
...
#   endif
    ...
    __forceinline void loadWord(uint8_t *TMEM, uint32_t tmemAddress, uint32_t tmemXorMask, const uint8_t *RDRAM, uint32_t textureAddress) {
        ...
        // cv64 (session 19): mask every RDRAM read to the physical N64 RDRAM size. Real RDP hardware
        // wraps RDRAM addresses to the installed RAM size; RT64 omitted this for speed assuming valid
        // input. Reaching gameplay surfaced actor display lists whose texture/TLUT dimensions are
        // garbage, walking textureAddress far past the buffer mid-load → access violation in this
        // exact line. Masking is hardware-faithful and bullet-proofs ALL load paths against the crash.
        if constexpr (RGBA32) {
            ...
            TMEM[(tmemAddress + 0) ^ tmemXorMask] = RDRAM[((textureAddress + (0 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[(tmemAddress + 1) ^ tmemXorMask] = RDRAM[((textureAddress + (1 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[(tmemAddress + 2) ^ tmemXorMask] = RDRAM[((textureAddress + (4 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[(tmemAddress + 3) ^ tmemXorMask] = RDRAM[((textureAddress + (5 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[((tmemAddress + 0) ^ tmemXorMask) | UpperTMEM] = RDRAM[((textureAddress + (2 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[((tmemAddress + 1) ^ tmemXorMask) | UpperTMEM] = RDRAM[((textureAddress + (3 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[((tmemAddress + 2) ^ tmemXorMask) | UpperTMEM] = RDRAM[((textureAddress + (6 & offsetMask)) ^ 3) & RDRAMSize];
            TMEM[((tmemAddress + 3) ^ tmemXorMask) | UpperTMEM] = RDRAM[((textureAddress + (7 & offsetMask)) ^ 3) & RDRAMSize];
        }
        else {
            // Copy the entire word.
            for (uint32_t i = 0; i < 8; i++) {
                TMEM[(tmemAddress + i) ^ tmemXorMask] = RDRAM[((textureAddress + (i & offsetMask)) ^ 3) & RDRAMSize];
            }
        }
    }
    ...
    void RDP::loadTileOperation(const LoadTile &loadTile, const LoadTexture &loadTexture, bool deferred) {
        ...
        if (deferred) {
            ...
            // cv64 (session 19): guard against a garbage texture address (out-of-bounds RDRAM read → crash).
            if (textureStart > RDRAMSize) {
                static int _bt = 0;
                if (++_bt <= 64) { fprintf(stderr, "[rt64_badtex] SKIP TILE load: textureStart=0x%08X addr=0x%08X w=%u siz=%u\n",
                    textureStart, loadTexture.address, loadTexture.width, loadTexture.siz); fflush(stderr); }
                return;
            }
            if (RGBA32) {
    ...
    void RDP::loadBlockOperation(const LoadTile &loadTile, const LoadTexture &loadTexture, bool deferred) {
        ...
        // cv64 SESSION 40e [fire7] — THE LAST LAYER: is the flame's TMEM load hijacked by a stale
        // tracked-framebuffer overlap (checkFramebufferOverlap tags TMEM regions / substitutes a GPU
        // tile copy when findMostRecentContaining matches — the flame's texels live at ~0x1D82xx,
        // right in CV64's framebuffer neighborhood, while the WORKING sibling's at 0x271BD8 sit far
        // outside), and what do the source RDRAM texels contain (IA16: final flame alpha == T0 alpha)?
        // Keyed on the flame combiner (set before SETTIMG in its DL); cap counts in-condition only.
        {
            const auto &_cc = colorCombinerStack[colorCombinerStackSize - 1];
            if ((_cc.L & 0xFFFFFFu) == 0x151660u && _cc.H == 0x2515257Fu) {
                static int _f7 = 0;
                if (_f7++ < 24) {
                    Framebuffer *_fb = state->framebufferManager.findMostRecentContaining(textureStart, textureEnd);
                    fprintf(stderr, "[fire7] loadBlock deferred=%d texStart=0x%06X end=0x%06X words=%u tmem=0x%X FBOVERLAP=%s",
                            deferred ? 1 : 0, textureStart, textureEnd, wordCount, tmemStart,
                            (_fb != nullptr) ? "YES" : "no");
                    if (_fb != nullptr) fprintf(stderr, " fb[0x%06X w=%u maxH=%u]", _fb->addressStart, _fb->width, _fb->maxHeight);
                    // S40f: dump SOURCE texels in BOTH branches (run #15 showed only deferred=1 reaches
                    // this combiner-keyed probe — the real copy replays in a different context). The flame
                    // pixels cluster right-of-center per row (ROM truth): frame-A +0x114 should read
                    // `0304 1F2B 5B80 BDFF EFFF EFFF...`; +0x914 = frame B same row; +0x000 = corner (zeros).
                    if ((textureStart + 0x924) <= RDRAMSize) {
                        static const uint32_t offs[3] = { 0x114u, 0x914u, 0x000u };
                        for (int r = 0; r < 3; r++) {
                            fprintf(stderr, " | +%03X:", offs[r]);
                            for (int k = 0; k < 16; k++) fprintf(stderr, "%s%02X", (k % 2) ? "" : " ", state->RDRAM[textureStart + offs[r] + k]);
                        }
                    }
                    fprintf(stderr, "\n"); fflush(stderr);
                }
        ...
        if (deferred) {
            ...
            if (textureStart > RDRAMSize) {
                static int _bt = 0;
                if (++_bt <= 64) { fprintf(stderr, "[rt64_badtex] SKIP BLOCK load: textureStart=0x%08X addr=0x%08X w=%u siz=%u\n",
                    textureStart, loadTexture.address, loadTexture.width, loadTexture.siz); fflush(stderr); }
                return;
            }
            if (RGBA32) {
    ...
    void RDP::loadTLUTOperation(const LoadTile &loadTile, const LoadTexture &loadTexture, bool deferred) {
        ...
        if (deferred) {
            ...
            const uint8_t *RDRAM = state->RDRAM;
            // cv64 (session 19): guard against a garbage texture address (out-of-bounds RDRAM read → crash).
            // First gameplay frame surfaced an actor whose deferred LoadTLUT resolves to a wild address
            // (e.g. 0x0264EBC2), crashing in loadWord's RDRAM[textureAddress^3]. Skip + log instead.
            const uint32_t _texEnd = textureStart + bytesPerRow * rowCount + (wordsPerRow << 1) + 8u;
            if (textureStart > RDRAMSize || _texEnd > RDRAMSize || _texEnd < textureStart) {
                static int _bt = 0;
                if (++_bt <= 96) { fprintf(stderr, "[rt64_badtex] TLUT oob start=0x%08X end=0x%08X addr=0x%08X w=%u siz=%u uls=%u ult=%u lrs=%u lrt=%u rows=%u wpr=%u\n",
                    textureStart, _texEnd, loadTexture.address, loadTexture.width, loadTexture.siz, loadTile.uls, loadTile.ult, loadTile.lrs, loadTile.lrt, rowCount, wordsPerRow); fflush(stderr); }
                return;
            }
            if (RGBA32) {
...
#   endif
    ...
    bool RDP::loadTileCopyCheck(uint8_t tile, uint16_t uls, uint16_t ult, uint16_t lrs, uint16_t lrt) {
        ...
        if (framebuffer != nullptr) {
            { static const bool _fblog = std::getenv("RT64_FBTEX_LOG") != nullptr;
              if (_fblog) { static int _n=0; if(_n<80){_n++; fprintf(stderr,"[fbtex] #%d src_fb=0x%08X fbW=%d fbH=%d | tileW=%d rows=%d siz=%d | dst colorImg=0x%08X dstW=%d\n",
                  _n, framebuffer->addressStart, framebuffer->width, framebuffer->height, tileWidth, rowCount, t.siz, colorImage.address, colorImage.width); fflush(stderr);} } }
            FramebufferTile fbTile;
...
#   endif
    ...
    void RDP::setEnvColor(uint32_t color) {
        ...
        // cv64 (session 11 visibility hack): force env color to white. The combiner
        // for many CV64 cutscene/non-lit drawcalls uses env color or prim color as the
        // color multiplier, ignoring vertex color. Dark env color -> dark output.
...
// ── [no2d 2026-08-28] DIAGNOSTIC ONLY, env RECOMP_NO2D=1, default off (byte-identical when unset).
// THE QUESTION: SOTE reaches Stage One and the player sees a pause/SAVE overlay that never goes
// away, with no 3D world (the 2D overlay persists while the radar keeps updating correctly).
// Uncapped counters say the world IS drawn every frame into the presented buffer (8,358,304
// perspective triangles, 0 culled; 5974/5974 perspective pairs landing in a VI-presented buffer;
// 29-31 world draws per second) and the Z-clear never flatlines (ratio 0.500, 0 stalls). But pair
// creation is not proof of PIXELS. Suppressing the 2D answers it in one look: if the world appears
// underneath, the geometry rasterises fine and the defect is the overlay never being cleared; if
// the screen is still empty, the perspective draws produce no pixels and the hunt moves to raster.
// fillRect is skipped ONLY for real colour fills -- a fill whose colour image IS the depth image is
// the per-frame Z CLEAR and must survive, or stale depth would hide the very geometry under test.
static bool rdp_no2d() {
    static const bool on = [] { const char *e = std::getenv("RECOMP_NO2D"); return (e != nullptr) && (e[0] == '1'); }();
    return on;
}
    ...
    void RDP::fillRect(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, const ExtendedAlignment &extAlignment) {
        // [fillcensus] KI Gold 2026-08-29 (flashing and persistent images were the
        // blocker for this game). Persistence means the framebuffer keeps
        // OLD pixels. On N64 a colour clear IS a full-screen fillRect, so the first question is
        // simply: does the game issue one per frame, and is it full-screen? Counts fills, and
        // separately those covering >=90% of the color image width, per ~1s. Ungated but ONE line
        // per second, and it prints the running totals so a zero is a real zero.
        {
            static long total = 0, full = 0, zfill = 0;
            static long last_report = 0;
            const bool is_z = (colorImage.address == depthImage.address);
            const int32_t w = (lrx - ulx) >> 2;   // fill coords are 10.2 fixed point
            total++;
            if (is_z) zfill++;
            else if (colorImage.width != 0 && w >= (int32_t)(colorImage.width * 9 / 10)) full++;
            if (total == 1 || (total - last_report) >= 200) {   // report the FIRST call too: at ~30fps a per-frame clear is only ~1800 in 60s, so a 2000 threshold reported NOTHING and looked like "never called"
                last_report = total;
                fprintf(stderr, "[fillcensus] fills=%ld fullscreen-colour=%ld zclears=%ld "
                                "cimg=0x%08X w=%u\n",
                        total, full, zfill, colorImage.address, (unsigned)colorImage.width);
                fflush(stderr);
            }
        }
        if (rdp_no2d() && (colorImage.address != depthImage.address)) { return; }   // [no2d] keep the Z clear
...
#   endif
    ...
    void RDP::drawRect(int32_t ulx, int32_t uly, int32_t lrx, int32_t lry, int16_t uls, int16_t ult, int16_t dsdx, int16_t dtdy, bool flip, const ExtendedAlignment &extAlignment) {
        ...
        // cv64 S41 timing layer: rect cost (coordinates are 10.2 fixed → area/16). Real RDP rates:
        // fill ≈ 4 pix/clock (16-bit), copy ≈ 2, 1-cycle ≈ 1, 2-cycle ≈ 0.5 — see rt64_rsp.cpp header.
        {
            double _area = double((lrx - ulx) > 0 ? (lrx - ulx) : 0) * double((lry - uly) > 0 ? (lry - uly) : 0) / 16.0;
            if (_area > 76800.0) _area = 76800.0;   // scissor bound: a rect can't fill past the framebuffer
            const uint32_t _ct = otherMode.cycleType();
            const double _cyclesPerPix = (_ct == G_CYC_FILL) ? 0.25 : (_ct == G_CYC_COPY) ? 0.5 : (_ct == G_CYC_2CYCLE) ? 2.0 : 1.0;
            g_rdp_estimated_cost_cycles += 64.0 + _area * _cyclesPerPix * 1.0 /* pure pipeline rate (v3 calib) */;
            // P1 v2 counting (TIMING_FIDELITY_DESIGN): rect pixels by cycle bucket + scanlines
            // (10.2 fixed → /4; clamped to the 240-line framebuffer like _area's 76800 clamp).
            {
                double _lines2 = double((lry - uly) > 0 ? (lry - uly) : 0) / 4.0;
                if (_lines2 > 240.0) _lines2 = 240.0;
                const int _b2 = (_ct == G_CYC_FILL) ? 0 : (_ct == G_CYC_COPY) ? 1
                              : (_ct == G_CYC_2CYCLE) ? 3 : 2;
                g_rdptime2_pix[_b2] += _area;
                g_rdptime2_lines += _lines2;
                g_rdptime2_setup += 64.0;
                g_rdptime2_prims++;
            }
        }
        ...
        // ── [rectdrop] (SOTE, 2026-08-26) — instrument only, shares the RECOMP_DL_CENSUS env ──────
        // An empty rect returns from here SILENTLY. That makes a game whose rect coordinates arrive
        // degenerate indistinguishable, from every upstream instrument, from a game that drew
        // nothing: the walker dispatches the command, the display list looks healthy, and no draw
        // call is ever produced. Motivating case: SOTE issues ~53 G_TEXRECTs per display list
        // (158,477 over 3,000 lists) and RT64's scene ends each frame with ONE rectangle — the
        // clear. Count both arms and sample the raw coordinates. No behaviour change.
        {
            static const bool _rdOn = [] { const char *e = std::getenv("RECOMP_DL_CENSUS"); return (e != nullptr) && (e[0] == '1'); }();
            if (_rdOn) {
                static uint64_t _rdIn = 0, _rdEmpty = 0;
                _rdIn++;
                const bool _rdIsEmpty = drawRect.isEmpty();
                if (_rdIsEmpty) {
                    _rdEmpty++;
                }

                if ((_rdIn <= 4) || ((_rdIn % 20000) == 0)) {
                    fprintf(stderr, "[rectdrop] rects=%llu emptyDropped=%llu | this: raw[%d,%d,%d,%d] -> draw[%d,%d,%d,%d] %s | cyc=0x%X scisNull=%d scis[%d,%d,%d,%d]\n",
                            (unsigned long long)(_rdIn), (unsigned long long)(_rdEmpty),
                            ulx, uly, lrx, lry,
                            drawRect.ulx, drawRect.uly, drawRect.lrx, drawRect.lry,
                            _rdIsEmpty ? "EMPTY-DROPPED" : "drawn",
                            otherMode.cycleType(), scissorRect.isNull() ? 1 : 0,
