 ...
 *
 * SESSION 44: identified in CHAMELEONTWIST_REV1 by signature (tools/match_libultra.py); these
 * four are the ones CHAMELEONTWIST_REV1's recompiled code actually calls.
 *
...
/* osPiReadIo is NOT stubbed here: librecomp (pi.cpp) ships the real
 * osPiReadIo_recomp engine-wide, and a per-tree open-bus copy both shadows it and
 * breaks the link (LNK2005 "already defined in os_stubs.obj"). The stub predates the
 * engine implementation; 312 of the 316 trees have already dropped it. */

/* ── osMapTLB — FAITHFUL: program the engine's LLE TLB. ─────────────────────
 * Unlike cv64pc (which no-op'd this and populated the TLB from its native
 * mapOverlay HLE), chameleontwist_rev1pc has no game patches — the game's own recompiled
