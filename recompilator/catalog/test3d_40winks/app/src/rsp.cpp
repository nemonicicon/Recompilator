/**
 * rsp.cpp — RSP microcode dispatch for TEST3D_40WINKSPC (Test3d_40winks 64, USA). Game #4.
 *
 * AUDIO: Test3d_40winks ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void test3d_40winks_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* test3d_40winks_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[test3d_40winkspc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = test3d_40winks_get_rsp_microcode,
        .capture_gfx = test3d_40winks_run_gfx_lle,
    };
}
