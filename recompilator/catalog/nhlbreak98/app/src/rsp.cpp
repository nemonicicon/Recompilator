/**
 * rsp.cpp — RSP microcode dispatch for NHLBREAK98PC (Nhlbreak98 64, USA). Game #4.
 *
 * AUDIO: Nhlbreak98 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void nhlbreak98_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* nhlbreak98_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[nhlbreak98pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = nhlbreak98_get_rsp_microcode,
        .capture_gfx = nhlbreak98_run_gfx_lle,
    };
}
