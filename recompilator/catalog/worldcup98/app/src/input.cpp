/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for WORLDCUP98PC.
 *
...
void worldcup98_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in worldcup98_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool worldcup98_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void worldcup98_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t worldcup98_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  worldcup98_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = worldcup98_poll_input,
        .get_input               = worldcup98_get_input,
        .set_rumble              = worldcup98_set_rumble,
        .get_connected_device_info = worldcup98_get_connected_device,
    };
}
