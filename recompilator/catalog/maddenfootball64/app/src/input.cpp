/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for MADDENFOOTBALL64PC.
 *
...
void maddenfootball64_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in maddenfootball64_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool maddenfootball64_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void maddenfootball64_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t maddenfootball64_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  maddenfootball64_get_input() already supplies
...
void maddenfootball64_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = maddenfootball64_poll_input,
        .get_input               = maddenfootball64_get_input,
        .set_rumble              = maddenfootball64_set_rumble,
        .get_connected_device_info = maddenfootball64_get_connected_device,
    };
}
