...
namespace RT64 {
    struct OptimizerCacheSPIRV {
        ...
        // Single-source-blend permutations (GPUs without dualSrcBlend, e.g. a GPU without dual-source blending).
        respv::Shader rasterPS_SS;
        respv::Shader rasterPSMS_SS;
        respv::Shader rasterPSFlat_SS;
        respv::Shader rasterPSFlatMS_SS;
        // Single-source-blend + single-texture permutations (GPUs without dualSrcBlend AND
        // without descriptorIndexing — the single-texture profile: per-draw bound tile textures).
        respv::Shader rasterPS_SS_ST;
        respv::Shader rasterPSMS_SS_ST;
        respv::Shader rasterPSFlat_SS_ST;
        respv::Shader rasterPSFlatMS_SS_ST;
    ...
    struct PipelineCreation {
        ...
        bool alphaBlend;
        bool additiveBlend = false; // M=framebuffer, B=ONE -> GPU dstBlend = ONE (additive) instead of alpha-over
        bool culling;
        bool NoN;
        bool zCmp;
        bool zUpd;
