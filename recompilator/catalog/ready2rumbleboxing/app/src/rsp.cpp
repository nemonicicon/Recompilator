/**
 * rsp.cpp — RSP microcode dispatch for READY2RUMBLEBOXINGPC (Ready2rumbleboxing 64, USA). Game #4.
 *
 * AUDIO: Ready2rumbleboxing ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void ready2rumbleboxing_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* ready2rumbleboxing_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[ready2rumbleboxingpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = ready2rumbleboxing_get_rsp_microcode,
        .capture_gfx = ready2rumbleboxing_run_gfx_lle,
    };
}
