/**
 * main.cpp — Super Bowling PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Super Bowling game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 56937a9e9536af55ebed06480413265848c4a04a · internal name "Super Bowling"
static constexpr uint64_t SUPERBOWLING_ROM_HASH        = 0x5287B99F58F4EF7BULL;
static constexpr uint32_t SUPERBOWLING_ENTRYPOINT_VRAM = 0x80025C00;
...
static void superbowling_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        superbowling_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        superbowling_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Super Bowling PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void superbowling_message_box(const char* msg) {
    superbowling_show_error("Super Bowling — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI superbowling_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in superbowlingpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in superbowlingpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Super Bowling — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void superbowling_vi_callback() {}
static void superbowling_gfx_init_callback() {}

static std::string superbowling_get_thread_name(const OSThread* t) {
    return "superbowling_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Super Bowling ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            superbowling_show_error("ROM Required",
                "A Super Bowling ROM is required.\n"
    ...
    // SUPERBOWLING_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("SUPERBOWLING_ROM");
    if (!rom_hint || !*rom_hint) {
        superbowling_show_error("ROM Required",
            "Pass the Super Bowling ROM path as the first argument,\n"
            "or set the SUPERBOWLING_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"superbowling_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        superbowling_show_error("Wrong ROM",
            "The selected ROM does not match Super Bowling.\n"
            "Expected SHA1: 56937a9e9536af55ebed06480413265848c4a04a");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        superbowling_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        superbowling_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int superbowling_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(superbowling_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "superbowlingpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "superbowlingpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "superbowlingpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[superbowlingpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "superbowlingpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[superbowlingpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = SUPERBOWLING_ROM_HASH;
    entry.internal_name      = "Super Bowling";
    entry.game_id            = u8"superbowling_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = SUPERBOWLING_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"superbowling_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            superbowling_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    superbowling_audio_init();
    superbowling_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = superbowling::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = superbowling_vi_callback,
        .gfx_init_callback = superbowling_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = superbowling_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = superbowling_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return superbowling_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return superbowling_app_main(argc, argv);
}
#endif
