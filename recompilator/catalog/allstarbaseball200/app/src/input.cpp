/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for ALLSTARBASEBALL200PC.
 *
...
void allstarbaseball200_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in allstarbaseball200_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool allstarbaseball200_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void allstarbaseball200_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t allstarbaseball200_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  allstarbaseball200_get_input() already supplies
...
void allstarbaseball200_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = allstarbaseball200_poll_input,
        .get_input               = allstarbaseball200_get_input,
        .set_rumble              = allstarbaseball200_set_rumble,
        .get_connected_device_info = allstarbaseball200_get_connected_device,
    };
}
