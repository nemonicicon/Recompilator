/**
 * rsp.cpp — RSP microcode dispatch for SNOW1080PC (Snow1080 64, USA). Game #4.
 *
 * AUDIO: Snow1080 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void snow1080_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* snow1080_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[snow1080pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = snow1080_get_rsp_microcode,
        .capture_gfx = snow1080_run_gfx_lle,
    };
}
