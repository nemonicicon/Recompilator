/**
 * rsp.cpp — RSP microcode dispatch for TUROK2R1PC (Turok2r1 64, USA). Game #4.
 *
 * AUDIO: Turok2r1 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void turok2r1_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* turok2r1_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[turok2r1pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = turok2r1_get_rsp_microcode,
        .capture_gfx = turok2r1_run_gfx_lle,
    };
}
