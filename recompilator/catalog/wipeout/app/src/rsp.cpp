/**
 * rsp.cpp — RSP microcode dispatch for WIPEOUTPC (Wipeout 64, USA). Game #4.
 *
 * AUDIO: Wipeout ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void wipeout_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* wipeout_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[wipeoutpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = wipeout_get_rsp_microcode,
        .capture_gfx = wipeout_run_gfx_lle,
    };
}
