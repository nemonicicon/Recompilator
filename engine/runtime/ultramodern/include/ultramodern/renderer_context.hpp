...
namespace ultramodern {
    namespace renderer {
        struct ViRegs {
            ...
            // Half-res scanout factor from osViSetXScale/osViSetYScale (1.0 = none). Carried on a SEPARATE
            // channel from VI_X_SCALE_REG — folding it into that register false-triggers RT64's 480i
            // doubled-stride detection (deinterlacedWidth) and black-screens. RT64 applies this in the
            // PRESENT (viewRectangle), not the VI register, so the framebuffer sizing stays correct.
