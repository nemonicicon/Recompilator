/**
 * audio.cpp â€” SDL2 push-mode audio backend for RUGRATSPC.
 *
 * RUGRATS uses Konami's custom audio sequencer running at 32 kHz.
...
void rugrats_queue_samples(int16_t* audio_data, size_t sample_count) {
    ...
    // RUGRATS (session 18, Reinhardt): cushion + production-rate measurement.
    ...
    //     ~43200 vs device 44100 it's a ~2% frame-timing deficit; if much lower it's a rate bug.
    {
        ...
        if (!primed) {
            ...
            // RUGRATS (session 18): cushion REMOVED â€” it sat in the SDL queue and inflated
    ...
    // RUGRATS (session 18, Reinhardt): diagnose popping + fast notes. If the game/audio runs faster
    ...
    // RUGRATS (session 18, Reinhardt): FIX for "loud static". `audio_data` points straight into the
...
size_t rugrats_get_frames_remaining() {
...
void rugrats_set_frequency(uint32_t freq) {
    fprintf(stderr, "[audio_freq] rugrats_set_frequency(%u) (was %u)\n", freq, current_freq);
    fflush(stderr);
    ...
    if (audio_device == 0) {
        fprintf(stderr, "[RUGRATSPC] SDL_OpenAudioDevice failed: %s\n", SDL_GetError());
        return;
    }
...
void rugrats_audio_init() {
    rugrats_set_frequency(current_freq);
}

// Public callback struct to pass to recomp::start().
ultramodern::audio_callbacks_t get_audio_callbacks() {
    return {
        .queue_samples      = rugrats_queue_samples,
        .get_frames_remaining = rugrats_get_frames_remaining,
        .set_frequency      = rugrats_set_frequency,
    };
}
