/**
 * main.cpp — Snowboard Kids PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Snowboard Kids game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 1583bacc9046a360df8ea4d536942155247e154c · internal name "Snowboard Kids"
static constexpr uint64_t SNOWKIDS_ROM_HASH        = 0xF384619787B78D4BULL;
static constexpr uint32_t SNOWKIDS_ENTRYPOINT_VRAM = 0x80000400;
...
static void snowkids_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        snowkids_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        snowkids_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
...
#endif
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Snowboard Kids PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void snowkids_message_box(const char* msg) {
    snowkids_show_error("Snowboard Kids — Error", msg);
}

static std::unique_ptr<ultramodern::renderer::RendererContext>
snowkids_create_render_context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode) {
#ifdef RDPC_CAPTURE_CHAIN
    ...
    if (preservation_core::lleconsole::active()) {
        ...
        extern void snowkids_lle_console_submit();
        preservation_core::lleconsole::set_submit_callback(&snowkids_lle_console_submit);
    ...
    snowkids_show_error("Renderer", "Console-only build: set RDPC_LLE_CONSOLE=1.");
    std::exit(1);
#else
    return snowkids::renderer::create_render_context(rdram, window_handle, developer_mode);
#endif
}

#if defined(_WIN32)
static LONG WINAPI snowkids_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in snowkidspc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in snowkidspc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Snowboard Kids — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void snowkids_vi_callback() {}
static void snowkids_gfx_init_callback() {}

static std::string snowkids_get_thread_name(const OSThread* t) {
    return "snowkids_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Snowboard Kids ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            snowkids_show_error("ROM Required",
                "A Snowboard Kids ROM is required.\n"
    ...
    // SNOWKIDS_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("SNOWKIDS_ROM");
    if (!rom_hint || !*rom_hint) {
        snowkids_show_error("ROM Required",
            "Pass the Snowboard Kids ROM path as the first argument,\n"
            "or set the SNOWKIDS_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"snowkids_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        snowkids_show_error("Wrong ROM",
            "The selected ROM does not match Snowboard Kids.\n"
            "Expected SHA1: 1583bacc9046a360df8ea4d536942155247e154c");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        snowkids_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        snowkids_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int snowkids_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(snowkids_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "snowkidspc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "snowkidspc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "snowkidspc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[snowkidspc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "snowkidspc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[snowkidspc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = SNOWKIDS_ROM_HASH;
    entry.internal_name      = "Snowboard Kids";
    entry.game_id            = u8"snowkids_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = SNOWKIDS_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"snowkids_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            snowkids_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = snowkids_create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = snowkids_vi_callback,
        .gfx_init_callback = snowkids_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = snowkids_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = snowkids_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return snowkids_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return snowkids_app_main(argc, argv);
}
#endif
