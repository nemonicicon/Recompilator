/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for MSPACMANMAZEMADNESPC.
 *
...
void mspacmanmazemadnes_poll_input() {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in mspacmanmazemadnes_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool mspacmanmazemadnes_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void mspacmanmazemadnes_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t mspacmanmazemadnes_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  mspacmanmazemadnes_get_input() already supplies
...
void mspacmanmazemadnes_input_init() {
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = mspacmanmazemadnes_poll_input,
        .get_input               = mspacmanmazemadnes_get_input,
        .set_rumble              = mspacmanmazemadnes_set_rumble,
        .get_connected_device_info = mspacmanmazemadnes_get_connected_device,
    };
}
