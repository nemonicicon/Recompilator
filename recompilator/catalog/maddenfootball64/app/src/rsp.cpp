/**
 * rsp.cpp — RSP microcode dispatch for MADDENFOOTBALL64PC (Maddenfootball64 64, USA). Game #4.
 *
 * AUDIO: Maddenfootball64 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void maddenfootball64_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* maddenfootball64_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[maddenfootball64pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = maddenfootball64_get_rsp_microcode,
        .capture_gfx = maddenfootball64_run_gfx_lle,
    };
}
