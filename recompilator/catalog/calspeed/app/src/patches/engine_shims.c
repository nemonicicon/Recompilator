 ...
 *     engine. Keyed to CV64 boot conditions; must never fire for CALSPEED.
 ...
 *     correct for static-only CALSPEED (overlay recompilation is PHASE 3).
...
/* 1 — CV64 NI boot-stub hook; loudly visible if it ever fires for CALSPEED. */
RECOMP_FUNC void gamestate_change(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    fprintf(stderr, "[engine_shim] gamestate_change(%d) called — CV64 NI boot-stub path fired for CALSPEED?!\n",
            (int)ctx->r4);
    fflush(stderr);
}
