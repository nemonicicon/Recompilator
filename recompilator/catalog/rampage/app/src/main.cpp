/**
 * main.cpp — Rampage World Tour PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Rampage World Tour game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 7a6c8758e67fd3da99e3420983c0d6df83e41c6b · internal name "Rampage World Tour"
static constexpr uint64_t RAMPAGE_ROM_HASH        = 0x4CAC6A61483A1AD8ULL;
static constexpr uint32_t RAMPAGE_ENTRYPOINT_VRAM = 0x80000400;
...
static void rampage_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        rampage_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Rampage World Tour PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        rampage_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Rampage World Tour PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void rampage_message_box(const char* msg) {
    rampage_show_error("Rampage World Tour — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI rampage_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in rampagepc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in rampagepc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Rampage World Tour — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void rampage_vi_callback() {}
static void rampage_gfx_init_callback() {}

static std::string rampage_get_thread_name(const OSThread* t) {
    return "rampage_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Rampage World Tour ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            rampage_show_error("ROM Required",
                "A Rampage World Tour ROM is required.\n"
    ...
    // RAMPAGE_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("RAMPAGE_ROM");
    if (!rom_hint || !*rom_hint) {
        rampage_show_error("ROM Required",
            "Pass the Rampage World Tour ROM path as the first argument,\n"
            "or set the RAMPAGE_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"rampage_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        rampage_show_error("Wrong ROM",
            "The selected ROM does not match Rampage World Tour.\n"
            "Expected SHA1: 7a6c8758e67fd3da99e3420983c0d6df83e41c6b");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        rampage_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        rampage_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int rampage_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(rampage_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "rampagepc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "rampagepc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "rampagepc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[rampagepc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "rampagepc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[rampagepc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = RAMPAGE_ROM_HASH;
    entry.internal_name      = "Rampage World Tour";
    entry.game_id            = u8"rampage_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::None;
    entry.is_enabled         = true;
    entry.entrypoint_address = RAMPAGE_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"rampage_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            rampage_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = rampage::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = rampage_vi_callback,
        .gfx_init_callback = rampage_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = rampage_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = rampage_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return rampage_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return rampage_app_main(argc, argv);
}
#endif
