/**
 * rsp.cpp — RSP microcode dispatch for WAVERACEPC (Waverace 64, USA). Console seam ported
 * from snowkidspc (2026-07-31 batch, starfox64pc port pattern).
 *
 * AUDIO: aspMain is THIS cartridge's own audio microcode, found in the cartridge by
 * tools/find_ucode.py and recompiled here, on this machine (rsp/aspMain.cpp). Until
 * 2026-09-07 one reference tree's recompiled microcode was copied into every game instead.
 * Many carts do carry the same stock SDK revision, so that was right for them by luck and
 * silently wrong for the rest — Army Men Sarge's Heroes played with no sound at all. If no
 * microcode is recognised, aspMain is a no-op that says so once and the game plays silently.
 * GFX: desktop tier HLE-rendered by RT64 via send_dl. Console tier: Wave Race 64 is a
 * Fast3D-era NO-gfx-string title (UCODE_CENSUS) — its gfx family is NOT in the catalog yet,
 * so the console serves through the UCODE REGISTRY only; a miss logs [ucode-miss] (the
 * bootstrap that names the family for its future catalog rung) and no-ops the task.
 */
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
// ── LLE CONSOLE (console seam ported from snowkidspc, 2026-07-31 batch): NO ARTIFACT YET ───
// Wave Race 64 = "NO gfx string" census class (Fast3D-era/custom); no recompiled gfx artifact
// exists for its family. The console tier consumes the UCODE REGISTRY; a MISS falls back to a
// logged no-op (empty RDP stream — the task lifecycle stays honest: stash/submit/dp_complete
// all still run). The organ's [ucode-miss] hash lines from the first Pi run name the family
// for its catalog rung — the same bootstrap every family went through.
static RspExitReason waverace_gfx_ucode_missing(uint8_t* /*rdram*/, uint32_t /*ucode_addr*/) {
    static int logs = 0;
    if (logs < 8) {
        logs++;
        fprintf(stderr, "[nonlle] gfx ucode not in catalog (Fast3D-era family, no artifact) — "
                        "task no-opped; see [ucode-miss] for the key\n");
        fflush(stderr);
    }
    return RspExitReason::Broke;
}

static RspUcodeFunc* waverace_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
    RspUcodeFunc* fn = recomp::rsp::dispatch_ucode(rdram, task);
    return fn != nullptr ? fn : waverace_gfx_ucode_missing;
}

// Desktop/HLE tier: no LLE capture (RT64 renders via send_dl).
static void waverace_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* waverace_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry serves
        // via waverace_run_gfx_capture; no recompiled artifact exists for this family yet.
        return rsp_noop_stub;
    default:
        fprintf(stderr, "[waveracepc] Unknown RSP task type %u — no-op stub\n",
...
// Ported verbatim from sm64pc/donkeykong64pc (the game-agnostic console machinery). The registry-
// served gfx ucode (none local yet — see the LLE CONSOLE note) runs per gfx task; DO_DP_END feeds the librecomp
...
static void waverace_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void waverace_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in waverace_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
void waverace_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    waverace_lle_render(rdram, job, n);
                }
...
static void waverace_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled gfx ucode (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = waverace_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            waverace_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            waverace_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) waverace_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/waverace_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=waverace tree=N64PC src=registry-LLE(no-artifact) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    ...
    // offline, SERVED_ROSTER rung, ENGINE_LEDGER 07-24). Wave Race ships the byte-identical
    // artifact (rsp/aspMain.cpp md5-matches snowkidspc's); a differing data window would
    // verify-MISS to the legacy direct call — safe either way. NO gfx registration: family
    // not in the catalog (see the LLE CONSOLE note above).
    recomp::rsp::register_ucode(0xEE85B9F8C730AF02ull, "aspMain", aspMain_logged,
                                0xD80, 0xA0, 0x19D507EA3E34D5F6ull);
    return {
        .get_rsp_microcode = waverace_get_rsp_microcode,
        // Console tier (-DRDPC_CAPTURE=ON): the preservation-core chain — registry-served gfx
        // -> rdpcore -> console VI. Desktop tier: inert no-op alongside RT64.
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = waverace_run_gfx_capture,
#else
        .capture_gfx = waverace_run_gfx_lle,
#endif
    };
}
