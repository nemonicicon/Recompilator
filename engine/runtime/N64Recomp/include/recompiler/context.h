...
namespace N64Recomp {
    struct Function {
        ...
        bool reimplemented;
        bool stubbed;
        // Emit the body as a live-gap dispatch (recomp_live_gap_set_target + trampoline) instead of
        // recompiling the words. Set by main.cpp when an auto-created static_ fails to recompile:
        // an EMPTY stub there is a silent no-op for a cross-function branch target that IS real code
        // (NC's audio-loop head static_15_8007C8B0 — the thread fell off its entry function), while
        // the trampoline JITs the RDRAM bytes at run time and fail-safes to a no-op on genuine data.
        bool gap_dispatch = false;
    ...
    struct ElfParsingConfig {
        ...
        // Functions whose manual size is EXACT ground truth — the severed-function auto-extension must
        // neither extend them NOR extend a neighbor across their start. Used to hand-bound a carved
        // bare-metal exception dispatch tail (NC: func_80080808 head + the func_80080A88/B84/C7C cause
        // handlers) so the head's jump table sizes (its cases become in-function gotos) while the carved
        // handlers stay independently recompiled+LOOKUP_FUNC-able (the runtime `jr $sN` dispatch needs
        // them) instead of being absorbed by a falls-off-end extension. Distinct from manually_sized_funcs
        // (e.g. CV64's auto gap-sizes) which DO still get extended when a wrong size lost their return.
        std::unordered_set<uint32_t> exact_size_func_vrams;
