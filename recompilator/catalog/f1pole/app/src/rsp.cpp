/**
 * rsp.cpp — RSP microcode dispatch for F1POLEPC (F1pole 64, USA). Game #4.
 *
 * AUDIO: F1pole ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void f1pole_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* f1pole_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[f1polepc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = f1pole_get_rsp_microcode,
        .capture_gfx = f1pole_run_gfx_lle,
    };
}
