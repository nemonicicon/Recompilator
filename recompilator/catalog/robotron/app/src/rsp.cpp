/**
 * rsp.cpp — RSP microcode dispatch for ROBOTRONPC (Robotron 64, USA). Game #4.
 *
 * AUDIO: Robotron ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
 * (verified in-ROM: aspMain text at Robotron rom 0x72910 matches SM64 rom 0xE7740 for 0x100,
 * aspMainData command jump table at PW rom 0x97be0). The RSPRecomp output generated for
 * cv64pc/sm64pc is therefore reused verbatim (rsp/aspMain.cpp) — the recompiled code is a
 * function of the ucode bytes only. FIFTH game on the same shared Nintendo-era ucode.
...
static void robotron_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* robotron_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[robotronpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = robotron_get_rsp_microcode,
        .capture_gfx = robotron_run_gfx_lle,
    };
}
