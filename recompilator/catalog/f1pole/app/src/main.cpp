/**
 * main.cpp — F1 Pole Position 64 PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── F1 Pole Position 64 game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 6e6bdaefc9980f066cd2abc5fa7f29f2353ca8b1 · internal name "F1 Pole Position 64"
static constexpr uint64_t F1POLE_ROM_HASH        = 0x179F0BEC46A7FA7AULL;
static constexpr uint32_t F1POLE_ENTRYPOINT_VRAM = 0x80000400;
...
static void f1pole_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        f1pole_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "F1 Pole Position 64 PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        f1pole_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "F1 Pole Position 64 PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void f1pole_message_box(const char* msg) {
    f1pole_show_error("F1 Pole Position 64 — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI f1pole_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in f1polepc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in f1polepc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "F1 Pole Position 64 — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void f1pole_vi_callback() {}
static void f1pole_gfx_init_callback() {}

static std::string f1pole_get_thread_name(const OSThread* t) {
    return "f1pole_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select F1 Pole Position 64 ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            f1pole_show_error("ROM Required",
                "A F1 Pole Position 64 ROM is required.\n"
    ...
    // F1POLE_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("F1POLE_ROM");
    if (!rom_hint || !*rom_hint) {
        f1pole_show_error("ROM Required",
            "Pass the F1 Pole Position 64 ROM path as the first argument,\n"
            "or set the F1POLE_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"f1pole_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        f1pole_show_error("Wrong ROM",
            "The selected ROM does not match F1 Pole Position 64.\n"
            "Expected SHA1: 6e6bdaefc9980f066cd2abc5fa7f29f2353ca8b1");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        f1pole_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        f1pole_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int f1pole_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(f1pole_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "f1polepc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "f1polepc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "f1polepc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[f1polepc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "f1polepc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[f1polepc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = F1POLE_ROM_HASH;
    entry.internal_name      = "F1 Pole Position 64";
    entry.game_id            = u8"f1pole_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = F1POLE_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"f1pole_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            f1pole_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = f1pole::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = f1pole_vi_callback,
        .gfx_init_callback = f1pole_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = f1pole_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = f1pole_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return f1pole_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return f1pole_app_main(argc, argv);
}
#endif
