/**
 * rsp.cpp — RSP microcode dispatch for MK4PC (Mk4 64, USA). Game #4.
 *
 * AUDIO: Mk4 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void mk4_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* mk4_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[mk4pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = mk4_get_rsp_microcode,
        .capture_gfx = mk4_run_gfx_lle,
    };
}
