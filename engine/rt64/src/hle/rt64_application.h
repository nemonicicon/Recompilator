...
namespace RT64 {
    ...
    struct Application : public ApplicationWindow::Listener {
        ...
        struct Core {
            ...
            // osViSetXScale/YScale half-res present factor (separate from VI_X_SCALE_REG). Optional:
            // null => 1.0 (no scaling), so apps that don't bind it are byte-identical.
            float *VI_X_SCALE_FACTOR = nullptr;
            float *VI_Y_SCALE_FACTOR = nullptr;
