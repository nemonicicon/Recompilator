/**
 * rsp.cpp — RSP microcode dispatch for CF63PC (Cf63 64, USA). Game #4.
 *
 * AUDIO: Cf63 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void cf63_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* cf63_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[cf63pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = cf63_get_rsp_microcode,
        .capture_gfx = cf63_run_gfx_lle,
    };
}
