/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for MK4PC.
 *
...
void mk4_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in mk4_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool mk4_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void mk4_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t mk4_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  mk4_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = mk4_poll_input,
        .get_input               = mk4_get_input,
        .set_rumble              = mk4_set_rumble,
        .get_connected_device_info = mk4_get_connected_device,
    };
}
