...
namespace RT64 {
    ...
    // An F3D revision that was in no database entry, so every display list using it was dropped
    // ("Unable to find a matching GBI ... not supported in HLE") and the game rendered nothing while
    // its audio and input ran normally. Identified from the cart: its microcode text is byte-identical
    // to a known 1996 F3D for 28 bytes after the entry jump and 45% overall, and the [gbihash]
    // instrument supplied both segment hashes below. Flags match the other plain 1996 F3D builds.
    const GBIInstance   F3D_SDK_UNKNOWN_I         = { "2.0?, 1996 (F3D SDK Unknown)",             GBIUCode::F3D,         { false,  false,  false,  false,  false } }; // Needs confirmation.
    ...
    // DKR 2026-09-04: Rare's in-house Fast3D. The ucode carries no "RSP SW Version" string; it names
    // itself "Version 7.7 29/09/97 15.00 L.Schuneman" (ROM 0xDDF4C). Text at RDRAM 0xDA970 / data at
    // 0xEABA0, a plain 1:1 cart image (ROM = RDRAM + 0xC00), hashes computed over the engine's swizzled
    // RDRAM byte order exactly as hashSegment() does. Assigned to F3D on the Blast Corps precedent
    // (Rare, 1997, F3D_BC); a Rare variant with moved parameter bits would show as drawn garbage, not
    // black - the GBIFlags above (triIndexDivisor / vtxLengthEncoded) are the knobs measured for SOTE.
    // Before this row every gfx task was dropped: [gbinull] every frame, 30 fps of black.
    // Attempt 1 mapped it to stock F3D: geometry placed correctly, flat white where textures belong,
    // and the DL census answered why - 39,408 of 199,098 commands had no handler, every 0x05 and 0x07.
    // It is Rare's DMA Fast3D family (DKR / JFG / Mickey's Speedway): GBIUCode::F3DDKR.
    const GBIInstance   F3D_DKR                   = { "Version 7.7 29/09/97 (F3DDKR Diddy Kong Racing)", GBIUCode::F3DDKR, { false,  false,  false,  false,  false } };
    // JFG 2026-09-06: the second member of Rare's in-house DMA Fast3D family the DKR row above predicted.
    // Jet Force Gemini's ucode names itself "Version 1.1 14/03/98 15:50 The Overlord" (ROM 0xA3DDC), a
    // 1998 rebuild of the same microcode by a different Rare author. Same blob layout as DKR (text, then
    // the version stamp, then data; ROM = RDRAM + 0xC00). MEASURED, not assumed: its DMEM data segment is
    // 482/512 words IDENTICAL to DKR's (94.1%) and every differing word is an IMEM jump-table pointer
    // (0x0F7F1080 vs DKR's 0x0EE71080) moved because the text was relaid out; the text itself is a fresh
    // build (46/1024 words). So the command DISPATCH is DKR's, but the PARAMETER layouts of two of the
    // geometry commands are not: disassembling both microcodes (2026-09-06, rabbitizer in RSP mode over
    // the cart images; IMEM layout taken from each ucode's own overlay table in DMEM 0x00-0x27) shows
    // JFG's DMA_MTX gained a source index and a multiply flag over an 8-deep matrix stack, and its
    // DMA_VTX takes an absolute destination slot instead of DKR's append counter, with no +1 on the
    // count. That is a second dialect of the same family -> GBIUCode::F3DDKR98 (see rt64_gbi_f3ddkr.cpp).
    // Text at RDRAM 0x9F3D0, data at 0x000B0350, both hashes taken live by the [gbihash] instrument below.
    // The text length is 0x1188, NOT DKR's 0x1400: JFG's loaded text stops being a copy of the cart between
    // 0x1188 and 0x1390 (offline and live hashes agree up to 0x1188 and disagree from 0x1390 on), so any
    // longer span would hash bytes the game rewrites. Data 0x800 is a clean cart image.
    // Before this row every gfx task was dropped: [gbinull] every frame, 30 fps of black.
    const GBIInstance   F3D_JFG                   = { "Version 1.1 14/03/98 (F3DDKR98 Jet Force Gemini)", GBIUCode::F3DDKR98, { false,  false,  false,  false,  false } };
    ...
    // SOTE 2026-07-18: stock-string Fast3D ("RSP SW Version: 2.0D, 04-01-96") in LucasArts' Shadows
    // of the Empire build; text blob is exactly 0x1430 (gfx text at 0xCDC60, audio text packed at
    // +0x1430). Hashes computed from the live RAM snapshot (word-swizzled like this engine's RDRAM).
    const GBIInstance   F3D_SOTE                  = { "2.0D, 04-01-96 (F3D Shadows of the Empire)", GBIUCode::F3D,        { false,  false,  false,  false,  false, 5, true } };   // triIndexDivisor=5 + vtxLengthEncoded: measured, see GBIFlags
    ...
    const GBIInstance   F3DEX2_NON_FIFO_ACCLAIM   = { "F3DEX2.NoN.fifo 2.05 (Acclaim)",            GBIUCode::F3DEX2,      { false,  true,   false,  false,  false } }; // Needs confirmation.
    ...
    //     
    static std::array<GBISegment, 98> textSegments = {
            GBISegment{ 0x1408,     0x9C0926F5E466BE70ULL,  { &F3D_SDK_E } }, // Needs confirmation.
            GBISegment{ 0x1408,     0xC2F203F2C4C77B54ULL,  { &F3D_SDK_UNKNOWN_I } }, // Needs confirmation.
            ...
            GBISegment{ 0x1430,     0xD33F14DF63CEA3ACULL,  { &F3D_SOTE } },
            ...
            GBISegment{ 0x1400,     0x6893D789BB054877ULL,  { &F3D_DKR } }, // DKR 2026-09-04: measured from the cart (see F3D_DKR).
            GBISegment{ 0x1188,     0x994AC031DDD1596CULL,  { &F3D_JFG } }, // JFG 2026-09-06: measured live by [gbihash] (see F3D_JFG).
    };

    static std::array<GBISegment, 109> dataSegments = {
            GBISegment{ 0x800,      0xEEB10D73400213B3ULL,  { &F3D_SDK_E } }, // Needs confirmation.
            GBISegment{ 0x800,      0x35785468616D7EC8ULL,  { &F3D_SDK_UNKNOWN_I } }, // Needs confirmation.
            ...
            GBISegment{ 0x800,      0xBD0BAAE05C1B4EE0ULL,  { &F3D_SOTE } },
            ...
            GBISegment{ 0x800,      0xC850B914F9B0EAAFULL,  { &F3D_DKR } }, // DKR 2026-09-04: measured from the cart (see F3D_DKR).
            GBISegment{ 0x800,      0x84AE0F119D75BD2CULL,  { &F3D_JFG } }, // JFG 2026-09-06: measured from the cart (see F3D_JFG).
    };
    ...
    GBI *GBIManager::getGBIForUCode(uint8_t *RDRAM, uint32_t textAddress, uint32_t dataAddress) {
        ...
        if (gbi.ucode == GBIUCode::Unknown) {
            ...
            case GBIUCode::F3DDKR:
                ...
                break;
            case GBIUCode::F3DDKR98:
                GBI_F3DDKR98::setup(&gbi);
                break;
            case GBIUCode::F3DEX:
                GBI_F3DEX::setup(&gbi);
                break;
    ...
    void GBIManager::deduceGBIInformation(uint8_t *RDRAM, uint32_t textAddress, uint32_t dataAddress) {
        ...
        // [gbihash] (JFG 2026-09-06, permanent instrument). Registering a cart whose microcode is absent
        // from the database means writing its text/data XXH3 rows above, and the ONLY authority on those
        // numbers is this function's own hashSegment(): it hashes RDRAM verbatim, i.e. the guest bytes in
        // the runtime's swizzled order, over a length that must already exist in the sorted table. Deriving
        // them offline from the cart works only while the segment is a 1:1 image of ROM and the exact
        // swizzle is reproduced — Jet Force Gemini's text is neither (its RDRAM copy diverges from the cart
        // past the first words), and one wrong length or one wrong byte order costs a full engine build to
        // discover. So print the real numbers here, once per unknown (text, data) pair, at every length the
        // table actually uses. Copy a line straight into textSegments/dataSegments.
        static std::set<std::pair<uint32_t, uint32_t>> reportedPairs;
        if (reportedPairs.insert({ textAddress, dataAddress }).second) {
            auto reportSegment = [&](const char *what, uint32_t address, const std::array<GBISegment, 98> *texts, const std::array<GBISegment, 109> *datas) {
                std::set<uint32_t> lengths;
                if (texts != nullptr) { for (const GBISegment &s : *texts) lengths.insert(s.hashLength); }
                if (datas != nullptr) { for (const GBISegment &s : *datas) lengths.insert(s.hashLength); }
                XXH3_state_t xxh3;
                XXH3_64bits_reset(&xxh3);
                const uint8_t *cursor = &RDRAM[address];
                uint32_t hashed = 0;
                for (uint32_t length : lengths) {
                    if (length <= hashed) continue;
                    XXH3_64bits_update(&xxh3, cursor, length - hashed);
                    cursor += (length - hashed);
                    hashed = length;
                    fprintf(stderr, "[gbihash] %s addr=0x%08X GBISegment{ 0x%X, 0x%016" PRIX64 "ULL, { &UCODE } },\n",
                            what, address, length, XXH3_64bits_digest(&xxh3));
                }
            };
            reportSegment("text", textAddress, &textSegments, nullptr);
            reportSegment("data", dataAddress, nullptr, &dataSegments);
            fflush(stderr);
        }
    }
