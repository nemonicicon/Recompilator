...
namespace RT64 {
    ...
    // A STRICT WEAK ORDERING, INCLUDING WHEN A DIFFERENCE IS NaN. difference comes from
    // matrixDifference() over the game's own view/projection matrices, which has no finite
    // check: one non-finite element makes it NaN. With a NaN every "<" is false, so the NaN
    // compares EQUIVALENT to every other element while the finite values stay ordered among
    // themselves - equivalence is then not transitive, the ordering is invalid, and
    // std::stable_sort on it is UNDEFINED. It walks past its buffer, which shows up either as
    // a corrupted heap (STATUS_HEAP_CORRUPTION 0xC0000374, thrown later by the allocator) or as
    // a wild pointer read inside ProjectionProcessor::processScene, which consumes this sorted
    // list. Both were seen on Pilotwings.
    //
    // Only reachable through GameFrame::match, which the workload queue calls only when
    // requiresFrameMatching - i.e. only when targetRate > 0, i.e. only at RefreshRate::Display
    // or Manual. At Original it is never called, which is why setting the refresh rate to
    // Display was the trigger; it was found that way, 2026-09-07.
    //
    // Finite values ascending, every NaN last, NaNs equivalent to one another.
    bool operator<(const MatchCandidate &lhs, const MatchCandidate &rhs) {
        const bool lhsNaN = std::isnan(lhs.difference);
        const bool rhsNaN = std::isnan(rhs.difference);
        if (lhsNaN || rhsNaN) {
        ...
        auto matchScenes = [&](const std::vector<GameScene> &curScenes, const std::vector<GameScene> &prevScenes) {
            ...
            for (uint32_t i = 0; i < curScenes.size(); i++) {
                ...
                for (uint32_t j = 0; j < prevScenes.size(); j++) {
                    ...
                    // A non-finite difference means "nothing measurable in common", which is
                    // FLT_MAX here, not NaN. The comparator above survives a NaN either way;
                    // this keeps one from reaching the matching logic and being treated as the
                    // best candidate in the list.
                    const float sceneDifference = matrixDifference(curViewTransform, prevViewTransform) + matrixDifference(curProjTransform, prevProjTransform);
                    matchCandidates.emplace_back(i, j, std::isfinite(sceneDifference) ? sceneDifference : FLT_MAX);
                }
            }
    ...
    void GameFrame::matchScene(WorkloadQueue &workloadQueue, const GameFrame &prevFrame, const GameScene &curScene, const GameScene &prevScene, std::unordered_map<uint32_t, ModifiedBuffers> &workloadsModified, bool &tileInterpolationUsed, bool &lookAtInterpolationUsed) {
        ...
        // SESSION 35: guard stale / out-of-range scene indices across a map transition.
        // matchScene (frame interpolation) indexes the CURRENT workloadQueue/frameMap with BOTH the
        // current and the PREVIOUS frame's projection indices. At a scene/map change the workload
        // count can shrink, leaving a stale prev index -> std::vector::operator[] is unchecked -> OOB
        // -> wild-pointer READ -> render-thread crash (Castlevania, Reinhardt, level 2). Frame interp is an
        // enhancement, so just skip matching this scene when any reference index is out of range.
        auto refInRange = [&](const GameIndices::Projection &pi) -> bool {
            if (pi.workloadIndex >= workloadQueue.workloads.size()) return false;
            if (pi.workloadIndex >= frameMap.workloads.size()) return false;
            const Workload &w = workloadQueue.workloads[pi.workloadIndex];
            if (pi.fbPairIndex >= w.fbPairs.size()) return false;
            if (pi.projectionIndex >= w.fbPairs[pi.fbPairIndex].projections.size()) return false;
            return true;
        };
        if (!refInRange(curScene.projections[0]) || !refInRange(prevScene.projections[0])) {
            static int _mg = 0;
            if (_mg++ < 16) { fprintf(stderr, "[matchguard] skip scene-match: stale/OOB projection index #%d\n", _mg); fflush(stderr); }
            return;
        }
            ...
            for (auto prevIt = prevRange.first; prevIt != prevRange.second; prevIt++) {
                ...
                if ((curCall.callDesc.tileCount == prevCall.callDesc.tileCount) && (curIt.second.doTileInterpolation && prevIt->second.doTileInterpolation)) {
                    for (uint32_t t = 0; t < curCall.callDesc.tileCount; t++) {
                        ...
                        // A pair is only worth checking when both tiles were recorded: the maps below are
                        // sized from the recorded tile lists, and an index past them is an out-of-bounds read.
                        if (((curCall.callDesc.tileIndex + t) >= curWorkload.drawData.rdpTiles.size()) ||
                            ((prevCall.callDesc.tileIndex + t) >= prevWorkload.drawData.rdpTiles.size()))
                        {
                            continue;
                        }
                ...
                if (curUsesTextureGen && prevUsesTextureGen && (true && true)) { // TODO: Do look at matching condition.
                    ...
                    // A draw can carry texture-gen lighting without a recorded look-at (a game that sets the
                    // geometry mode and never loads one): its index then points past the recorded list, and
                    // the look-at maps below are sized from that list. Matching such a pair reads out of
                    // bounds (SM64 title screen, 2026-09-09); there is nothing to interpolate for it anyway.
                    if ((curLookAtIndex >= curWorkload.drawData.rspLookAt.size()) ||
                        (prevLookAtIndex >= prevWorkload.drawData.rspLookAt.size()))
                    {
                        continue;
                    }
        ...
        for (const IndexPair &indices : transformCheckSet) {
            // The check sets are filled from every workload in the scene but applied to the FIRST
            // workload's arrays (the FIXME above); an index from another workload can lie past them.
        ...
        for (const IndexPair &indices : tileCheckSet) {
            if ((indices.first >= firstCurWorkloadMap.tiles.size()) || (indices.second >= firstCurWorkloadMap.prevTilesMapped.size())) {
                continue;
            }
        ...
        for (const IndexPair &indices : lookAtCheckSet) {
            if ((indices.first >= firstCurWorkloadMap.lookAt.size()) || (indices.second >= firstCurWorkloadMap.prevLookAtMapped.size())) {
                continue;
            }
