 ...
 *
 * SESSION 44: identified in BLASTCORPS by signature (tools/match_libultra.py); these
 * four are the ones BLASTCORPS's recompiled code actually calls.
 *
 ...
 * mapOverlay HLE), blastcorpspc has no game patches — the game's own recompiled
...
/* blastcorps links a SECOND copy of osSetEventMesg at ROM 0x896170; the multi-reference
 * libultra match names duplicates per-address, but no alias was generated (LNK2001,
 * found by the 2026-07-15 fresh sweep). Forward to the engine's HLE. */
extern void osSetEventMesg_recomp(uint8_t* rdram, recomp_context* ctx);
void osSetEventMesg_recomp_00896170(uint8_t* rdram, recomp_context* ctx) {
    osSetEventMesg_recomp(rdram, ctx);
}

// sweep 2026-09-02 attempt 2: the hd_code overlay carries its own osDestroyThread (0x802D6740); N64Recomp suffixes the
// duplicate name with its ROM offset (same as osSetEventMesg_recomp_00896170 above). Forward to the engine HLE.
extern void osDestroyThread_recomp(uint8_t* rdram, recomp_context* ctx);
void osDestroyThread_recomp_00891F80(uint8_t* rdram, recomp_context* ctx) {
