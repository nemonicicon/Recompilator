/**
 * rsp.cpp — RSP microcode dispatch for SHADOWSPC (Shadows 64, USA). Console seam ported
 * from snowkidspc (2026-07-31 batch, starfox64pc port pattern).
 *
 * AUDIO: rsp/aspMain.cpp is SOTE's OWN RSPRecomp output — NOT the shared CV64/SM64/LoD
 * artifact (md5 differs from snowkidspc's rsp/aspMain.cpp; SOTE is a 1996 LucasArts title
 * with its own ucode lineage). Served by the direct aspMain_logged call, exactly as before
 * the console seam port.
 * GFX: desktop tier HLE-rendered by RT64 via send_dl. Console tier: SOTE is a
 * NO-gfx-string title (UCODE_CENSUS — custom LucasArts ucode); its family is NOT in the catalog,
...
// Shadows of the Empire = "NO gfx string" census class (custom LucasArts ucode); no recompiled
// gfx artifact
...
static RspExitReason shadows_gfx_ucode_missing(uint8_t* /*rdram*/, uint32_t /*ucode_addr*/) {
    static int logs = 0;
    if (logs < 8) {
        logs++;
...
static RspUcodeFunc* shadows_gfx_ucode_for(uint8_t* rdram, const OSTask* task) {
    RspUcodeFunc* fn = recomp::rsp::dispatch_ucode(rdram, task);
    return fn != nullptr ? fn : shadows_gfx_ucode_missing;
}

// Desktop/HLE tier: no LLE capture (RT64 renders via send_dl).
static void shadows_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* shadows_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
        return aspMain_logged;
    case M_GFXTASK:
        // HLE tier: routed to RT64 via send_dl (not reached). Console tier: the registry serves
        // via shadows_run_gfx_capture; no recompiled artifact exists for this family yet.
        return rsp_noop_stub;
    default:
        fprintf(stderr, "[shadowspc] Unknown RSP task type %u — no-op stub\n",
...
// gfx ucode (shadows_recomp, f3dex208.cpp) runs per gfx task; DO_DP_END feeds the librecomp
...
static void shadows_lle_render(uint8_t* rdram, const std::vector<uint8_t>& stream,
                            uint32_t task_n) {
...
static void shadows_lle_stash(uint8_t* rdram, const std::vector<uint8_t>& stream, uint32_t task_n) {
    ...
    // (The post-render note in shadows_lle_render stays; the registry dedups.)
    for (uint32_t k = 0; k + 8 <= stream.size();) {
        uint8_t op = stream[k] & 0x3F;
        uint32_t nlen = 8;
        if (op >= 0x08 && op <= 0x0F)
...
void shadows_lle_console_submit() {
        ...
        std::thread([] {
            for (;;) {
                ...
                if (!job.empty() && rdram != nullptr) {
                    shadows_lle_render(rdram, job, n);
                }
...
static void shadows_run_gfx_capture(uint8_t* rdram, const OSTask* task) {
    ...
    // Stage + run the recompiled gfx ucode (shared dmem[] with the audio LLE — serialize).
    {
        ...
        RspExitReason r = shadows_gfx_ucode_for(rdram, task)(rdram, task->t.ucode);
        last_exit = (int)r;
    ...
    if (lle_render && !dump) {
        if (preservation_core::lleconsole::active()) {
            ...
            shadows_lle_stash(rdram, rdpc_stream_take(), task_n);
        } else {
            shadows_lle_render(rdram, rdpc_stream_take(), task_n);
        }
    }
    if (dump) {
        const std::vector<uint8_t>& stream = rdpc_stream_take();
        if (lle_render) shadows_lle_render(rdram, stream, task_n);
        char path[512];
        snprintf(path, sizeof(path), "%s/shadows_task_%u.rdpc", dump_dir, task_n);
        if (FILE* f = fopen(path, "wb")) {
            char meta[256];
            int meta_len = snprintf(meta, sizeof(meta),
                "game=shadows tree=N64PC src=registry-LLE(no-artifact) task=%u", task_n);
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    // UCODE CATALOG REGISTRY (organ 3): NOTHING registered for SOTE. Its aspMain is its OWN
    // recompile (md5 differs from the shared CV64/SM64 artifact — the EE85B9F8C730AF02 key
    // belongs to the SHARED bytes and must not be pointed at a different artifact), and its
    // gfx family is not in the catalog. Both serve via the legacy direct-call path; the
    // organ's [ucode-miss] lines from the first Pi run name the real keys for future rungs.
    return {
        .get_rsp_microcode = shadows_get_rsp_microcode,
...
#ifdef RDPC_CAPTURE_CHAIN
        .capture_gfx = shadows_run_gfx_capture,
#else
        .capture_gfx = shadows_run_gfx_lle,
#endif
    };
}
