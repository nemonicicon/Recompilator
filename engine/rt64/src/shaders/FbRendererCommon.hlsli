...
// single-texture mode (no descriptorIndexing): sets 1/2 are this draw's tile0/tile1 textures,
// bound per-draw by the framebuffer renderer. No bindless arrays, no raw-TMEM sampling.
// Set 1 - tile0 texture.
Texture2D<float4> gTexture0 : register(t0, space1);

// Set 2 - tile1 texture.
Texture2D<float4> gTexture1 : register(t0, space2);

// Threading helpers so shared sampling signatures only change in this permutation.
#define RT64_TEX_PARAM Texture2D<float4> stTexture,
#define RT64_TEX_ARG(t) t,
#define RT64_TEX_FWD stTexture,
#else
...
#define RT64_TEX_PARAM
#define RT64_TEX_ARG(t)
#define RT64_TEX_FWD
#endif
