/**
 * rsp.cpp — RSP microcode dispatch for BAM99PC (Bam99 64, USA). Game #4.
 *
 * AUDIO: Bam99 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void bam99_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* bam99_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[bam99pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = bam99_get_rsp_microcode,
        .capture_gfx = bam99_run_gfx_lle,
    };
}
