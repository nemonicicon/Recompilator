/**
 * rsp.cpp — RSP microcode dispatch for CHOPPERPC (Chopper 64, USA). Game #4.
 *
 * AUDIO: Chopper ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void chopper_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* chopper_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[chopperpc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = chopper_get_rsp_microcode,
        .capture_gfx = chopper_run_gfx_lle,
    };
}
