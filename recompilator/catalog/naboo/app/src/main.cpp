/**
 * main.cpp — Battle for Naboo PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Battle for Naboo game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: e4441a6eeb67861408c2e009baae8aad4df34021 · internal name "Battle for Naboo"
static constexpr uint64_t NABOO_ROM_HASH        = 0xF1FEF7568D4264FDULL;
static constexpr uint32_t NABOO_ENTRYPOINT_VRAM = 0x80000400;
...
static void naboo_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        naboo_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        naboo_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Battle for Naboo PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void naboo_message_box(const char* msg) {
    naboo_show_error("Battle for Naboo — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI naboo_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in naboopc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in naboopc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Battle for Naboo — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void naboo_vi_callback() {}
static void naboo_gfx_init_callback() {}

static std::string naboo_get_thread_name(const OSThread* t) {
    return "naboo_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Battle for Naboo ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            naboo_show_error("ROM Required",
                "A Battle for Naboo ROM is required.\n"
    ...
    // NABOO_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("NABOO_ROM");
    if (!rom_hint || !*rom_hint) {
        naboo_show_error("ROM Required",
            "Pass the Battle for Naboo ROM path as the first argument,\n"
            "or set the NABOO_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"naboo_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        naboo_show_error("Wrong ROM",
            "The selected ROM does not match Battle for Naboo.\n"
            "Expected SHA1: e4441a6eeb67861408c2e009baae8aad4df34021");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        naboo_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        naboo_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int naboo_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(naboo_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "naboopc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "naboopc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "naboopc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[naboopc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "naboopc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[naboopc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = NABOO_ROM_HASH;
    entry.internal_name      = "Battle for Naboo";
    entry.game_id            = u8"naboo_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = NABOO_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"naboo_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            naboo_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = naboo::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = naboo_vi_callback,
        .gfx_init_callback = naboo_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = naboo_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = naboo_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return naboo_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return naboo_app_main(argc, argv);
}
#endif
