/**
 * rsp.cpp — RSP microcode dispatch for READY2RUMBLEBOXING1PC (Ready2rumbleboxing1 64, USA). Game #4.
 *
 * AUDIO: Ready2rumbleboxing1 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void ready2rumbleboxing1_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* ready2rumbleboxing1_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[ready2rumbleboxing1pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = ready2rumbleboxing1_get_rsp_microcode,
        .capture_gfx = ready2rumbleboxing1_run_gfx_lle,
    };
}
