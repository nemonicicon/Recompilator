/**
 * main.cpp — Bottom of the 9th PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Bottom of the 9th game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 4ee0e3768b9f23112e4e5ef0c81a2b29ae22eab2 · internal name "Bottom of the 9th"
static constexpr uint64_t BOTTOM9_ROM_HASH        = 0x5867D468AD2CC7FAULL;
static constexpr uint32_t BOTTOM9_ENTRYPOINT_VRAM = 0x80000400;
...
static void bottom9_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        bottom9_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Bottom of the 9th PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        bottom9_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Bottom of the 9th PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void bottom9_message_box(const char* msg) {
    bottom9_show_error("Bottom of the 9th — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI bottom9_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in bottom9pc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in bottom9pc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Bottom of the 9th — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void bottom9_vi_callback() {}
static void bottom9_gfx_init_callback() {}

static std::string bottom9_get_thread_name(const OSThread* t) {
    return "bottom9_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Bottom of the 9th ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            bottom9_show_error("ROM Required",
                "A Bottom of the 9th ROM is required.\n"
    ...
    // BOTTOM9_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("BOTTOM9_ROM");
    if (!rom_hint || !*rom_hint) {
        bottom9_show_error("ROM Required",
            "Pass the Bottom of the 9th ROM path as the first argument,\n"
            "or set the BOTTOM9_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"bottom9_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        bottom9_show_error("Wrong ROM",
            "The selected ROM does not match Bottom of the 9th.\n"
            "Expected SHA1: 4ee0e3768b9f23112e4e5ef0c81a2b29ae22eab2");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        bottom9_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        bottom9_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int bottom9_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(bottom9_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "bottom9pc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "bottom9pc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "bottom9pc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[bottom9pc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "bottom9pc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[bottom9pc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = BOTTOM9_ROM_HASH;
    entry.internal_name      = "Bottom of the 9th";
    entry.game_id            = u8"bottom9_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = BOTTOM9_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"bottom9_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            bottom9_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }
    ...
    cfg.renderer_callbacks   = { .create_render_context = bottom9::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = bottom9_vi_callback,
        .gfx_init_callback = bottom9_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = bottom9_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = bottom9_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return bottom9_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return bottom9_app_main(argc, argv);
}
#endif
