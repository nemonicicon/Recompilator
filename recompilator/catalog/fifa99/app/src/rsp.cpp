/**
 * rsp.cpp — RSP microcode dispatch for FIFA99PC (Fifa99 64, USA). Game #4.
 *
 * AUDIO: Fifa99 ships the SAME audio microcode as CV64/SM64/LoD — byte-identical
...
static void fifa99_run_gfx_lle(uint8_t* /*rdram*/, const OSTask* /*task*/) {}

static RspUcodeFunc* fifa99_get_rsp_microcode(const OSTask* task) {
    switch (task->t.type) {
    case M_AUDTASK:
    ...
    default:
        fprintf(stderr, "[fifa99pc] Unknown RSP task type %u — no-op stub\n",
...
recomp::rsp::callbacks_t get_rsp_callbacks() {
    return {
        .get_rsp_microcode = fifa99_get_rsp_microcode,
        .capture_gfx = fifa99_run_gfx_lle,
    };
}
