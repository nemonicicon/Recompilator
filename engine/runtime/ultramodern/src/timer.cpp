...
// Game speed multiplier (1 means no speedup). [ff] env RECOMP_FF=<n> overrides for bench
// fast-forward: VI retraces, timers and the CPU counter all scale together, so the guest
// experiences a consistent (just faster) world — the whole intro in seconds, unattended.
static uint32_t speed_multiplier_init() {
...
static const uint32_t speed_multiplier = speed_multiplier_init();
...
void timer_thread(RDRAM_ARG1) {
    ...
    while (true) {
        ...
        if (wait_duration.count() >= 0 && timer_context.action_queue.wait_dequeue_timed(cur_action, wait_duration)) {
            ...
            // [timerfire] capped: wall-clock ms at fire, so an arm->fire->rearm cycle can be split
            // into "timer late" vs "guest slow". Pairs with [settimer] in ultra_translation.cpp.
            { static int _tf = 0; if (_tf++ < 24) {
                auto _ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::high_resolution_clock::now().time_since_epoch()).count();
                fprintf(stderr, "[timerfire] t=%lld ms mq=0x%08X%c", (long long)(_ms % 100000), (uint32_t)cur_timer->mq, 0x0A);
                fflush(stderr); } }
