...
namespace RT64 {
    struct VI {
        ...
        // osViSetXScale/YScale half-res present factor (1.0 = none). Carried separately from xTransform so
        // it never perturbs framebuffer sizing (deinterlacedWidth/fbAddress); only viewRectangle uses it.
        float xScaleFactor = 1.0f;
        float yScaleFactor = 1.0f;

        uint8_t fbSiz() const;
        uint32_t deinterlacedWidth() const;
