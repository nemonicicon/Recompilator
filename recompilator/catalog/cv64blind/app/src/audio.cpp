/**
 * audio.cpp â€” SDL2 push-mode audio backend for CV64BLINDPC.
 *
 * CV64BLIND uses Konami's custom audio sequencer running at 32 kHz.
...
void cv64blind_queue_samples(int16_t* audio_data, size_t sample_count) {
    ...
    // CV64BLIND (session 18, Reinhardt): cushion + production-rate measurement.
    ...
    //     ~43200 vs device 44100 it's a ~2% frame-timing deficit; if much lower it's a rate bug.
    {
        ...
        if (!primed) {
            ...
            // CV64BLIND (session 18): cushion REMOVED â€” it sat in the SDL queue and inflated
    ...
    // CV64BLIND (session 18, Reinhardt): diagnose popping + fast notes. If the game/audio runs faster
    ...
    if (SDL_GetQueuedAudioSize(audio_device) > MAX_QUEUED_BYTES) {
        // [tempo 20260906] 1/s drop census. A DROPPED buffer is audio the game's sequencer already
        // advanced through, so a steady drop rate reads to the ear as "the music runs fast".
        // Counting only -- the drop behaviour itself is unchanged.
        { static unsigned long long _drop = 0, _last = 0;
          static std::chrono::steady_clock::time_point _t0 = std::chrono::steady_clock::now();
          _drop++;
          double _s = std::chrono::duration<double>(std::chrono::steady_clock::now() - _t0).count();
          if (_s >= 1.0) {
              fprintf(stderr, "[audio_drop] %llu dropped buffers in %.2fs (total %llu)\n",
                      _drop - _last, _s, _drop);
              fflush(stderr);
              _last = _drop; _t0 = std::chrono::steady_clock::now();
          } }
        return;
    }
    ...
    // CV64BLIND (session 18, Reinhardt): FIX for "loud static". `audio_data` points straight into the
    ...
    // [tempo 20260906] RECOMP_PCM_DUMP=<path>: capped raw dump of exactly what reaches the DAC
    // (int16 stereo little-endian at the device rate) so the musical period can be measured
    // offline by autocorrelation. Inert unless the env is set. RECOMP_PCM_DUMP_SECS caps it (60).
    {
        static FILE* _pf = [] () -> FILE* {
            const char* p = getenv("RECOMP_PCM_DUMP");
            if (p == nullptr || p[0] == '\0') return nullptr;
            FILE* f = fopen(p, "wb");
            fprintf(stderr, "[pcmdump] %s -> %s\n", f ? "open" : "FAILED", p); fflush(stderr);
            return f;
        }();
        static bool _closed = false;
        if (_pf != nullptr && !_closed) {
            static const unsigned long long _cap_secs = [] {
                const char* e = getenv("RECOMP_PCM_DUMP_SECS");
                double v = (e && e[0]) ? atof(e) : 60.0;
                return (unsigned long long)(v > 0.0 ? v : 60.0);
            }();
            static unsigned long long _written = 0;
            if (_written >= _cap_secs * (unsigned long long)current_freq) {
                fclose(_pf); _closed = true;
                fprintf(stderr, "[pcmdump] closed at %llu frames (device=%uHz)\n", _written, current_freq);
                fflush(stderr);
            } else {
                for (size_t i = 0; i < pair_count; i++) {
                    int16_t s[2] = { (int16_t)(float_buf[i * 2 + 0] * 32767.0f),
                                     (int16_t)(float_buf[i * 2 + 1] * 32767.0f) };
                    fwrite(s, sizeof(int16_t), 2, _pf);
                }
                _written += pair_count;
            }
        }
...
size_t cv64blind_get_frames_remaining() {
...
void cv64blind_set_frequency(uint32_t freq) {
    fprintf(stderr, "[audio_freq] cv64blind_set_frequency(%u) (was %u)\n", freq, current_freq);
    fflush(stderr);
    ...
    if (audio_device == 0) {
        fprintf(stderr, "[CV64BLINDPC] SDL_OpenAudioDevice failed: %s\n", SDL_GetError());
        return;
    }
...
void cv64blind_audio_init() {
    cv64blind_set_frequency(current_freq);
}

// Public callback struct to pass to recomp::start().
ultramodern::audio_callbacks_t get_audio_callbacks() {
    return {
        .queue_samples      = cv64blind_queue_samples,
        .get_frames_remaining = cv64blind_get_frames_remaining,
        .set_frequency      = cv64blind_set_frequency,
    };
}
