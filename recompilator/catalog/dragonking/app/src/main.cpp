/**
 * main.cpp — Dragon King (Smash Bros prototype) PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Dragon King (Smash Bros prototype) game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: a9f1cd8c6edb24e12c48a1a263dfd74bde283e23 · internal name "Dragon King (Smash Bros prototype)"
static constexpr uint64_t DRAGONKING_ROM_HASH        = 0x2CD1513C501F56E6ULL;
static constexpr uint32_t DRAGONKING_ENTRYPOINT_VRAM = 0x80140000;
...
static void dragonking_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        dragonking_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        "Dragon King (Smash Bros prototype) PC",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1920, 1080,
        win_flags);
    if (!sdl_window) {
        dragonking_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Dragon King (Smash Bros prototype) PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void dragonking_message_box(const char* msg) {
    dragonking_show_error("Dragon King (Smash Bros prototype) — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI dragonking_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in dragonkingpc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in dragonkingpc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Dragon King (Smash Bros prototype) — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void dragonking_vi_callback() {}
static void dragonking_gfx_init_callback() {}

static std::string dragonking_get_thread_name(const OSThread* t) {
    return "dragonking_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Dragon King (Smash Bros prototype) ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            dragonking_show_error("ROM Required",
                "A Dragon King (Smash Bros prototype) ROM is required.\n"
    ...
    // DRAGONKING_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("DRAGONKING_ROM");
    if (!rom_hint || !*rom_hint) {
        dragonking_show_error("ROM Required",
            "Pass the Dragon King (Smash Bros prototype) ROM path as the first argument,\n"
            "or set the DRAGONKING_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"dragonking_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        dragonking_show_error("Wrong ROM",
            "The selected ROM does not match Dragon King (Smash Bros prototype).\n"
            "Expected SHA1: a9f1cd8c6edb24e12c48a1a263dfd74bde283e23");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        dragonking_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        dragonking_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int dragonking_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(dragonking_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "dragonkingpc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "dragonkingpc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "dragonkingpc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[dragonkingpc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "dragonkingpc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[dragonkingpc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = DRAGONKING_ROM_HASH;
    entry.internal_name      = "Dragon King (Smash Bros prototype)";
    entry.game_id            = u8"dragonking_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = DRAGONKING_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"dragonking_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            dragonking_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    dragonking_audio_init();
    dragonking_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = dragonking::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = dragonking_vi_callback,
        .gfx_init_callback = dragonking_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = dragonking_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = dragonking_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return dragonking_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return dragonking_app_main(argc, argv);
}
#endif
