/**
 * main.cpp — Banjo-Kazooie PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Banjo-Kazooie game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: 1fe1632098865f639e22c11b9a81ee8f29c75d7a · internal name "Banjo-Kazooie"
static constexpr uint64_t BANJOKAZOOIE_ROM_HASH        = 0x1B67585D56E07F8CULL;
static constexpr uint32_t BANJOKAZOOIE_ENTRYPOINT_VRAM = 0x80000400;
...
static void banjokazooie_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        banjokazooie_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        banjokazooie_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Banjo-Kazooie PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void banjokazooie_message_box(const char* msg) {
    banjokazooie_show_error("Banjo-Kazooie — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI banjokazooie_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in banjokazooiepc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in banjokazooiepc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Banjo-Kazooie — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void banjokazooie_vi_callback() {}
static void banjokazooie_gfx_init_callback() {}

static std::string banjokazooie_get_thread_name(const OSThread* t) {
    return "banjokazooie_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        if (!GetOpenFileNameA(&ofn)) {
            banjokazooie_show_error("ROM Required",
                "A Banjo-Kazooie ROM is required.\n"
    ...
    // BANJOKAZOOIE_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("BANJOKAZOOIE_ROM");
    if (!rom_hint || !*rom_hint) {
        banjokazooie_show_error("ROM Required",
            "Pass the Banjo-Kazooie ROM path as the first argument,\n"
            "or set the BANJOKAZOOIE_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"banjokazooie_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        banjokazooie_show_error("Wrong ROM",
            "The selected ROM does not match Banjo-Kazooie.\n"
            "Expected SHA1: 1fe1632098865f639e22c11b9a81ee8f29c75d7a");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        banjokazooie_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        banjokazooie_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int banjokazooie_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(banjokazooie_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "banjokazooiepc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "banjokazooiepc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "banjokazooiepc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[banjokazooiepc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "banjokazooiepc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[banjokazooiepc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = BANJOKAZOOIE_ROM_HASH;
    entry.internal_name      = "Banjo-Kazooie";
    entry.game_id            = u8"banjokazooie_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Eep4k;
    entry.is_enabled         = true;
    entry.entrypoint_address = BANJOKAZOOIE_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"banjokazooie_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            banjokazooie_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    banjokazooie_audio_init();
    banjokazooie_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = banjokazooie::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = banjokazooie_vi_callback,
        .gfx_init_callback = banjokazooie_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = banjokazooie_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = banjokazooie_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return banjokazooie_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return banjokazooie_app_main(argc, argv);
}
#endif
