...
namespace RT64 {
    ...
    bool FramebufferManager::makeFramebufferTile(Framebuffer *fb, uint32_t addressStart, uint32_t addressEnd, uint32_t lineWidth, uint32_t tileHeight, FramebufferTile &outTile, bool RGBA32) {
        ...
        // SESSION 28 cont.18e: a degenerate framebuffer (width 0 → stride 0) makes the loop below an
        // INFINITE loop AND the `/ fbStride` divisions below an integer divide-by-zero (0xC0000094 crash,
        // hit on the Necronomicon/menu book rendering). Skip the tile copy instead — same class as the
        // existing setColorImage/loadTLUT invariant guards.
        if (fbStride == 0) {
            static int _ftz = 0;
            if (++_ftz <= 32) { fprintf(stderr, "[fbtile] skip: fbStride=0 (fb->width=%u siz=%u)\n", fb->width, (unsigned)fb->siz); fflush(stderr); }
            return false;
        }
        ...
        // cont.18e: a 4-bit framebuffer (siz=0 → pixelSize=0) makes the `% pixelSize` below a
        // divide-by-zero. Skip the tile copy instead.
        if (pixelSize == 0) {
            static int _ftz2 = 0;
            if (++_ftz2 <= 32) { fprintf(stderr, "[fbtile] skip: pixelSize=0 (siz=%u)\n", (unsigned)fb->siz); fflush(stderr); }
            return false;
        }
        ...
        const uint8_t *RDRAM, bool canDiscard, std::vector<FramebufferOperation> &fbOps, std::vector<uint32_t> &fbDiscards, const ShaderLibrary *shaderLibrary)
    {
        ...
        for (size_t i = 0; i < differentFbsCount; i++) {
            ...
            // [fbdiscard] KI Gold 2026-08-29. This 1/4 is the RETENTION heuristic: if the game
            // modified >=25% of the framebuffer in RDRAM, RT64 throws its GPU copy away and
            // rebuilds from RAM; below that it KEEPS the GPU target and writes only the changed
            // regions in. MEASURED: KI Gold never clears the colour buffer (exactly ONE fillRect
            // in 60s) and repaints only PARTIAL regions, so correct output depends entirely on
            // which side of this threshold each frame lands. Too sticky -> stale pixels survive
            // (persistence); flipping across frames -> alternating stale/fresh (flashing). Both
            // are what was reported, together.
            // RECOMP_FB_DISCARD_PCT=N sets the percentage (default 25). 0 = ALWAYS rebuild from
            // RAM (the game's own framebuffer wins); 100 = never discard.
            static const uint32_t DiscardPct = [] {
                const char *e = std::getenv("RECOMP_FB_DISCARD_PCT");
                if (e == nullptr) return 25u;
                unsigned long v = strtoul(e, nullptr, 10);
                if (v > 100u) v = 100u;
                fprintf(stderr, "[fbdiscard] threshold overridden to %lu%% (RECOMP_FB_DISCARD_PCT)\n", v);
                fflush(stderr);
                return (uint32_t)v;
            }();
            const uint32_t DifferenceFractionNum = DiscardPct;
            const uint32_t DifferenceFractionDiv = 100;
