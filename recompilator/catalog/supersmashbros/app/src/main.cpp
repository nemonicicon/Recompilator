/**
 * main.cpp — Super Smash Bros. PC port entry point. Generated by recompilator/scaffold_app.py.
 *
...
// ── Super Smash Bros. game identity (recon: ROM header + XXH3) ───────────────────────
// SHA1: e2929e10fccc0aa84e5776227e798abc07cedabf · internal name "Super Smash Bros."
static constexpr uint64_t SUPERSMASHBROS_ROM_HASH        = 0x38912AC86097BFECULL;
static constexpr uint32_t SUPERSMASHBROS_ENTRYPOINT_VRAM = 0x80000400;
...
static void supersmashbros_show_error(const char* title, const char* msg) {
#if defined(_WIN32)
...
static ultramodern::gfx_callbacks_t::gfx_data_t create_gfx() {
    if (SDL_InitSubSystem(SDL_INIT_VIDEO) != 0) {
        supersmashbros_show_error("SDL_InitSubSystem Error", SDL_GetError());
    }
    return nullptr;
}
...
create_window(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/)
{
    ...
    sdl_window = SDL_CreateWindow(
        ...
        win_flags);
    if (!sdl_window) {
        supersmashbros_show_error("Window Creation Error", SDL_GetError());
        std::exit(1);
    }
...
static void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t /*gfx_data*/) {
    ...
    }
    {
        ...
        if (sdl_window != nullptr && fps != last_fps) {
            last_fps = fps;
            char title[80];
            snprintf(title, sizeof(title), "Super Smash Bros. PC  -  %.1f FPS", fps);
            SDL_SetWindowTitle(sdl_window, title);
        }
    }
}

static void supersmashbros_message_box(const char* msg) {
    supersmashbros_show_error("Super Smash Bros. — Error", msg);
}

#if defined(_WIN32)
static LONG WINAPI supersmashbros_unhandled_exception(EXCEPTION_POINTERS* ep) {
    fflush(stderr);
    ...
    snprintf(msg, sizeof(msg),
        "UNHANDLED EXCEPTION\n\nCode:        0x%08X\nAddress:     0x%p\n"
        "RVA (map):   0x%08llX%s\n\nThis offset matches a line in supersmashbrospc.map.\n"
    ...
    __try {
        ...
        if (exe != nullptr && ctx != nullptr) {
            ...
            fprintf(stderr, "[crashstack] Rsp=0x%p — call sites in supersmashbrospc.exe (base=0x%p), nearest first:\n",
    ...
    MessageBoxA(nullptr, msg, "Super Smash Bros. — Crash", MB_ICONERROR | MB_OK);
    return EXCEPTION_CONTINUE_SEARCH;
}
#endif // _WIN32 (crash handler)

static void supersmashbros_vi_callback() {}
static void supersmashbros_gfx_init_callback() {}

static std::string supersmashbros_get_thread_name(const OSThread* t) {
    return "supersmashbros_" + std::to_string(t->id);
}
...
static void prompt_for_rom(const char* rom_hint) {
    ...
    if (rom_hint && *rom_hint) {
        ...
        ofn.lpstrTitle  = "Select Super Smash Bros. ROM";
        ofn.Flags       = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
        if (!GetOpenFileNameA(&ofn)) {
            supersmashbros_show_error("ROM Required",
                "A Super Smash Bros. ROM is required.\n"
    ...
    // SUPERSMASHBROS_ROM env var (e.g. an SBC/console that already has the ROM at a known path).
    if (!rom_hint || !*rom_hint) rom_hint = std::getenv("SUPERSMASHBROS_ROM");
    if (!rom_hint || !*rom_hint) {
        supersmashbros_show_error("ROM Required",
            "Pass the Super Smash Bros. ROM path as the first argument,\n"
            "or set the SUPERSMASHBROS_ROM environment variable.");
        std::exit(0);
    }
    rom_path = rom_hint;
#endif

    std::u8string out_id = u8"supersmashbros_us";
    ...
    case recomp::RomValidationError::IncorrectVersion:
        supersmashbros_show_error("Wrong ROM",
            "The selected ROM does not match Super Smash Bros..\n"
            "Expected SHA1: e2929e10fccc0aa84e5776227e798abc07cedabf");
        std::exit(1);
    case recomp::RomValidationError::NotARom:
        supersmashbros_show_error("Not an N64 ROM", "The selected file does not appear to be an N64 ROM.");
        std::exit(1);
    default:
        supersmashbros_show_error("ROM Error", "Failed to read the selected ROM file.");
        std::exit(1);
    }
}

static int supersmashbros_app_main(int argc, char** argv)
{
    const char* rom_hint = (argc > 1) ? argv[1] : nullptr;

#if defined(_WIN32)
    SetUnhandledExceptionFilter(supersmashbros_unhandled_exception);
#endif
    SDL_SetMainReady();
    ...
    if (!appdata) appdata = ".";
    std::filesystem::path config_path = std::filesystem::path(appdata) / "supersmashbrospc";
#else
    ...
    std::filesystem::path config_base =
        (xdg && *xdg) ? std::filesystem::path(xdg)
                      : (home ? std::filesystem::path(home) / ".config"
                              : std::filesystem::path("."));
    std::filesystem::path config_path = config_base / "supersmashbrospc";
#endif
    std::filesystem::create_directories(config_path);

    {
        std::filesystem::path log_path = config_path / "supersmashbrospc_diag.log";
        FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
        if (f) { setvbuf(f, nullptr, _IONBF, 0); fprintf(stderr, "[supersmashbrospc] diagnostic log started\n"); }
        std::filesystem::path stdout_path = config_path / "supersmashbrospc_stdout.log";
        FILE* fout = nullptr;
#if defined(_WIN32)
...
#endif
        if (fout) { setvbuf(fout, nullptr, _IONBF, 0); fprintf(stdout, "[supersmashbrospc] stdout log started\n"); }
    }

    recomp::register_config_path(config_path);

    recomp::GameEntry entry{};
    entry.rom_hash           = SUPERSMASHBROS_ROM_HASH;
    entry.internal_name      = "Super Smash Bros.";
    entry.game_id            = u8"supersmashbros_us";
    entry.mod_game_id        = "";
    entry.save_type          = recomp::SaveType::Sram;
    entry.is_enabled         = true;
    entry.entrypoint_address = SUPERSMASHBROS_ENTRYPOINT_VRAM;
    ...
    std::u8string game_id = u8"supersmashbros_us";
    if (!recomp::is_rom_valid(game_id)) {
        prompt_for_rom(rom_hint);
        recomp::check_all_stored_roms();
        if (!recomp::is_rom_valid(game_id)) {
            supersmashbros_show_error("Error", "ROM validation failed after selection.");
            return 1;
        }
    }

    SDL_Init(SDL_INIT_AUDIO);
    supersmashbros_audio_init();
    supersmashbros_input_init();
    ...
    cfg.renderer_callbacks   = { .create_render_context = supersmashbros::renderer::create_render_context };
    ...
    cfg.events_callbacks     = {
        .vi_callback       = supersmashbros_vi_callback,
        .gfx_init_callback = supersmashbros_gfx_init_callback,
    };
    cfg.error_handling_callbacks = { .message_box = supersmashbros_message_box };
    cfg.threads_callbacks    = { .get_game_thread_name = supersmashbros_get_thread_name };
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    return supersmashbros_app_main(__argc, __argv);   // CRT-provided argc/argv
}
#else
int main(int argc, char** argv) {
    return supersmashbros_app_main(argc, argv);
}
#endif
