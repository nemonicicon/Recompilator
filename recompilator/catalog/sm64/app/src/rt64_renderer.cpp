/**
 * rt64_renderer.cpp — RT64Context implementation for SM64PC.
 *
 * Same wrapper as cv64pc's (the engine-proven shape), minus the CV64-specific
 * DL-dump probes. SM64 uses the same F3DEX2 fifo 2.06 microcode as CV64.
 */
                      ...
                      bool developer_mode)
{
    ...
    // PLAN_V2 P3 console mode: RDPC_LLE_CONSOLE=1 replaces RT64 with the preservation core
    // (rdpcore renders into RDRAM via the capture chain; fb_present scans it out). RT64 is
    // never initialized — no Vulkan/D3D anywhere in the process.
    if (sm64::lleconsole::active()) {
        return sm64::lleconsole::create_console_context(rdram);
    }
#endif
