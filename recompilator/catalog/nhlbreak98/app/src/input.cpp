/**
 * input.cpp â€” SDL2 gamepad / keyboard input backend for NHLBREAK98PC.
 *
...
void nhlbreak98_poll_input() {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_CONTROLLERDEVICEREMOVED:
            ...
            break;
        case SDL_QUIT:
            fprintf(stderr, "[quitwatch] SDL_QUIT received in nhlbreak98_poll_input\n"); fflush(stderr);
            ultramodern::quit();
            break;
        default:
            break;
...
bool nhlbreak98_get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
...
void nhlbreak98_set_rumble(int controller_num, bool rumble) {
...
ultramodern::input::connected_device_info_t nhlbreak98_get_connected_device(int controller_num) {
    if (controller_num != 0) {
        return { ultramodern::input::Device::None, ultramodern::input::Pak::None };
    }
    // Always report port 0 as having a controller.  nhlbreak98_get_input() already supplies
...
ultramodern::input::callbacks_t get_input_callbacks() {
    return {
        .poll_input              = nhlbreak98_poll_input,
        .get_input               = nhlbreak98_get_input,
        .set_rumble              = nhlbreak98_set_rumble,
        .get_connected_device_info = nhlbreak98_get_connected_device,
    };
}
